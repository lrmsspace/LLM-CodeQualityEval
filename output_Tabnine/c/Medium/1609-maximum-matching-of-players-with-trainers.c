...
int matchPlayersAndTrainers(int* players, int playersSize, int* trainers, int trainersSize) {
    ...
    while (i < playersSize && j < trainersSize) {
        // If the current player's ability is less than or equal to the trainer's capacity, increment the maxMatchings
        if (players[i] <= trainers[j]) {
            maxMatchings++;
            i++;
            j++;
        }
        // If the current player's ability is greater than the trainer's capacity, increment the trainer's index
        else {
            j++;
        }
    }
