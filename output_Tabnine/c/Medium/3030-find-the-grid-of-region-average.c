    ...
    *returnSize = imageSize;
    for (int i = 0; i < imageSize; i++) {
        ...
        for (int j = 0; j < imageColSize[i]; j++) {
            ...
            for (int k = -1; k <= 1; k++) {
                for (int l = -1; l <= 1; l++) {
                    if (i + k >= 0 && i + k < imageSize && j + l >= 0 && j + l < imageColSize[i]) {
                        sum += image[i + k][j + l];
                        count++;
                    }
                }
            ...
            while (result[i][j] >= threshold) {
                result[i][j]--;
            }
        }
        result[i][imageColSize[i] - 1] = image[i][imageColSize[i] - 1];
        result[i][0] = image[i][0];
        for (int j = 1; j < imageColSize[i] - 1; j++) {
