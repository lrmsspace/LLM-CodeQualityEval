...
double maxAmount(char* initialCurrency, char*** pairs1, int pairs1Size, int* pairs1ColSize, double* rates1, int rates1Size, char*** pairs2, int pairs2Size, int* pairs2ColSize, double* rates2, int rates2Size) {
    ...
    for (int i = 0; i < pairs1Size; i++) {
        char* startCurrency = pairs1[i][0];
        char* targetCurrency = pairs1[i][1];
        double rate = rates1[i];

        if (strcmp(initialCurrency, startCurrency) == 0) {
            amount *= rate;
            strcpy(initialCurrency, targetCurrency);
        }
    }

    // Perform conversions on day 2
    for (int i = 0; i < pairs2Size; i++) {
        char* startCurrency = pairs2[i][0];
        char* targetCurrency = pairs2[i][1];
