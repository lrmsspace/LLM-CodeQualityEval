...
int** specialGrid(int n, int* returnSize, int** returnColumnSizes) {
    ...
    *returnColumnSizes = (int*)malloc(sizeof(int) * n);
    ...
    int topRight = 0;
    int bottomRight = quadrantSize * quadrantSize;
    int bottomLeft = bottomRight + quadrantSize;
    int topLeft = bottomLeft + quadrantSize;    
    for (int i = 0; i < quadrantSize; i++) {
        for (int j = 0; j < quadrantSize; j++) {
            grid[i][j] = topRight++;
            grid[i][n - j - 1] = bottomRight--;
            grid[n - i - 1][j] = bottomLeft++;
            grid[n - i - 1][n - j - 1] = topLeft--;
        }
    }
    return grid;
