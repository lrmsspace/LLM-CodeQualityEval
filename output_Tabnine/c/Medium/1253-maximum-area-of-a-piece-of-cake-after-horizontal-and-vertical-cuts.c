...
int maxArea(int h, int w, int* horizontalCuts, int horizontalCutsSize, int* verticalCuts, int verticalCutsSize) {
    ...
    // Calculate the maximum length of the cake after cutting horizontally
    int maxH = 0;
    for (int i = 1; i < horizontalCutsSize; i++) {
        maxH = max(maxH, horizontalCuts[i] - horizontalCuts[i - 1]);
    }
    maxH = max(maxH, h - horizontalCuts[horizontalCutsSize - 1]);
    
    // Calculate the maximum length of the cake after cutting vertically
    int maxW = 0;
    for (int i = 1; i < verticalCutsSize; i++) {
        maxW = max(maxW, verticalCuts[i] - verticalCuts[i - 1]);
    }
    maxW = max(maxW, w - verticalCuts[verticalCutsSize - 1]);
    
    // Calculate the maximum area of the cake
    return (long)maxH * maxW % (int)(1e9 + 7);
