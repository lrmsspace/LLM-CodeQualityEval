...
int* baseUnitConversions(int** conversions, int conversionsSize, int* conversionsColSize, int* returnSize) {
    int* result = (int*)malloc(sizeof(int) * conversionsSize);
    *returnSize = conversionsSize;
    
    // Initialize baseUnitConversion array with 1 for each unit
    for (int i = 0; i < conversionsSize; i++) {
        result[i] = 1;
    }
    // Perform conversions from unit 0 to other units
    ...
    for (int i = 0; i < conversionsSize; i++) {
        ...
        // Update baseUnitConversion array for each unit that can be reached from unit 0
        for (int j = 0; j < conversionsSize; j++) {
            if (conversions[j][1] == sourceUnit) {
                result[targetUnit] = (result[targetUnit] + (result[conversions[j][0]] * conversionFactor) % 1000000007) % 1000000007;
            }
        }
    }
