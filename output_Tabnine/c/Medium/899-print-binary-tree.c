...
char*** printTree(struct TreeNode* root, int* returnSize, int** returnColumnSizes) {
    ...
    *returnColumnSizes = malloc(height * sizeof(int));
    (*returnColumnSizes)[0] = pow(2, height) - 1;
    
    // Initialize the matrix with empty strings
    char*** res = malloc(height * sizeof(char**));
    for (int i = 0; i < height; i++) {
        (*returnColumnSizes)[i] = 2 * pow(2, height - i) - 1;
        res[i] = malloc((*returnColumnSizes)[i] * sizeof(char*));
        for (int j = 0; j < (*returnColumnSizes)[i]; j++) {
            res[i][j] = malloc(2 * height + 1 * sizeof(char));
            res[i][j][0] = '\0';
        }
    }
    
    // Construct the matrix recursively
    constructMatrix(root, 0, (height - 1) / 2, res);
