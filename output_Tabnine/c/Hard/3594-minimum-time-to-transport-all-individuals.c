/*
 ...
 0.5 <= mul[i] <= 2.0
*/
double minTimeHelper(int n, int k, int m, int* time, int timeSize, double* mul, int mulSize, int stage, int remaining, double totalTime) {
    if (remaining == 0) {
        return totalTime;
    }
    ...
    if (remaining > k) {
        ...
        for (int i = 0; i < groupSize; i++) {
            maxTime = fmax(maxTime, time[i]);
        }
        double crossingTime = maxTime * mul[stage];
        double nextStage = (stage + floor(crossingTime)) % m;
        double nextTotalTime = minTimeHelper(n, k, m, time, timeSize, mul, mulSize, nextStage, remaining - groupSize, totalTime + crossingTime);

        if (nextTotalTime == -1) {
            return -1;
        }

        int returnIndex = groupSize - 1;
        double returnTime = time[returnIndex] * mul[nextStage];
        double returnStage = (nextStage + floor(returnTime)) % m;
        double returnTotalTime = minTimeHelper(n, k, m, time, timeSize, mul, mulSize, returnStage, remaining, nextTotalTime + returnTime);

        return returnTotalTime;
    } else {
        int groupSize = remaining;
        double maxTime = 0;
        for (int i = 0; i < groupSize; i++) {
            maxTime = fmax(maxTime, time[i]);
        }
...
double minTime(int n, int k, int m, int* time, int timeSize, double* mul, int mulSize) {
    return minTimeHelper(n, k, m, time, timeSize, mul, mulSize, 0, n, 0);
}