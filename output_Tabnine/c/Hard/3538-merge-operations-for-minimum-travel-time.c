...
int minTravelTime(int l, int n, int k, int* position, int positionSize, int* time, int timeSize) {
    ...
    for (int i = 0; i < k; i++) {
        ...
        for (int j = 1; j < n - 1; j++) {
            if (position[j] > currentPosition && position[j + 1] <= l && time[j] + time[j + 1] < minTime) {
                minTime = time[j] + time[j + 1];
                mergeIndex = j;
            }
        }

        time[mergeIndex + 1] += time[mergeIndex];
        totalTime += time[mergeIndex + 1];
        currentPosition = position[mergeIndex + 1];
    }
