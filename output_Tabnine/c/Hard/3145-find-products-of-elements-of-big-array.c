...
int* findProductsOfElements(long long** queries, int queriesSize, int* queriesColSize, int* returnSize) {
    ...
    // Create a vector to store the powerful array
    int* powerfulArray = (int*)malloc(1016 * sizeof(int));
    powerfulArray[0] = 1;
    for (int i = 1; i < 1016; i++) {
        powerfulArray[i] = powerfulArray[i - 1] * 2;
    }
    
    // Create a vector to store the powerful array concatenated
    int* bigNums = (int*)malloc((queries[queriesSize - 1][1] + 1) * sizeof(int));
    bigNums[0] = powerfulArray[0];
    for (int i = 1; i <= queries[queriesSize - 1][1]; i++) {
        bigNums[i] = bigNums[i - 1] * powerfulArray[i];
    }
    ...
    for (int i = 0; i < queriesSize; i++) {
        answer[i] = (bigNums[queries[i][0]] * bigNums[queries[i][1] + 1]) % queries[i][2];
    }
