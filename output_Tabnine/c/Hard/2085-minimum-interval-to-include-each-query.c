...
int* minInterval(int** intervals, int intervalsSize, int* intervalsColSize, int* queries, int queriesSize, int* returnSize) {
    ...
    *returnSize = queriesSize;
    ...
    // Sort intervals and queries based on their left values
    qsort(intervals, intervalsSize, sizeof(int*), compareIntervals);
    qsort(queries, queriesSize, sizeof(int*), compareQueries);
    
    int i = 0, j = 0;
    while (i < intervalsSize && j < queriesSize) {
        if (intervals[i][0] <= queries[j]) {
            int minInterval = INT_MAX;
            for (int k = i; k < intervalsSize && intervals[k][0] <= queries[j]; k++) {
                minInterval = fmin(minInterval, intervals[k][1] - intervals[k][0] + 1);
            }
            result[j++] = minInterval;
            i++;
        } else {
            j++;
        }
