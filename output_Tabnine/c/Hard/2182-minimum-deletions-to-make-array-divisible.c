...
int minOperations(int* nums, int numsSize, int* numsDivide, int numsDivideSize) {
    ...
    qsort(numsDivide, numsDivideSize, sizeof(int), compare);
    
    // Initialize the minimum number of deletions
    int minDeletions = 0;
    
    // Iterate through the sorted numsDivide array
    for (int i = 0; i < numsDivideSize; i++) {
        // Find the first nums element that is greater than or equal to numsDivide[i]
        int index = binarySearch(nums, numsSize, numsDivide[i]);
        
        // If no such nums element exists, return -1
        if (index == -1) {
            return -1;
        }
        
        // Delete the nums element at index
