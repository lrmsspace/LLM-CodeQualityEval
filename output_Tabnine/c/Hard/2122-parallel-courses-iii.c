...
int minimumTime(int n, int** relations, int relationsSize, int* relationsColSize, int* time, int timeSize) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        memset(adjList[i], 0, n * sizeof(int));
    }
    ...
    // Compute the time taken to complete each course
    for (int i = 0; i < n; i++) {
        if (queueSize == 0) {
            return -1; // Cycle detected
        }
        
        int node = queue[--queueSize];
