...
var baseUnitConversions = function(conversions) {
    // Initialize an array to store the number of units equivalent to a single unit of type i
    const baseUnitConversion = Array(conversions.length + 1).fill(0);
    
    // Iterate through each conversion
    for (const [sourceUniti, targetUniti, conversionFactori] of conversions) {
        // Update the number of units equivalent to a single unit of type targetUniti
        baseUnitConversion[targetUniti] = (baseUnitConversion[targetUniti] + baseUnitConversion[sourceUniti] * conversionFactori) % 1000000007;
    }
    
    // Return the array of base unit conversions
    return baseUnitConversion.slice(1);
