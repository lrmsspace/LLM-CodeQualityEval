...
var mostFrequentPrime = function(mat) {
    ...
    const numCells = mat.length * mat[0].length;
    const count = Array(numCells).fill(0);  
        ...
        for (let j = 0; j < mat[0].length; j++) {
            ...
            for (let dir of directions) {
                ...
                if (newX >= 0 && newX < mat.length && newY >= 0 && newY < mat[0].length) {
                    const newNum = mat[newX][newY];
                    if (primes.includes(newNum)) {
                        count[i * mat[0].length + j]++;
                    }
                }
            }
        }
    ...
    for (let i = 0; i < numCells; i++) {
        if (count[i] > maxCount) {
            maxCount = count[i];
            maxPrime = mat[Math.floor(i / mat[0].length)][i % mat[0].length];
        } else if (count[i] === maxCount && mat[Math.floor(i / mat[0].length)][i % mat[0].length] > maxPrime) {
            maxPrime = mat[Math.floor(i / mat[0].length)][i % mat[0].length];
        }
    }
    return maxPrime > 10? maxPrime : -1;
