...
var makeSubKSumEqual = function(arr, k) {
    ...
    // calculate the target sum for each subarray of length k
    const targetSum = totalSum / k;
    
    // calculate the minimum number of operations required to make the sum of each subarray equal to the target sum
    let operations = 0;
    for (let i = 0; i < arr.length; i++) {
        const currentSum = arr.slice(i, i + k).reduce((acc, curr) => acc + curr, 0);
        operations += Math.abs(currentSum - targetSum);
    }
