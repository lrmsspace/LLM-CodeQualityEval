...
var minIncrease = function(n, edges, cost) {
    ...
    const root = parent.find(parentVal => parentVal === -1);
    const rootCost = cost.filter((_, index) => index === root);

    const minCost = Math.min(...rootCost);
    const increaseCost = rootCost.reduce((acc, curr) => acc + (curr - minCost), 0);
