...
var kthSmallest = function(par, vals, queries) {
  ...
  const xorSums = Array.from({ length: n }, () => new Set());

  for (let i = 1; i < n; i++) {
    tree[par[i]].push(i);
  }

  const dfs = (node, parent, xorSum) => {
    xorSums[node].add(xorSum);

    for (const child of tree[node]) {
      if (child !== parent) {
        dfs(child, node, xorSum ^ vals[child]);
      }
    }
  };
  ...
  for (const [u, k] of queries) {
    const distinctXorSums = [...xorSums[u]].sort((a, b) => a - b);
    ans.push(distinctXorSums[k - 1] !== undefined ? distinctXorSums[k - 1] : -1);
  }
