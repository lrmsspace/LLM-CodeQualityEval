...
var canMouseWin = function(grid, catJump, mouseJump) {
    ...
    const queue = [[...mouse, 0]];
    while (queue.length) {
        const [mx, my, turns] = queue.shift();
        if (visited.has([mx, my])) continue;
        visited.add([mx, my]);
        
        if (mx === food[0] && my === food[1]) return true;
        
        for (let dx = -catJump; dx <= catJump; dx++) {
            for (let dy = -catJump; dy <= catJump; dy++) {
                if (dx === 0 && dy === 0) continue;
                if (Math.abs(dx) + Math.abs(dy) > catJump) continue;
