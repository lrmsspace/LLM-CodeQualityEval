...
var longestSpecialPath = function(edges, nums) {
    ...
    while (queue.length > 0) {
        ...
        for (const [v, length] of graph[u]) {
            const newPath = [...path,...longestSpecialPath(edges, nums, v)];
            const newLength = path.length + newPath.length - 1;
            
            if (newLength > longestPath) {
                longestPath = newLength;
                minNodes = Math.min(minNodes, newPath.length);
            } else if (newLength === longestPath) {
                minNodes = Math.min(minNodes, newPath.length);
            }
