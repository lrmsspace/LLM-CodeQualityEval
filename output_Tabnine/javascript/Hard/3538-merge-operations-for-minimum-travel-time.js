...
var minTravelTime = function(l, n, k, position, time) {
    ...
    // Iterate through the position array and merge signs
    for (let i = 1; i < n; i++) {
        // Calculate the distance between the current and previous signs
        let distance = position[i] - position[start];
        
        // Calculate the travel time for the current segment
        let travelTime = distance * time[start];
        
        // Add the travel time to the total travel time
        totalTime += travelTime;
        
        // Check if we have reached the maximum number of merges
        if (k === 0) {
            break;
        }
        
        // Merge the signs at indices i and i - 1
        time[i - 1] += time[i];
        
        // Remove the sign at index i
        n--;
        
        // Update the start index of the current segment
        start = i - 1;
