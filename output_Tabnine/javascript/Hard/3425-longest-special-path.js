...
var longestSpecialPath = function(edges, nums) {
    ...
    function dfs(node, length, count) {
        if (visited.has(node) || values.has(nums[node])) return;

        visited.add(node);
        values.add(nums[node]);

        maxLength = Math.max(maxLength, length);
        minNodes = Math.min(minNodes, count);

        for (const [neighbor, edgeLength] of graph[node]) {
            dfs(neighbor, length + edgeLength, count + 1);
        }

        visited.delete(node);
        values.delete(nums[node]);
    }
