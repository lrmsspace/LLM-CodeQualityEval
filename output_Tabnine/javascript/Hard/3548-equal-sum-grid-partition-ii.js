...
var canPartitionGrid = function(grid) {
    ...
    const totalSum = grid.reduce((acc, row) => acc + row.reduce((acc2, num) => acc2 + num, 0), 0);
    ...
    for (let i = 0; i < m; i++) {
        for (let j = 0; j < n; j++) {
            // Iterate through the dp array and update the results
            for (let k = targetSum; k >= grid[i][j]; k--) {
