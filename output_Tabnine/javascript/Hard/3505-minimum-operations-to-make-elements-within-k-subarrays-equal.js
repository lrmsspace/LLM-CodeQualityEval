...
var minOperations = function(nums, x, k) {
    ...
    for (let right = 0; right < nums.length; right++) {
        ...
        while (right - left + 1 >= k * x) {
            sum -= nums[left];
            left++;
        }
        
        if (right - left + 1 === k * x && sum === 0) {
            count++;
        }
        
        minOperations = Math.min(minOperations, count + (k * x - (right - left + 1)) / x);
    }
    
    return minOperations === Infinity? -1 : Math.floor(minOperations);
