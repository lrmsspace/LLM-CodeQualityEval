...
class Solution(object):
    def minIncrease(self, n, edges, cost):
        ...
        def dfs(node, parent, total_cost):
            min_cost = float('inf')
            for neighbor in graph[node]:
                if neighbor!= parent:
                    new_total_cost = total_cost + cost[neighbor]
                    min_cost = min(min_cost, dfs(neighbor, node, new_total_cost))
            return min_cost
        
        min_increase = 0
        for node in range(n):
            min_cost = dfs(node, -1, cost[node])
            min_increase += min_cost - cost[node]
