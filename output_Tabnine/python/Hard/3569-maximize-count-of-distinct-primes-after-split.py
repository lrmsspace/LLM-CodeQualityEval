...
class Solution(object):
    def maximumCount(self, nums, queries):
        ...
        for idx, val in queries:
            nums[idx] = val
            prefix_primes = [num for num in primes if num <= nums[idx]]
            suffix_primes = [num for num in primes if num <= nums[len(nums) - 1 - idx]]
            result.append(len(prefix_primes) + len(suffix_primes))
