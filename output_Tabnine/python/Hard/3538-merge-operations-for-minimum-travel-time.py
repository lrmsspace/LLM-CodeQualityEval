...
class Solution(object):
    def minTravelTime(self, l, n, k, position, time):
        ...
        :rtype: int
        """
        # Sort the positions and times in ascending order
        position.sort()
        time.sort()
        
        # Initialize the minimum travel time
        min_time = 0
        
        # Perform k merge operations
        for _ in range(k):
            # Find the minimum time to merge two adjacent signs
            min_time_merge = min(time[0] + time[1], position[1] - position[0])
            
            # Update the sign at index 1 and remove the sign at index 0
            time[1] += time[0]
            position = position[1:] + position[:1]
            
            # Update the minimum travel time
            min_time += min_time_merge
