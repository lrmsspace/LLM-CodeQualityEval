...
class Solution(object):
    def minOperations(self, word1, word2):
        ...
        :rtype: int
        """
        diff = [c1!= c2 for c1, c2 in zip(word1, word2)]
        replace_count, swap_count, reverse_count = 0, 0, 0
        
        # Count the number of replacements, swaps, and reversals
        ...
        # Count the number of reversals that can be performed in-place
        for i in range(len(diff) // 2):
            if diff[i]!= diff[len(diff) - i - 1]:
                reverse_count -= 1
        
        return replace_count + swap_count + reverse_count
