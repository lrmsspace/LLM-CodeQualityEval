...
public:
    vector<int> filterRestaurants(vector<vector<int>>& restaurants, int veganFriendly, int maxPrice, int maxDistance) {
        vector<pair<int, pair<int, int>>> filteredRestaurants;
        
        for (const auto& restaurant : restaurants) {
            if (restaurant[2] == veganFriendly && restaurant[3] <= maxPrice && restaurant[4] <= maxDistance) {
                filteredRestaurants.push_back({restaurant[1], {restaurant[0], restaurant[1]}});
            }
        }
        ...
        for (const auto& restaurant : filteredRestaurants) {
            result.push_back(restaurant.second.first);
        }
