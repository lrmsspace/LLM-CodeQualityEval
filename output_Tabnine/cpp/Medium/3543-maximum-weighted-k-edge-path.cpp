...
public:
    int maxWeight(int n, vector<vector<int>>& edges, int k, int t) {
        ...
        // dp[i][j] represents the maximum sum of weights for the first i nodes and j edges
        // Initialize dp[0][0] = 0
        dp[0][0] = 0;   
        ...
        for (int i = 1; i < n; ++i) {
            ...
            for (int j = 0; j <= k; ++j) {
                // Iterate through each possible parent node
                for (int p = 0; p < i; ++p) {
                    // If the parent node has j - 1 edges and the current node has at least one edge
                    if (j - 1 >= 0 && adj[p].size() >= 1) {
                        // Update dp[i][j]
                        dp[i][j] = max(dp[i][j], dp[p][j - 1] + adj[p].back().second);
                    }
                }
            }
        }
        // Return the maximum sum of weights for a path with exactly k edges and total sum less than t
        int maxSum = -1;
        for (int j = 0; j <= k; ++j) {
