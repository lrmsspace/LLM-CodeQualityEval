...
public:
    int minimumCost(vector<int>& start, vector<int>& target, vector<vector<int>>& specialRoads) {
        int n = target[0] - start[0] + 1;
        vector<vector<int>> dp(n, vector<int>(n, INT_MAX));
        for (auto& road : specialRoads) {
            int x1 = road[0] - start[0];
            int y1 = road[1] - start[1];
            int x2 = road[2] - start[0];
            int y2 = road[3] - start[1];
            dp[x1][y1] = min(dp[x1][y1], road[4]);
            dp[x2][y2] = min(dp[x2][y2], road[4]);
        }
        for (int k = 0; k < n; ++k) {
            for (int i = 0; i < n; ++i) {
                for (int j = 0; j < n; ++j) {
        ...
        return dp[start[0] - start[0]][start[1] - start[1]];
