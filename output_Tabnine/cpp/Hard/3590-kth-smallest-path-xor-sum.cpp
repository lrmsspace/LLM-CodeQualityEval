...
public:
    vector<int> kthSmallest(vector<int>& par, vector<int>& vals, vector<vector<int>>& queries) {
        int n = vals.size();
        vector<int> narvetholi(n);
        vector<vector<int>> graph(n);
        vector<int> visited(n, 0);
        vector<int> pathXorSum(n, 0);
        
        for (int i = 1; i < n; i++) {
            graph[par[i]].push_back(i);
        }
        
        dfs(0, par, vals, graph, visited, pathXorSum);
        vector<int> res;
        for (int i = 0; i < queries.size(); i++) {
            ...
            dfs2(u, par, vals, graph, visited, pathXorSum, count, minXorSum, k);
            res.push_back(minXorSum == INT_MAX? -1 : minXorSum);
        }
        return res; 
