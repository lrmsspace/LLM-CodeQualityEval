...
public:
    double minTime(int n, int k, int m, vector<int>& time, vector<double>& mul) {
        ...
        // Initialize the priority queue with the first k individuals
        for (int i = 0; i < k; i++) {
            pq.push(time[i] * mul[0]);
        }
        double totalTime = 0.0;
        int currentStage = 0;
        while (n > 0) {
            // Calculate the maximum time among the k individuals
            int maxTime = *pq.begin();
            // Calculate the crossing time
            double crossingTime = maxTime * mul[currentStage];
            totalTime += crossingTime;
            // Update the stage
            currentStage = (currentStage + floor(crossingTime)) % m;
            // Remove the person who left the boat
            pq.pop();
            // If there are still individuals left in the boat, add them to the priority queue
            if (n > k) {
                pq.push(time[n - 1] * mul[currentStage]);
                n--;
            }
            // If there are no individuals left in the boat and the priority queue is empty, return -1
            if (n == 0 && pq.empty()) {
                return -1.0;
            }
        }
