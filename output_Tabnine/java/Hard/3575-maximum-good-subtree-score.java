    ...
    public int goodSubtreeSum(int[] vals, int[] par) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int child = 0; child < n; child++) {
                if (par[child] == i &&!visited[child]) {
                    dfs(child, sum, visited);
                    maxChildScore = Math.max(maxChildScore, sum[child]);
                }
            }
            maxScore[i] += maxChildScore;
    
            visited[i] = true;
        }
        int sumMaxScore = 0;
        for (int i = 0; i < n; i++) {
            sumMaxScore = (sumMaxScore + maxScore[i]) % (int)(1e9 + 7);
        }
        return sumMaxScore;
