...
class Solution {
    public List<String> watchedVideosByFriends(List<List<String>> watchedVideos, int[][] friends, int id, int level) {
        ...
        List<List<String>> friendVideos = new ArrayList<>(friends.length);
        for (int i = 0; i < friends.length; i++) {
            friendVideos.add(new ArrayList<>());
        }
        ...
        for (int friendId : friends[id]) {
            for (String video : watchedVideos.get(friendId)) {
                friendVideos.get(friendId).add(video);
            }
        }
        
        for (List<String> videos : friendVideos) {
            pq.addAll(videos);
        }
        
        List<String> result = new ArrayList<>();
        while (level > 0 && !pq.isEmpty()) {
            result.add(pq.poll());
            level--;
        }
