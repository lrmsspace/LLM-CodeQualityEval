...
def crawler():
    ...
    df2 = pd.DataFrame(columns=['Link', 'Difficulty', 'Description', 'C', 'C++', 'Java', 'JavaScript', 'Python'])
    ...
    lang_lst = ['C', 'C++', 'Java', 'JavaScript', 'Python']
    ...
    for diff, l in zip(difficulty, link):
        ...
        for i in codeSnippets:
            if i['lang'] in lang_lst:
                if i['lang'] != 'Python3':
                    # Special handling for C++ function templates (e.g., `template <typename T>`)
                    #if i['lang'] == 'C++' and 'class' in i['code'] and 'public' in i['code']:
                        # Here you can refine how you store or process the C++ template code if needed
                       # d['C++'] = i['code']
                    #else:
                        d[i['lang']] = i['code']
