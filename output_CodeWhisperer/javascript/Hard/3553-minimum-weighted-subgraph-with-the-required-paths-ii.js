...
var minimumWeight = function(edges, queries) {
    ...
    const distFrom = Array.from({ length: n }, (_, i) => dist(i));
    
    const answer = [];
    for (const [src1, src2, dest] of queries) {
        let minDist = Infinity;
        const dist1 = distFrom[src1];
        const dist2 = distFrom[src2];
        const distDest = distFrom[dest];
        for (let i = 0; i < n; i++) {
            minDist = Math.min(minDist, distDest[i] + dist1[i] + dist2[i]);
        }
        answer.push(minDist === Infinity ? -1 : minDist);
    }
