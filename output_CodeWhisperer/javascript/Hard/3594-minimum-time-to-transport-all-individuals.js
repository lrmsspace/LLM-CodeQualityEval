...
var minTime = function(n, k, m, time, mul) {
    ...
    for (let i = 1; i <= n; i++) {
        for (let j = i; j >= 1; j--) {
            let maxTime = 0;
            for (let x = j; x <= i; x++) {
                maxTime = Math.max(maxTime, time[x - 1]);
            }
            const crossingTime = maxTime * mul[(i - 1) % m] * Math.pow(mul[(i - 1) % m], Math.floor(maxTime * mul[(i - 1) % m]));
            const returnTime = time[(i - 1) % n] * mul[(i - 1) % m] * Math.pow(mul[(i - 1) % m], Math.floor(time[(i - 1) % n] * mul[(i - 1) % m]));
            const stageAdvance = Math.floor(crossingTime);
            const nextStage = (i - 1 + stageAdvance) % m;
            dp[i] = Math.min(dp[i], dp[j - 1] + crossingTime + returnTime * Math.pow(mul[nextStage], Math.floor(returnTime)));
        }
    }
