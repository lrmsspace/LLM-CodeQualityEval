...
var minCost = function(maxTime, edges, passingFees) 
{
    ...
    const dp = new Array(n).fill().map(() => new Array(maxTime + 1).fill(Number.MAX_SAFE_INTEGER));
    ...
    while (q.length > 0) {
        ...
        for (const [v, t] of graph[u]) {
            if (time + t <= maxTime) {
                if (dp[v][time + t] > dp[u][time] + passingFees[v]) {
                    dp[v][time + t] = dp[u][time] + passingFees[v];
                    q.push([v, time + t]);
                }
            }
        }
