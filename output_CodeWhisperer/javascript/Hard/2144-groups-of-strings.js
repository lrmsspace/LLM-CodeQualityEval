...
var groupStrings = function(words) {
    ...
    const dfs = (word, group) => {
      ...
      for (let i = 0; i < word.length; i++) {
        for (let j = 0; j < 26; j++) {
          ...
          if (i === 0) {
            newWord = newChar + word.slice(1);
          } else if (i === word.length - 1) {
            newWord = word.slice(0, -1) + newChar;
          } else {
            newWord = word.slice(0, i) + newChar + word.slice(i + 1);
          }
    ...
    return [groups.length, maxGroupSize];
};