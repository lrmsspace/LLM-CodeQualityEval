...
var goodSubtreeSum = function(vals, par) {
    ...
    const dfs = (node) => {
        let score = vals[node];
        let uniqueDigits = new Set([vals[node]]);
        
        for (const child of children[node]) {
            const [childScore, childUniqueDigits] = dfs(child);
            score += childScore;
            uniqueDigits = new Set([...uniqueDigits, ...childUniqueDigits]);
        }
        
        if (uniqueDigits.size === String(score).length) {
            maxScore[node] = score;
        } else {
            maxScore[node] = 0;
        }
        
        return [score, uniqueDigits];
    };
    
    dfs(0);
    
    return maxScore.reduce((acc, val) => acc + val, 0) % (10**9 + 7);    
};