...
class Solution {
    public int minimumCost(int[] start, int[] target, int[][] specialRoads) {
        ...
        for (int i = 0; i <= n; i++) {
            dist[i] = i == 0 ? start : specialRoads[i - 1].clone();
            if (i > 0) {
                dist[i][2] = Math.abs(dist[i][0] - dist[i][2]) + Math.abs(dist[i][1] - dist[i][3]);
            }
        }
        int[] dp = new int[n + 1];
        ...
        for (int i = 1; i <= n; i++) {
            for (int j = 0; j <= n; j++) {
                dp[j] = Math.min(dp[j], dp[j == 0 ? i : i == j ? 0 : j - 1] + dist[j][4]);
                dp[i] = Math.min(dp[i], dp[j] + dist[j][2] + dist[i][4]);
            }
        }
        return Math.abs(target[0] - dist[n][0]) + Math.abs(target[1] - dist[n][1]) + dp[n];     
    }
}