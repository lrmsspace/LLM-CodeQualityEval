...
class Solution {
    public int reachableNodes(int[][] edges, int maxMoves, int n) {
        ...
        while (!pq.isEmpty()) {
            ...
            if (!seen.containsKey(node)) {
                ...
                for (Map.Entry<Integer, Integer> neighbor : graph.getOrDefault(node, new HashMap<>()).entrySet()) {
                    ...
                    int edgeCnt = neighbor.getValue();
                    int nextMoves = moves - edgeCnt - 1;
                    
                    if (nextMoves >= 0 && !seen.containsKey(nextNode)) {
                        pq.offer(new int[]{nextNode, nextMoves});
                    }
                }
            }
        }
        ...
        for (int[] edge : edges) {
            int u = edge[0], v = edge[1], cnt = edge[2];
            res += Math.min(cnt, Math.max(0, seen.getOrDefault(u, 0) + seen.getOrDefault(v, 0) - maxMoves));
        }
