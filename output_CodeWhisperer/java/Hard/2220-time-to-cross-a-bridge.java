...
class Solution {
    public int findCrossingTime(int n, int k, int[][] time) {
        ...
        PriorityQueue<int[]> left = new PriorityQueue<int[]>((a, b) -> a[0] - b[0]);
        PriorityQueue<int[]> right = new PriorityQueue<int[]>((a, b) -> a[0] - b[0]);
        for (int i = 0; i < k; i++) {
            left.offer(new int[] { -time[i][0] - time[i][2], -i });
        }
        int cur = 0;
        while (n > 0) {
            while (!left.isEmpty() && left.peek()[1] < 0) {
                int i = -left.peek()[1];
                if (time[i][0] > cur) {
                    break;
                }
                left.poll();
                cur += time[i][2];
                right.offer(new int[] { -time[i][0] - time[i][2], -i });
            }
            if (right.isEmpty()) {
                int i = -left.peek()[1];
                cur = Math.max(cur, time[i][0]) + time[i][2];
            } else {
                int i = -right.peek()[1];
                cur += time[i][2];
                n--;
            ...
            while (!right.isEmpty() && right.peek()[1] < 0) {
                int i = -right.peek()[1];
                if (time[i][0] > cur) {
                    break;
                }
