...
class Solution {
    public int[] minReverseOperations(int n, int p, int[] banned, int k) {
        ...
        for (int i = 0; i < n; i++) {
            if (i != p && !Arrays.stream(banned).anyMatch(x -> x == i)) {
                set.add(i);
            }
        }
        ...
        while (!queue.isEmpty()) {
            int curr = queue.poll();
            int left = Math.max(0, curr - (k - 1) / 2);
            int right = Math.min(n - 1, left + k - 1);
            TreeSet<Integer> subSet = new TreeSet<>(set.subSet(left, true, right, true));
            while (subSet.size() > 0) {
                int next = subSet.pollFirst();
                res[next] = res[curr] + 1;
                set.remove(next);
                queue.offer(next);
            }
