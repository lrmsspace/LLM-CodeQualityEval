...
public:
    string abbreviateProduct(int left, int right) {
        long long suffix = 1, max_suffix = (long long)1e10;
        double prefix = 1.0;
        int zeros = 0;
        for (int i = left; i <= right; i++) {
            prefix *= i;
            while (prefix >= 1000000) {
                prefix /= 10;
                suffix *= 10;
            }
            while (suffix > 1 && prefix >= 1) {
                suffix /= 10;
                prefix /= 10;
            }
            if (suffix == 1) {
                zeros += countTrailingZeros(i);
            }
        }
        if (suffix < max_suffix) {
            return to_string((int)prefix) + "e" + to_string(zeros);
        } else {
            string s = to_string((long long)prefix) + "e" + to_string(zeros);
