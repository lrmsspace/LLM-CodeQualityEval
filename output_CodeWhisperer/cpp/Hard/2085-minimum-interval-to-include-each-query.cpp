...
class Solution {
    public:
        vector<int> minInterval(vector<vector<int>>& intervals, vector<int>& queries) {
            ...
            // Create a vector of query indices to maintain original order
            ...
            // pair: {interval size, end time}
            priority_queue<pair<int, int>, vector<pair<int, int>>, greater<pair<int, int>>> pq;
            
            vector<int> result(q, -1);
            int intervalIdx = 0;
            
            for (auto& [queryVal, queryIdx] : queriesWithIndex) {
