...
class Solution {
    public:
        string validIPAddress(string queryIP) {
            // Check if it contains '.' for IPv4 or ':' for IPv6
    ...
    private:
        bool isValidIPv4(string ip) {
            ...
            for (const string& seg : segments) {
                ...
                // Check if segment is in range [0, 255]
                try {
                    int value = stoi(seg);
            ...
            // Additional check to ensure there's no trailing dot
            return ip.back() != '.';
        }
        ...
        bool isValidIPv6(string ip) {
            ...
            // IPv6 should have exactly 8 segments
            if (segments.size() != 8) {
                return false;
            }
            ...
            for (const string& seg : segments) {
                ...
                // Segment length should be between 1 and 4
                if (seg.length() > 4) {
                    return false;
                }
                
                // Check if segment contains only valid hexadecimal characters
                for (char c : seg) {
                    if (!isdigit(c) && 
                        !('a' <= c && c <= 'f') && 
                        !('A' <= c && c <= 'F')) {
            ...
            // Additional check to ensure there's no trailing colon
            return ip.back() != ':';
        }
    };
