...
class Solution(object):
    def findMedian(self, n, edges, queries):
        ...
        def dfs(node, parent, total, count):
            ans = -1
            for child, weight in graph[node]:
                if child != parent:
                    child_total, child_count = dfs(child, node, total + weight, count + 1)
                    if child_count % 2 == 0 and child_total >= total // 2:
                        ans = node
                        break
            return total, count + 1 if ans == -1 else child_count

        return [dfs(start, -1, 0, 0)[0] for start, end in queries]
