...
class Solution(object):
    def isSelfCrossing(self, distance):
        ...
        for i in range(3, n):
            ...
            if i >= 5 and distance[i] >= distance[i-2] - distance[i-4] and distance[i-1] <= distance[i-3] and distance[i-2] > distance[i-4] and distance[i] >= distance[i-2] - distance[i-4] + distance[i-6]:
                return True
        return False
