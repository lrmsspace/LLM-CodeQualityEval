...
class Solution(object):
    def minimumCost(self, start, target, specialRoads):
        ...
        :rtype: int
        """
        specialRoads = [tuple(road) for road in specialRoads]
        specialRoads = set(specialRoads)
        visited = set()
        ...
        while queue:
            ...
            for road in specialRoads:
                if (x, y) == (road[0], road[1]):
                    heapq.heappush(queue, (cost, road[2], road[3]))
                else:
                    heapq.heappush(queue, (cost + abs(x - road[0]) + abs(y - road[1]), road[2], road[3]))
            heapq.heappush(queue, (cost + abs(x - target[0]) + abs(y - target[1]), target[0], target[1]))
        return -1
