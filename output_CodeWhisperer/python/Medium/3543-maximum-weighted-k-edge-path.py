...
class Solution(object):
    def maxWeight(self, n, edges, k, t):
        ...
        :rtype: int
        """
        # Initialize a 2D array to store the maximum weight for each node and edge count
        dp = [[-1] * (k + 1) for _ in range(n)]

        # Sort the edges by weight in descending order
        edges.sort(key=lambda x: x[2], reverse=True)

        # Initialize the maximum weight for each node and edge count
        for i in range(n):
            dp[i][0] = 0

        # Iterate through the sorted edges
        for u, v, w in edges:
            # Update the maximum weight for each node and edge count
            for i in range(k, 0, -1):
                if dp[u][i - 1] != -1:
                    dp[v][i] = max(dp[v][i], dp[u][i - 1] + w)

        # Find the maximum weight for k edges
        max_weight = max(max(row) for row in dp)

        # Return the maximum weight if it is less than t, otherwise return -1
        return max_weight if max_weight < t else -1        