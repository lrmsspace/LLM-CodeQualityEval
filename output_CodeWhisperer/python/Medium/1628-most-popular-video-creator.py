...
class Solution(object):
    def mostPopularCreator(self, creators, ids, views):
        ...
        for i in range(len(creators)):
            ...
            video_id = ids[i]
            view_count = views[i]
            creator_views[creator] = creator_views.get(creator, 0) + view_count
            if creator not in creator_video:
                creator_video[creator] = []
            creator_video[creator].append((video_id, view_count))
        max_views = max(creator_views.values())
        max_creators = [creator for creator, view_count in creator_views.items() if view_count == max_views]
        result = []
        for creator in max_creators:
            max_video = max(creator_video[creator], key=lambda x: (x[1], -ord(x[0][0])))
            result.append([creator, max_video[0]])
        return result
