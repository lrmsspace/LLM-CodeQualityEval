...
int* beautifulIndices(char* s, char* a, char* b, int k, int* returnSize) {
    ...
    *returnSize = 0;
    int aLen = strlen(a);
    int bLen = strlen(b);
    int* aIndices = (int*)malloc(sizeof(int) * 100000);
    int aCount = 0;
    int* bIndices = (int*)malloc(sizeof(int) * 100000);
    int bCount = 0;
    for (int i = 0; i <= strlen(s) - aLen; i++) {
        if (strncmp(s + i, a, aLen) == 0) {
            aIndices[aCount++] = i;
        }
    }
    for (int i = 0; i <= strlen(s) - bLen; i++) {
