...
int minIncrease(int n, int** edges, int edgesSize, int* edgesColSize, int* cost, int costSize) {
    int maxCost = 0;
    int* maxCosts = (int*)malloc(n * sizeof(int));
    memset(maxCosts, 0, n * sizeof(int));

    for (int i = 0; i < edgesSize; i++) {
        int u = edges[i][0];
        int v = edges[i][1];
        maxCosts[v] = std::max(maxCosts[v], maxCosts[u] + cost[v]);
        maxCost = std::max(maxCost, maxCosts[v]);
    }

    int totalCost = 0;
    for (int i = 0; i < n; i++) {
        totalCost += maxCost - maxCosts[i];
    }
