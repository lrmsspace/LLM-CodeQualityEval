...
int* filterRestaurants(int** restaurants, int restaurantsSize, int* restaurantsColSize, int veganFriendly, int maxPrice, int maxDistance, int* returnSize) {
    int* result = (int*)malloc(restaurantsSize * sizeof(int));
    int count = 0;
    ...
    *returnSize = count;
    for (int i = 0; i < count - 1; i++) {
        for (int j = i + 1; j < count; j++) {
            if (restaurants[result[i] - 1][1] < restaurants[result[j] - 1][1] || (restaurants[result[i] - 1][1] == restaurants[result[j] - 1][1] && result[i] < result[j])) {
                int temp = result[i];
                result[i] = result[j];
                result[j] = temp;
            }
