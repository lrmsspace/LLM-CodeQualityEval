...
int* maximumBeauty(int** items, int itemsSize, int* itemsColSize, int* queries, int queriesSize, int* returnSize) {
    ...
    *returnSize = queriesSize;
    int* maxBeauty = (int*)malloc(sizeof(int) * 100001);
    memset(maxBeauty, 0, sizeof(int) * 100001);
    for (int i = 0; i < itemsSize; i++) {
        maxBeauty[items[i][0]] = max(maxBeauty[items[i][0]], items[i][1]);
    }
    int max = 0;
    for (int i = 1; i <= 100000; i++) {
        max = max(maxBeauty[i], max);
        maxBeauty[i] = max;
    }
