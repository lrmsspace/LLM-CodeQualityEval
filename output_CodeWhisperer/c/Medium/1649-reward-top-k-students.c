...
int* topStudents(char** positive_feedback, int positive_feedbackSize, char** negative_feedback, int negative_feedbackSize, char** report, int reportSize, int* student_id, int student_idSize, int k, int* returnSize) {
    ...
    int* points = (int*)malloc(sizeof(int) * student_idSize);
    for (int i = 0; i < student_idSize; i++) {
        points[i] = 0;
        char* temp = (char*)malloc(sizeof(char) * (strlen(report[i]) + 1));
        strcpy(temp, report[i]);
    ...
    int* index = (int*)malloc(sizeof(int) * student_idSize);
    for (int i = 0; i < student_idSize; i++) {
        index[i] = i;
    }
    for (int i = 0; i < student_idSize - 1; i++) {
        for (int j = i + 1; j < student_idSize; j++) {
            if (points[i] < points[j] || (points[i] == points[j] && student_id[i] > student_id[j])) {
                int temp = points[i];
                points[i] = points[j];
                points[j] = temp;
                temp = student_id[i];
