...
int minOperationsMaxProfit(int* customers, int customersSize, int boardingCost, int runningCost) {
    int maxProfit = 0, maxProfitRotation = -1, currentProfit = 0, waitingCustomers = 0, rotation = 0;
    for (int i = 0; i < customersSize || waitingCustomers > 0; i++) {
        if (i < customersSize) {
            waitingCustomers += customers[i];
        }
        int boardingCustomers = min(waitingCustomers, 4);
    ...
    return maxProfit > 0 ? maxProfitRotation : -1;
}