...
char* findReplaceString(char* s, int* indices, int indicesSize, char** sources, int sourcesSize, char** targets, int targetsSize) {
    ...
    for (i = 0; i < indicesSize; i++) {
        ...
        if (strncmp(&res[index], source, sourceLen) == 0) {
            if (len + targetLen - sourceLen < len) {
                for (j = len - 1; j >= index + sourceLen; j--) {
                    res[j + targetLen - sourceLen] = res[j];
                }
            }
            for (j = 0; j < targetLen; j++) {
                res[index + j] = target[j];
            }
