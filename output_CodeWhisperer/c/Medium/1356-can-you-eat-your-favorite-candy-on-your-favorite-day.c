...
bool* canEat(int* candiesCount, int candiesCountSize, int** queries, int queriesSize, int* queriesColSize, int* returnSize) {
    ...
    *returnSize = queriesSize;
    long long* prefixSum = (long long*)malloc(sizeof(long long) * candiesCountSize);
    ...
    for (int i = 0; i < queriesSize; i++) {
        ...
        long long min = favoriteDay + 1;
        long long max = (long long)dailyCap * (favoriteDay + 1);
        if (favoriteType > 0) {
            min = prefixSum[favoriteType - 1] + favoriteDay + 1;
        }
        if (favoriteType < candiesCountSize) {
            max = prefixSum[favoriteType];
        }
        answer[i] = (min <= max);
    }
    return answer;
