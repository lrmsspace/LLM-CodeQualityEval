...
public:
    int minimumCoins(vector<int>& prices) {
        ...
        while (i < n) {
            totalCoins += prices[i]; // Purchase the (i+1)th fruit
            int freeFruits = i; // Number of next fruits you can take for free
            i += freeFruits + 1; // Move to the next fruit to purchase
        }
