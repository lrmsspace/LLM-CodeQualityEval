    ...
    int minIncrease(int n, vector<vector<int>>& edges, vector<int>& cost) {
        ...
        function<int(int, int)> dfs = [&](int node, int parent) {
            ...
            for (int neighbor : tree[node]) {
                if (neighbor != parent) {
                    maxPath = max(maxPath, dfs(neighbor, node));
                }
            }
            return maxPath + cost[node];
        };

        int maxPathSum = dfs(0, -1);
        int result = 0;

        function<void(int, int, int)> computeIncrements = [&](int node, int parent, int currentSum) {
            int maxChildSum = 0;
            for (int neighbor : tree[node]) {
                if (neighbor != parent) {
                    maxChildSum = max(maxChildSum, dfs(neighbor, node));
                }
            }
            int requiredIncrement = maxPathSum - (currentSum + cost[node]);
            if (requiredIncrement > 0) {
                result++;
                currentSum += requiredIncrement;
            }
            for (int neighbor : tree[node]) {
                if (neighbor != parent) {
                    computeIncrements(neighbor, node, currentSum);
                }
            }
        };

        computeIncrements(0, -1, 0);
        return result;
