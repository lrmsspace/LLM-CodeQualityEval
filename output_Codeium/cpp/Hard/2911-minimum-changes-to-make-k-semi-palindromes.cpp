...
public:
    int minimumChanges(string s, int k) {
        ...
        // Try all possible partitions of the string into k parts
        for (int i = 1; i < n; ++i) {
            if (n % i != 0) continue; // i must be a divisor of n
            vector<int> changes(i, 0);
            ...
            for (int start = 0; start < i; ++start) {
                ...
                changes[start] = count - maxFreq;
            }
            
            // Sum up the changes for all groups
            int totalChanges = accumulate(changes.begin(), changes.end(), 0);
