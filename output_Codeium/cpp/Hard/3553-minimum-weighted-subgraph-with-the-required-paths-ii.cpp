...
public:
    vector<int> minimumWeight(vector<vector<int>>& edges, vector<vector<int>>& queries) {
        vector<int> result;
        for (auto& q : queries) {
            int x = find(edges, q[0]);
            int y = find(edges, q[1]);
            int z = find(edges, q[2]);
            int ans = distance(edges, x, y) + distance(edges, y, z);
    ...
    int find(vector<vector<int>>& edges, int x) {
        if (x == -1) return -1;
        if (edges[x][2] == -1) return x;
        return find(edges, edges[x][2]);
    }

    int distance(vector<vector<int>>& edges, int x, int y) {
        if (x == -1 || y == -1) return 0;
        if (x == y) return 0;
        int w = edges[x][2];
        edges[x][2] = -1;
        int ans = distance(edges, edges[x][1], y) + w;
        edges[x][2] = w;
        return ans;
    }
