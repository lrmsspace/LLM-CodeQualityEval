...
public:
    int findCrossingTime(int n, int k, vector<vector<int>>& time) {
        ...
        for (int i = 0; i < k; ++i) {
            leftQueue.push({time[i][0] + time[i][2], i});
        }
        int timeElapsed = 0, rightCount = 0;
        while (n > 0 || !rightQueue.empty()) {
            while (!leftQueue.empty() && leftQueue.top().first <= timeElapsed) {
                rightQueue.push({timeElapsed + time[leftQueue.top().second][1] + time[leftQueue.top().second][3], leftQueue.top().second});
                leftQueue.pop();
                --rightCount;
            }
            if (rightQueue.empty()) {
                timeElapsed = leftQueue.top().first;
            } else {
                timeElapsed = max(timeElapsed, rightQueue.top().first);
                if (rightCount < n) {
                    leftQueue.push({timeElapsed + time[rightQueue.top().second][0] + time[rightQueue.top().second][2], rightQueue.top().second});
                    ++rightCount;
                }
                rightQueue.pop();
                --n;
