...
public:
    vector<int> maximumCount(vector<int>& nums, vector<vector<int>>& queries) {
        ...
        for (auto& query : queries) {
            ...
            distinctPrimes[0] = primes[0].size();
            distinctPrimes[1] = primes[1].size();
            int maxSum = 0;
            for (int i = 0; i <= idx; ++i) {
                maxSum = max(maxSum, distinctPrimes[0] + distinctPrimes[1] - (isPrime(nums[i]) ? 1 : 0));
            }
