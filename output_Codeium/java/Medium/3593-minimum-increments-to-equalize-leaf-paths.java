...
class Solution {
    public int minIncrease(int n, int[][] edges, int[] cost) {
        ...
        int[] maxLeafCost = new int[n];
        boolean[] visited = new boolean[n];
        
        // Calculate maximum leaf cost for each node
        dfs(0, -1, cost, tree, maxLeafCost, visited);

        int maxPathCost = maxLeafCost[0];
        int increments = 0;

        // Calculate the number of nodes to be incremented
        for (int i = 0; i < n; i++) {
            if (maxLeafCost[i] < maxPathCost) {
                increments++;
            }
        }
    ...
    private void dfs(int node, int parent, int[] cost, Map<Integer, List<Integer>> tree, int[] maxLeafCost, boolean[] visited) {
        visited[node] = true;
        maxLeafCost[node] = cost[node];
        for (int neighbor : tree.getOrDefault(node, Collections.emptyList())) {
            if (neighbor != parent && !visited[neighbor]) {
                dfs(neighbor, node, cost, tree, maxLeafCost, visited);
                maxLeafCost[node] = Math.max(maxLeafCost[node], cost[node] + maxLeafCost[neighbor]);
            }
        }
    }
}