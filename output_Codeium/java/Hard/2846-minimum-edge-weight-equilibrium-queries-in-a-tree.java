...
class Solution {
    public int[] minOperationsQueries(int n, int[][] edges, int[][] queries) {
        ...
        for (int i = 0; i < queries.length; i++) {
            ...
            // BFS to find the path from start to end and count frequency of weights
            ...
            while (!queue.isEmpty()) {
                ...
                for (int[] neighbor : tree[node]) {
                    ...
                    if (!visited[nextNode] && nextNode != parent) {
                        queue.offer(new int[]{nextNode, node});
            ...
            int maxFreq = 0;
            for (int freq : weightCount.values()) {
                maxFreq = Math.max(maxFreq, freq);
            }
            result[i] = weightCount.size() - maxFreq;
        }
