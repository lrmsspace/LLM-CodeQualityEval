...
class Solution {
    public int minimumChanges(String s, int k) {
        ...
        for (int i = 1; i <= k; i++) {
            for (int j = 1; j <= n; j++) {
                for (int x = 0; x < j; x++) {
                    dp[i][j] = Math.min(dp[i][j], dp[i - 1][x] + calculateChangesToSemiPalindrome(s, x, j - x));
                }
            }
        }
    ...
    private int calculateChangesToSemiPalindrome(String s, int start, int end) {
        int changes = 0;
        for (int i = start; i < start + end; i++) {
            if (s.charAt(i) != s.charAt(start + end - 1 - (i - start))) {
                changes++;
            }
        }
        return changes;
