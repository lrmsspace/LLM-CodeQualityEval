...
class Solution {
    public int[] findXSum(int[] nums, int k, int x) {
        ...
        answer[0] = getSum(count, x);
        for (int i = 1; i < n - k + 1; i++) {
            count.put(nums[i + k - 1], count.getOrDefault(nums[i + k - 1], 0) + 1);
            count.put(nums[i - 1], count.get(nums[i - 1]) - 1);
            if (count.get(nums[i - 1]) == 0) {
                count.remove(nums[i - 1]);
            }
            answer[i] = getSum(count, x);
        }
        return answer;
    }

    private int getSum(Map<Integer, Integer> count, int x) {
        List<Map.Entry<Integer, Integer>> list = new ArrayList<>(count.entrySet());
        Collections.sort(list, (a, b) -> b.getValue() == a.getValue() ? b.getKey() - a.getKey() : b.getValue() - a.getValue());
        int sum = 0;
