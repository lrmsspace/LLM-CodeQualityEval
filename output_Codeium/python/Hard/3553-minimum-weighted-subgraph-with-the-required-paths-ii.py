...
class Solution(object):
    def minimumWeight(self, edges, queries):
        ...
        for src1, src2, dest in queries:
            dist_from_src1 = dijkstra(src1, graph, len(edges) + 1)
            dist_from_src2 = dijkstra(src2, graph, len(edges) + 1)
            dist_from_dest = dijkstra(dest, graph, len(edges) + 1)
            min_weight = float('inf')
            for node in range(len(edges) + 1):
                if dist_from_src1[node] != float('inf') and dist_from_src2[node] != float('inf') and dist_from_dest[node] != float('inf'):
                    total_weight = dist_from_src1[node] + dist_from_src2[node] + dist_from_dest[node]
