...
int countWords(char** words1, int words1Size, char** words2, int words2Size) {
    ...
    for (int i = 0; i < words1Size; i++) {
        ...
        for (j = 0; j < words2Size; j++) {
            if (!strcmp(words1[i], words2[j])) break;
        }
        if (j == words2Size) continue;
        int occur1 = 0, occur2 = 0;
        for (int k = 0; k < words1Size; k++) {
            if (!strcmp(words1[i], words1[k])) occur1++;
        }
        for (int k = 0; k < words2Size; k++) {
            if (!strcmp(words1[i], words2[k])) occur2++;
        }
        if (occur1 == 1 && occur2 == 1) count++;
    }
    return count;
