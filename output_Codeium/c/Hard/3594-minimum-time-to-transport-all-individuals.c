...
double minTime(int n, int k, int m, int* time, int timeSize, double* mul, int mulSize) {
    ...
    double dp[1 << n][m]; // dp[mask][stage] - minimum time to transport individuals with mask at stage
    for (int mask = 0; mask < (1 << n); ++mask) {
        for (int stage = 0; stage < m; ++stage) {
            dp[mask][stage] = DBL_MAX;
        }
    }
    dp[0][0] = 0.0;
    ...
    for (int mask = 0; mask < (1 << n); ++mask) {
        for (int stage = 0; stage < m; ++stage) {
            if (dp[mask][stage] == DBL_MAX) continue;
            for (int new_mask = 1; new_mask < (1 << n); ++new_mask) {
                if ((mask & new_mask) != 0) continue;
                if (__builtin_popcount(new_mask) > k) continue; // skip if group size exceeds capacity
                ...
                int next_stage = (stage + (int)floor(crossing_time)) % m;

                dp[mask | new_mask][next_stage] = fmin(dp[mask | new_mask][next_stage],
                                                       dp[mask][stage] + crossing_time);

                // Handle the return trip if individuals are left behind
                if (__builtin_popcount(mask | new_mask) < n) {
                    for (int i = 0; i < n; ++i) {
                        if (new_mask & (1 << i)) {
                            double return_time = time[i] * mul[next_stage];
                            int return_stage = (next_stage + (int)floor(return_time)) % m;
                            dp[mask][return_stage] = fmin(dp[mask][return_stage],
                                                          dp[mask | new_mask][next_stage] + return_time);
                        }
                    }
                }
            }
    ...
    for (int stage = 0; stage < m; ++stage) {
        result = fmin(result, dp[(1 << n) - 1][stage]);
    }
    return result == DBL_MAX ? -1.0 : result;
}
