...
int maxCandies(int* status, int statusSize, int* candies, int candiesSize, int** keys, int keysSize, int* keysColSize, int** containedBoxes, int containedBoxesSize, int* containedBoxesColSize, int* initialBoxes, int initialBoxesSize) {
    int ans = 0;
    int* vis = (int*)malloc(sizeof(int) * statusSize);
    memset(vis, 0, sizeof(int) * statusSize);
    for (int i = 0; i < initialBoxesSize; ++i) {
        if (vis[initialBoxes[i]]) continue;
        vis[initialBoxes[i]] = 1;
        ans += candies[initialBoxes[i]];
        for (int j = 0; j < keysColSize[initialBoxes[i]]; ++j) {
            int key = keys[initialBoxes[i]][j];
            status[key] = 1;
            for (int k = 0; k < containedBoxesColSize[key]; ++k) {
                int box = containedBoxes[key][k];
                if (!vis[box]) {
                    vis[box] = 1;
                    ans += candies[box];
                }
