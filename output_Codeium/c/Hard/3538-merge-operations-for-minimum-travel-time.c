...
int minTravelTime(int l, int n, int k, int* position, int positionSize, int* time, int timeSize) {
    int dp[51][51] = {0}; // dp[i][j] represents the minimum travel time with j merges for the first i signs
    ...
    for (int i = 1; i < n; ++i) {
        for (int j = 0; j <= k; ++j) {
            // Calculate the travel time without any merge
            int distance = position[i] - position[i - 1];
            dp[i][j] = dp[i - 1][j] + distance * time[i - 1];

            // Try merging with previous signs
            if (j > 0) {
                int sumTime = time[i];
                for (int m = i - 1; m > 0; --m) {
                    sumTime += time[m];
                    distance = position[i] - position[m - 1];
                    dp[i][j] = fmin(dp[i][j], dp[m - 1][j - 1] + distance * sumTime);
                }
            }
        }
    }
