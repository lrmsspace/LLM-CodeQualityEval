...
char*** deleteDuplicateFolder(char*** paths, int pathsSize, int* pathsColSize, int* returnSize, int** returnColumnSizes) {
    ...
    for (int i = 0; i < pathsSize; i++) {
        ...
        for (int j = 0; j < pathsColSize[i]; j++) {
            path += paths[i][j];
            if (j != pathsColSize[i] - 1) path += "/";
        }
        count[path]++;
        if (pathsColSize[i] > 1) {
            string parent = path.substr(0, path.find_last_of('/'));
            graph[parent].push_back(vector<string>(paths[i], paths[i] + pathsColSize[i]));
        }
    }
    vector<char*> result;
    for (auto& p : graph) {
        if (count[p.first] == 1) {
            for (auto& child : p.second) {
                int len = 0;
                for (auto& s : child) len += strlen(s);
                char* path = (char*)malloc(len + 1);
                int idx = 0;
                for (auto& s : child) {
                    for (int i = 0; s[i] != '\0'; i++) path[idx++] = s[i];
                    path[idx++] = '/';
                }
                path[--idx] = '\0';
    ...
    *returnColumnSizes = (int*)malloc((*returnSize) * sizeof(int));
    ...
    for (int i = 0; i < *returnSize; i++) {
        ...
        ret[i] = (char**)malloc((*returnColumnSizes)[i] * sizeof(char*));
