...
int minimumLevels(int* possible, int possibleSize) {
    int alicePoints = 0, bobPoints = 0, aliceLevels = 0;

    // Calculate initial points for Bob if Alice plays 0 levels.
    ...
    // Alice must play at least 1 level.
    for (int i = 0; i < possibleSize; i++) {
        if (possible[i] == 1) {
            alicePoints++;
            bobPoints--;
    ...
    // If not possible for Alice to have more points than Bob.
    return -1;
}
