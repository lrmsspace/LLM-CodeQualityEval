...
int minOperationsMaxProfit(int* customers, int customersSize, int boardingCost, int runningCost) {
    int max_profit = -1, max_rotations = -1, curr_profit = 0, rotations = 0, waiting = 0;
    for (int i = 0; i < customersSize || waiting > 0; i++) {
        if (i < customersSize) waiting += customers[i];
        int board = fmin(waiting, 4);
        waiting -= board;
        curr_profit += board * boardingCost - runningCost;
    ...
    return max_profit <= 0 ? -1 : max_rotations;
