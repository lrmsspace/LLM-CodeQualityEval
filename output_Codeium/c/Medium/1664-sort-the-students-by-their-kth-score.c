...
int** sortTheStudents(int** score, int scoreSize, int* scoreColSize, int k, int* returnSize, int** returnColumnSizes) {
    //Create a 2D array to store the sorted scores
    int** sortedScore = (int**)malloc(scoreSize * sizeof(int*));
    for (int i = 0; i < scoreSize; i++) {
        sortedScore[i] = (int*)malloc(*scoreColSize * sizeof(int));
    ...
    for (int i = 0; i < scoreSize - 1; i++) {
        for (int j = 0; j < scoreSize - i - 1; j++) {
            if (sortedScore[j][k] < sortedScore[j + 1][k]) {
    ...
    //Set the return size and column size
