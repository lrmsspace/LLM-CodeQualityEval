...
int* getOrder(int** tasks, int tasksSize, int* tasksColSize, int* returnSize) {
    ...
    qsort(tasks, tasksSize, sizeof(int[2]), compare);

    // Process the tasks
    for (int i = 0; i < tasksSize; i++) {
        int currentTime = tasks[i][0];
        int processingTime = tasks[i][1];

        // Find the task with the shortest processing time
        int minTime = INT_MAX;
        int minIndex = -1;
        for (int j = i; j < tasksSize; j++) {
            if (tasks[j][0] <= currentTime && tasks[j][1] < minTime) {
                minTime = tasks[j][1];
                minIndex = j;
            }
        }
        ...
        if (minIndex == -1) {
            ...
            currentTime = max(currentTime, tasks[minIndex][0]) + tasks[minIndex][1];
