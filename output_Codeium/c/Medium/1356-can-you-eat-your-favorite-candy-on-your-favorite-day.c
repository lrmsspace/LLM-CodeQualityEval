...
bool* canEat(int* candiesCount, int candiesCountSize, int** queries, int queriesSize, int* queriesColSize, int* returnSize) {
    ...
    for (int i = 0; i < queriesSize; i++) {
        ...
        for (int j = 0; j < favoriteType; j++) {
            required += candiesCount[j];
        }
        result[i] = total + dailyCap * favoriteDay >= required && total + candiesCount[favoriteType] <= dailyCap * (favoriteDay + 1);
        total += candiesCount[favoriteType];
    }
