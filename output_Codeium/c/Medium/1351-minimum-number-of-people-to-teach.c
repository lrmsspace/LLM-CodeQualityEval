...
int minimumTeachings(int n, int** languages, int languagesSize, int* languagesColSize, int** friendships, int friendshipsSize, int* friendshipsColSize) {
    ...
    for (int i = 0; i < friendshipsSize; i++) {
        ...
        for (int j = 0; j < languagesColSize[friendships[i][0] - 1]; j++)
            if (languages[friendships[i][0] - 1][j] == languages[friendships[i][1] - 1][0]) {
                a = 0;
                break;
            }
        for (int j = 0; j < languagesColSize[friendships[i][1] - 1]; j++)
            if (languages[friendships[i][1] - 1][j] == languages[friendships[i][0] - 1][0]) {
                b = 0;
                break;
            }
        if (a == -1) cnt[languages[friendships[i][0] - 1][0]]++;
        if (b == -1) cnt[languages[friendships[i][1] - 1][0]]++;
    }
    int ans = 0;
