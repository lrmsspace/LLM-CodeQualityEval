...
int wordCount(char** startWords, int startWordsSize, char** targetWords, int targetWordsSize) {
    ...
    for (int i = 0; i < targetWordsSize; ++i) {
        ...
        for (int j = 0; j < startWordsSize; ++j) {
            if (strlen(startWords[j]) + 1 == strlen(targetWords[i])) {
                char temp[27] = {0};
                strcpy(temp, startWords[j]);
                temp[strlen(startWords[j])] = targetWords[i][strlen(startWords[j])];
                qsort(temp, strlen(temp), sizeof(char), compare);
                if (strcmp(temp, targetWords[i]) == 0) {
                    found = true;
                    break;
                }
            }
