...
long long minNumberOfSeconds(int mountainHeight, int* workerTimes, int workerTimesSize) {
    long long left = 0, right = LLONG_MAX, result = LLONG_MAX;
    ...
    while (left <= right) {
        ...
        for (int i = 0; i < workerTimesSize; i++) {
            ...
            // Calculate max height worker i can reduce within 'mid' seconds
            for (int h = 1; ; h++) {
                long long timeNeeded = time * h * (h + 1) / 2;
                if (timeNeeded > mid) break;
                maxHeight = h;
            }
