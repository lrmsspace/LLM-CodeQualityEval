...
var minimumTotalPrice = function(n, edges, price, trips) {
    ...
    for (const trip of trips) {
        dfs(trip[0], graph, price, dp);
        dfs(trip[1], graph, price, dp);
    }
    let res = Infinity;
    for (let i = 0; i < n; i++) {
        res = Math.min(res, dp[i][0] + dp[i][1]);
    }
    return res;

    function dfs(node, graph, price, dp) {
        if (dp[node][0] !== Infinity) return;
        dp[node][0] = price[node];
        for (const neighbor of graph[node]) {
            dfs(neighbor, graph, price, dp);
            dp[node][0] += Math.min(dp[neighbor][0], dp[neighbor][1]);
            dp[node][1] = dp[node][0] - price[node] + dp[neighbor][1];
        }
    }
};