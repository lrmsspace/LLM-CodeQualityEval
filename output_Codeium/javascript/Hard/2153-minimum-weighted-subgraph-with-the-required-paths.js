...
var minimumWeight = function(n, edges, src1, src2, dest) {
    ...
    let res = Infinity;
    for (let k = 0; k < n; ++k) {
        if (k === src1 || k === src2 || k === dest) continue;
        const d1 = graph[src1][k] === Infinity ? Infinity : graph[src1][k] + graph[k][dest];
        const d2 = graph[src2][k] === Infinity ? Infinity : graph[src2][k] + graph[k][dest];
