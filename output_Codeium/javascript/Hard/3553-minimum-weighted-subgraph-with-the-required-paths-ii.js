...
var minimumWeight = function(edges, queries) {
    ...
    for (const [src1, src2, dest] of queries) {
        ...
        for (const [v1, w1] of graph[src1]) {
            for (const [v2, w2] of graph[src2]) {
                if (v1 === v2) {
                    minWeight = Math.min(minWeight, w1 + w2);
                    break;
                }
            }
        }
        for (const [v1, w1] of graph[dest]) {
            for (const [v2, w2] of graph[src1]) {
                if (v1 === v2) {
                    minWeight = Math.min(minWeight, w1 + w2);
                    break;
                }
