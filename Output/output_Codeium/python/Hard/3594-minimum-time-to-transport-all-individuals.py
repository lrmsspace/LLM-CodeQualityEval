    ...
    def minTime(self, n, k, m, time, mul):
        ...
        for bitmask in range(1 << n):
            for stage in range(m):
                if dp[stage][bitmask] == float('inf'):
                    continue

                for i in range(n):
                    if bitmask & (1 << i) == 0:
                        new_mask = bitmask | (1 << i)
                        time_taken = time[i] * mul[stage]
                        next_stage = (stage + math.floor(time_taken)) % m
                        dp[next_stage][new_mask] = min(dp[next_stage][new_mask], dp[stage][bitmask] + time_taken)
        
        min_time = float('inf')
        for stage in range(m):
            min_time = min(min_time, dp[stage][(1 << n) - 1])
