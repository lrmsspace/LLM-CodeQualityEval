...
class Solution(object):
    def findCrossingTime(self, n, k, time):
        ...
        left, right = [], []
        for i in range(k):
            heapq.heappush(left, (time[i][0] + time[i][2], i))
        timeUsed = 0
        while n > 0 or right:
            while left and left[0][0] <= timeUsed:
                _, i = heapq.heappop(left)
                heapq.heappush(right, (time[i][1] + time[i][3], i))
            if right:
                t, i = heapq.heappop(right)
                timeUsed = max(timeUsed, t)
                n -= 1
            else:
                timeUsed = left[0][0]
            while left and left[0][0] == timeUsed:
