...
class Solution(object):
    def getOrder(self, tasks):
        ...
        # Add index to tasks for tracking the original order
        tasks = [(enqueue, process, i) for i, (enqueue, process) in enumerate(tasks)]
        # Sort tasks based on enqueue time
        tasks.sort()

        result = []
        min_heap = []  # Min-heap to store available tasks based on processing time and index
        time = 0
        i = 0
        n = len(tasks)
        ...
        while i < n or min_heap:
            ...
            if min_heap:
                # Get the task with the smallest processing time (and smallest index)
