...
class Solution(object):
    def mostPopularCreator(self, creators, ids, views):
        ...
        for creator, videoId, view in zip(creators, ids, views):
            if creator in creatorViews:
                creatorViews[creator][0] += view
                if view > creatorViews[creator][1][1]:
                    creatorViews[creator][1] = (videoId, view)
            else:
                creatorViews[creator] = [view, (videoId, view)]
            maxViews = max(maxViews, view)
        result = []
        for creator, (totalView, (videoId, view)) in creatorViews.items():
            if totalView == maxViews:
                result.append([creator, videoId])
        return result
