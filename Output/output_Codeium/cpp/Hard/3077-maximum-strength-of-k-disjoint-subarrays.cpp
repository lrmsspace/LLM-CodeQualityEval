...
public:
    long long maximumStrength(vector<int>& nums, int k) {
        ...
        dp[0][0] = 0; // base case: 0 subarrays selected, strength is 0

        for (int i = 1; i <= k; ++i) {
            for (int j = i; j <= n; ++j) { // ensure at least i elements are used
                for (int m = i - 1; m < j; ++m) {
                    long long currentStrength = (i * (prefixSum[j] - prefixSum[m]));
                    dp[i][j] = max(dp[i][j], dp[i-1][m] + currentStrength);
                }
            }
        }
        ...
        for (int i = k; i <= n; ++i) {
            maxStrength = max(maxStrength, dp[k][i]);
        }
