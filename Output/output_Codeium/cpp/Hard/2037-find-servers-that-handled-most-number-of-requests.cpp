...
public:
    vector<int> busiestServers(int k, vector<int>& arrival, vector<int>& load) {
        ...
        for (int i = 0; i < arrival.size(); ++i) {
            ...
            if (it == availableServers.end()) it = availableServers.begin();
            
            int serverIndex = *it;
            availableServers.erase(it);
            
            requestsHandled[serverIndex]++;
            busyServers.emplace(arrival[i] + load[i], serverIndex);
        }
        
        int maxRequests = *max_element(requestsHandled.begin(), requestsHandled.end());
        vector<int> result;
        for (int i = 0; i < k; ++i) {
            if (requestsHandled[i] == maxRequests) result.push_back(i);
        }
        return result;
