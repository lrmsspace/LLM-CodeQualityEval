...
public:
    int maxConsecutive(int bottom, int top, vector<int>& special) {
        ...
        maxConsecutiveFloors = max(maxConsecutiveFloors, special[0] - bottom);

        // Check the gaps between the special floors
        for (int i = 1; i < special.size(); ++i) {
            maxConsecutiveFloors = max(maxConsecutiveFloors, special[i] - special[i - 1] - 1);
        }

        // Check the gap after the last special floor
        maxConsecutiveFloors = max(maxConsecutiveFloors, top - special.back());
