...
class Solution {
    public List<List<String>> mostPopularCreator(String[] creators, String[] ids, int[] views) {
        ...
        for (int i = 0; i < creators.length; i++) {
            ...
            if (!maxView.containsKey(creators[i]) || views[i] > maxView.get(creators[i])) {
                maxView.put(creators[i], views[i]);
            }
        }
        ...
        for (Map.Entry<String, Integer> entry : popularity.entrySet()) {
            ...
            if (entry.getValue() == maxPopularity) {
                res.add(Arrays.asList(entry.getKey(), ids[ Arrays.stream(creators).findFirst().indexOf(entry.getKey()) ]));
            }
        }
        return res;
    }
