...
class Solution {
    public double minTime(int n, int k, int m, int[] time, double[] mul) {
        double totalTime = 0;
        int currentStage = 0;
        boolean[] atDestination = new boolean[n];

        while (true) {
            int remaining = 0;
            for (boolean b : atDestination) {
                if (!b) remaining++;
            }
            if (remaining == 0) break;

            int[] group = new int[Math.min(k, remaining)];
            int gIndex = 0;

            for (int i = 0; i < n && gIndex < group.length; i++) {
                if (!atDestination[i]) {
                    group[gIndex++] = i;
                }
            }
            ...
            for (int i : group) {
                atDestination[i] = true;
            }

            int nextStage = (currentStage + (int)Math.floor(crossingTime)) % m;
            if (remaining > group.length) {
                int returnIndex = group[0];
                double returnTime = time[returnIndex] * mul[nextStage];
                totalTime += returnTime;
                atDestination[returnIndex] = false;
                currentStage = (nextStage + (int)Math.floor(returnTime)) % m;
            } else {
