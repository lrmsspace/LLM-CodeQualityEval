...
double log10OfProduct = 0;
long trailingZeros = 0;
long firstFiveDigits = 1, lastFiveDigits = 1;

for (int i = left; i <= right; i++) {
    log10OfProduct += Math.log10(i);
    if (firstFiveDigits < 1e10) {
        firstFiveDigits *= i;
        while (firstFiveDigits >= 1e5) {
            firstFiveDigits /= 10;
        }
    }
    lastFiveDigits *= i;
    while (lastFiveDigits % 10 == 0) {
        lastFiveDigits /= 10;
        trailingZeros++;
    }
    lastFiveDigits %= 1e5;
...
int totalDigits = (int) log10OfProduct;
String result;
if (totalDigits > 10) {
    result = String.format("%de%d", firstFiveDigits, trailingZeros);
} else {
    result = String.valueOf(lastFiveDigits);
}
return result;
    }
}