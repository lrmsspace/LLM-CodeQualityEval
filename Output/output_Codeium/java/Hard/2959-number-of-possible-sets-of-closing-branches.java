...
class Solution {
    ...
    int result = 0;
    Set<Set<Integer>> possibleSets = new HashSet<>();
    ...
    for (int mask = 0; mask < (1 << n); mask++) {
        ...
        if (isValid(closed, n, maxDistance, roads)) {
            possibleSets.add(closed);
        }
    }
...
private boolean isValid(Set<Integer> closed, int n, int maxDistance, int[][] roads) {
    ...
    // Update distances based on roads
    for (int[] road : roads) {
        int u = road[0];
        int v = road[1];
        int w = road[2];
    ...
    // Check if all remaining branches are within maxDistance
    for (int i = 0; i < n; i++) {
        if (!closed.contains(i)) {
            for (int j = 0; j < n; j++) {
                if (!closed.contains(j) && distance[i][j] > maxDistance) {
                    return false;
                }
            }
        }
