...
int unhappyFriends(int n, int** preferences, int preferencesSize, int* preferencesColSize, int** pairs, int pairsSize, int* pairsColSize) {
    ...
    for (int i = 0; i < pairsSize; i++) {
        ...
        int rankX = -1;
        int rankY = -1;
        for (int j = 0; j < preferencesColSize[x]; j++) {
            if (preferences[x][j] == y) {
                rankX = j;
                break;
            }
        }
        for (int j = 0; j < preferencesColSize[y]; j++) {
        ...
        for (int j = 0; j < n; j++) {
            if (j != x && j != y) {
                ...
                for (int k = 0; k < preferencesColSize[x]; k++) {
                    if (preferences[x][k] == j) {
                        rankX2 = k;
                        break;
                    }
                }
                for (int k = 0; k < preferencesColSize[y]; k++) {
                    if (preferences[y][k] == j) {
                        rankY2 = k;
                        break;
                    }
                }
                if (rankX > rankX2 && rankY > rankY2) {
                    unhappy++;
                    break;
                }
            }
