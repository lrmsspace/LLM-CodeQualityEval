...
int** specialGrid(int n, int* returnSize, int** returnColumnSizes) {
    int totalNum = 1 << (2 * n);
    int** ans = (int**)malloc(sizeof(int*) * totalNum);
    int* colSizes = (int*)malloc(sizeof(int) * totalNum);
    int index = 0;
    
    void dfs(int n, int start, int end) {
        if (start == end) {
            ans[index] = (int*)malloc(sizeof(int) * 1);
            ans[index][0] = start;
            colSizes[index++] = 1;
            return;
        }
        ...
        int mid = (start + end) / 2;
        int leftSize = (mid - start + 1) / 2;
        int rightSize = (end - mid) / 2;
        
        ans[index] = (int*)malloc(sizeof(int) * (leftSize + rightSize));
        int k = 0;
        ...
        colSizes[index++] = leftSize + rightSize;
