...
char** watchedVideosByFriends(char*** watchedVideos, int watchedVideosSize, int* watchedVideosColSize, int** friends, int friendsSize, int* friendsColSize, int id, int level, int* returnSize) {
    ...
    while (!isEmpty(&q) && currentLevel < level) {
    ...
    HashMap videoCount;
    initHashMap(&videoCount);
    while (!isEmpty(&q)) {
        int person = dequeue(&q);
        for (int j = 0; j < watchedVideosColSize[person]; j++) {
            char* video = watchedVideos[person][j];
            incrementCount(&videoCount, video);
        }
    }

    // Create result array
    *returnSize = getHashMapSize(&videoCount);
    char** result = (char**)malloc(*returnSize * sizeof(char*));
    fillResultArray(&videoCount, result);

    // Sort result array by frequency and then lexicographically
    qsort(result, *returnSize, sizeof(char*), compareVideos);
