 ...
 */ 
void dfs(char** board, int m, int n, int r, int c, int directions[8][2]) {
    ...
    if (mineCount > 0) {
        ...
        board[r][c] = 'B';
        for (int i = 0; i < 8; i++) {
            dfs(board, m, n, r + directions[i][0], c + directions[i][1], directions);
        }
    }
}
char** updateBoard(char** board, int boardSize, int* boardColSize, int* click, int clickSize, int* returnSize, int** returnColumnSizes) {
    ...
    if (board[clickr][clickc] == 'M') {
        ...
        dfs(board, m, n, clickr, clickc, directions);
    }
