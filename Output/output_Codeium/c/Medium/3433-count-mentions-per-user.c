...
int* countMentions(int numberOfUsers, char*** events, int eventsSize, int* eventsColSize, int* returnSize) {
    ...
    memset(mentions, 0, numberOfUsers * sizeof(int));

    for (int i = 0; i < eventsSize; i++) {
        if (strcmp(events[i][0], "MESSAGE") == 0) {
            char* token = strtok(events[i][2], " ");
            while (token) {
                if (strcmp(token, "ALL") == 0) {
                    ...
                    for (int j = 0; j < eventsSize; j++) {
                        if (i != j && strcmp(events[j][0], "OFFLINE") == 0 && atoi(events[j][1]) <= atoi(events[i][1]) && atoi(events[j][1]) + 60 > atoi(events[i][1])) {
                            continue;
                        }
