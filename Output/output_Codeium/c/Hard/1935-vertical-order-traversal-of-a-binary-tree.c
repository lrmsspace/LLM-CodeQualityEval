 ...
 */ 
// Define a structure to store node information along with its row and column
typedef struct {
    int val;
    int row;
    int col;
...
// Comparator function to sort NodeInfo
...
void dfs(struct TreeNode* root, int row, int col, NodeInfo* nodes, int* count) {
    if (!root) return;
...
int** verticalTraversal(struct TreeNode* root, int* returnSize, int** returnColumnSizes) {
    ...
    qsort(nodes, count, sizeof(NodeInfo), compare);

    int** result = (int**)malloc(1000 * sizeof(int*));
    *returnColumnSizes = (int*)malloc(1000 * sizeof(int));
    *returnSize = 0;

    int currentCol = nodes[0].col;
    ...
    for (int i = 0; i < count; i++) {
        ...
        result[*returnSize][size++] = nodes[i].val;
    }
    (*returnColumnSizes)[*returnSize] = size;
    (*returnSize)++;
