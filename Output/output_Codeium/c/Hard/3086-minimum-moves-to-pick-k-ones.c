...
long long minimumMoves(int* nums, int numsSize, int k, int maxChanges) {
    int i, j, currentOnes = 0, minMoves = numsSize;
    int left = 0, right = 0, changes = 0;

    // Initial count of ones in the first window
    ...
    while (right < numsSize) {
        if (nums[right] == 0 && changes < maxChanges) {
            changes++;
        }
        // If we have picked enough ones
        if (currentOnes == k) {
            minMoves = right - left;
            if (nums[left] == 0) changes--;
            left++;
            currentOnes--;
        }
        right++;
