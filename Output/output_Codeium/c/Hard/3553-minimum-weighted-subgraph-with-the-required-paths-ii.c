...
int* minimumWeight(int** edges, int edgesSize, int* edgesColSize, int** queries, int queriesSize, int* queriesColSize, int* returnSize) {
    *returnSize = queriesSize;
    ...
    for (int i = 0; i < queriesSize; i++) {
        ...
        int* depth = (int*)malloc(edgesSize * sizeof(int));
        dfs(edges, edgesSize, edgesColSize, src1, parent, depth, 0);
        dist1 = depth[dest];
        dfs(edges, edgesSize, edgesColSize, src2, parent, depth, 0);
        dist2 = depth[dest];
        dfs(edges, edgesSize, edgesColSize, dest, parent, depth, 0);
        dist3 = depth[src1] + depth[src2];
        result[i] = dist1 + dist2 - dist3;
    }
    return result;
}