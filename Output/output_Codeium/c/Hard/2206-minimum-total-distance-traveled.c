...
long long minimumTotalDistance(int* robot, int robotSize, int** factory, int factorySize, int* factoryColSize) {
    // sort the robots and the factories
    qsort(robot, robotSize, sizeof(int), compare);
    qsort(factory, factorySize, sizeof(int*), compareFactory);

    // dp[i][j] is the minimum total distance if we know the first i robots and the first j factories
    ...
    for (int i = 0; i < robotSize; i++) {
        for (int j = 0; j < factorySize; j++) {
            if (dp[i][j] != LLONG_MAX) {
                // move the robot to the current factory
                dp[i + 1][j + 1] = min(dp[i + 1][j + 1], dp[i][j] + abs(robot[i] - factory[j][0]));
                // move the robot to the next factory
                dp[i + 1][j] = min(dp[i + 1][j], dp[i][j] + abs(robot[i] - factory[j + 1][0]));
            }
        }
    }
