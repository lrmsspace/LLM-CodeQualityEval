...
char* abbreviateProduct(int left, int right) {
    ...
    for (int i = left; i <= right; i++) {
        ...
        product %= 10000000000; // Keep product manageable by limiting its size
    }

    int length = snprintf(result, 20, "%llue%d", product, trailingZeros);

    if (length > 10) {
        char prefix[6], suffix[6];
        snprintf(prefix, 6, "%llu", product);
        snprintf(suffix, 6, "%llu", product % 100000);
        snprintf(result, 20, "%s...%se%d", prefix, suffix, trailingZeros);
    }
