...
int findCrossingTime(int n, int k, int** time, int timeSize, int* timeColSize) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (int j = 0; j < k; j++) {
            int efficiency = time[j][0] + time[j][2];
            if (efficiency > maxEfficiency || (efficiency == maxEfficiency && j > selectedWorker)) {
                maxEfficiency = efficiency;
                selectedWorker = j;
            }
        }
        
        totalTime += time[selectedWorker][0] + time[selectedWorker][1] + time[selectedWorker][2] + time[selectedWorker][3];
    }
