            ...
            if (coords[i][0] === coords[j][0]) { // x-axis parallel, same x-coordinate
                for (let k = 0; k < coords.length; k++) {
                    if (k !== i && k !== j && (coords[k][1] === coords[i][1] || coords[k][1] === coords[j][1])) {
                        let base = Math.abs(coords[j][1] - coords[i][1]);
                        let height = Math.abs(coords[k][0] - coords[i][0]);
                ...
                for (let k = 0; k < coords.length; k++) {
                    if (k !== i && k !== j && (coords[k][0] === coords[i][0] || coords[k][0] === coords[j][0])) {
                        let base = Math.abs(coords[j][0] - coords[i][0]);
                        let height = Math.abs(coords[k][1] - coords[i][1]);
                        let area = base * height;
                        if (area > maxArea) {
                            maxArea = area;
                        }
