...
var minimumCost = function(start, target, specialRoads) {
    ...
    while (q.length) {
        ...
        if (cost > minCost || visited.has(pos.join(','))) {
            continue;
        }
        visited.add(pos.join(','));
        for (let road of specialRoads) {
            if (road[0] === pos[0] && road[1] === pos[1]) {
                q.push([[road[2], road[3]], cost + road[4]]);
            }
        }
        q.push([[pos[0] + 1, pos[1]], cost + 1]);
        q.push([[pos[0] - 1, pos[1]], cost + 1]);
        q.push([[pos[0], pos[1] + 1], cost + 1]);
        q.push([[pos[0], pos[1] - 1], cost + 1]);
    }
    return minCost;
};