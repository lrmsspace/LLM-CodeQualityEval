...
var minEdgeReversals = function(n, edges) {
    ...
    // BFS to find minimum reversals starting from each node
    const bfs = (startNode) => {
        ...
        while (queue.length) {
            const [node, reversals] = queue.shift();
            answer[node] = Math.min(answer[node], reversals);

            for (const neighbor of graph[node]) {
                if (!visited[neighbor]) {
                    visited[neighbor] = true;
                    queue.push([neighbor, reversals]);
                }
            }
            ...
            for (const neighbor of reverseGraph[node]) {
                if (!visited[neighbor]) {
                    visited[neighbor] = true;
                    queue.push([neighbor, reversals + 1]);
                }
            }
        }
    };
