...
var goodSubtreeSum = function(vals, par) {
    let MOD = 1e9 + 7;
    let maxScore = new Array(n).fill(0);
    function dfs(node, parent) {
        ...
        for (let i = 0; i < n; i++) {
            if (par[i] === node) {
                let [res, seen2] = dfs(i, node);
                if (res > 0) {
                    score += res;
                    for (let j = 0; j < 10; j++) {
                        if (!seen2.has(j)) seen.delete(j);
                    }
                }
            }
        }
        for (let i = 0; i < 10; i++) {
            if (!seen.has(i)) {
                let digit = (vals[node] + '').indexOf(i + '');
                if (digit === -1) continue;
                let val = parseInt((vals[node] + '').substr(digit, 1));
                score += val;
            }
        }
        maxScore[node] = score;
