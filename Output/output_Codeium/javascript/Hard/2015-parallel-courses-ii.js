...
let prerequisites = Array(n).fill(0);
let dp = Array(1 << n).fill(Number.MAX_VALUE);
...
for (let state = 0; state < (1 << n); state++) {
    ...
    for (let i = 0; i < n; i++) {
        if ((state & prerequisites[i]) === prerequisites[i]) {
            availableCourses |= (1 << i);
        }
    }
    availableCourses &= ~state;

    for (let subset = availableCourses; subset > 0; subset = (subset - 1) & availableCourses) {
