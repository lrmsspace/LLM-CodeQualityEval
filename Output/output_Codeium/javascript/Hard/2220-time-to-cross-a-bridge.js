...
var findCrossingTime = function(n, k, time) {
    let workers = time.map((t, i) => [t[0] + t[1] + t[2] + t[3], i]).sort((a, b) => a[0] - b[0]);
    let left = [], right = [], timeElapsed = 0, nBoxesLeft = n;
    while (nBoxesLeft > 0) {
        while (right.length && right[0][0] <= timeElapsed) {
            let [, i] = right.shift();
            left.push([time[i][2] + timeElapsed, i]);
            left.sort((a, b) => a[0] - b[0]);
        }
        if (left.length < k && nBoxesLeft) {
            let [, i] = workers.pop();
            right.push([time[i][0] + timeElapsed, i]);
            right.sort((a, b) => a[0] - b[0]);
            nBoxesLeft--;
        }
        timeElapsed = Math.min(left[0][0], right[0][0]);
        left[0][0] -= timeElapsed;
        right[0][0] -= timeElapsed;
    }
    return timeElapsed;
};