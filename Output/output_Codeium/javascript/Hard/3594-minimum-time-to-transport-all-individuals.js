...
var minTime = function(n, k, m, time, mul) {
    if (k >= n) {
        return Math.max(...time) * mul[0];
    }
    const dp = new Array(m).fill(0).map(() => new Array(k + 1).fill(Infinity));
    dp[0][0] = 0;
    for (let i = 0; i < m; i++) {
        for (let j = 1; j <= k; j++) {
            for (let person = 0; person < n; person++) {
                const nextStage = (i + Math.floor(time[person] * mul[i])) % m;
                dp[nextStage][j] = Math.min(dp[nextStage][j], Math.max(dp[i][j - 1], time[person] * mul[i]));
            }
        }
    }
    return dp[m - 1][k] === Infinity ? -1 : dp[m - 1][k];
};