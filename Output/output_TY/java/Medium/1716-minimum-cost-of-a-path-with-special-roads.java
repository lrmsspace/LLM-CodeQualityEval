...
class Solution {
    public int minimumCost(int[] start, int[] target, int[][] specialRoads) {
        ...
        for(int[] road: specialRoads){
            ...
            if(Math.abs(x1-start[0]) + Math.abs(y1-start[1]) <= cost && Math.abs(x2-target[0]) + Math.abs(y2-target[1]) <= cost){
                ans = Math.min(ans, Math.abs(x1-start[0]) + Math.abs(y1-start[1]));
                ans = Math.min(ans, Math.abs(x2-target[0]) + Math.abs(y2-target[1]));
            }
        }
        return ans == 0 ? Math.abs(start[0]-target[0]) + Math.abs(start[1]-target[1]) : ans;
    }
}