...
class Solution {
    public int[] colorTheArray(int n, int[][] queries) {
        ...
        for(int i=0;i<queries.length;i++){
            ...
            if(index<n-1 && arr[index]==arr[index+1]) count--;
            if(prev>0 && prev==arr[index]) count++;
            if(index>0 && prev>0 && prev==arr[index-1]) count++;
        }    
        ans[0]=count;
        for(int i=1;i<queries.length;i++){
            ...
            int prev=arr[index];
            if(index>0 && prev>0 && prev==arr[index-1]) count--;
            if(index<n-1 && prev>0 && prev==arr[index+1]) count--;
            arr[index]=color;
            if(index>0 && arr[index]==arr[index-1]) count++;
            if(index<n-1 && arr[index]==arr[index+1]) count++;
            ans[i]=count;
