...
class Solution(object):
    def topStudents(self, positive_feedback, negative_feedback, report, student_id, k):
        ...
        for i in range(len(positive_feedback)):
            pos_map[positive_feedback[i]]=3
        for i in range(len(negative_feedback)):
            neg_map[negative_feedback[i]]=-1
        res=[]
        for i in range(len(report)):
            score=0
            for j in range(len(report[i].split(" "))):
                if report[i].split(" ")[j] in pos_set:
                    score+=pos_map[report[i].split(" ")[j]]
            for j in range(len(report[i].split(" "))):
                if report[i].split(" ")[j] in neg_set:
                    score+=neg_map[report[i].split(" ")[j]]
