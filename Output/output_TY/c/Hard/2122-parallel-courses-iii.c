...
int minimumTime(int n, int** relations, int relationsSize, int* relationsColSize, int* time, int timeSize){
    ...
    memset(maxTime, 0, n * sizeof(int));
    int result = 0;
    for (int i = 0; i < queueSize; i++) {
        int course = queue[i];
        result = max(result, maxTime[course]);
        for (int j = 0; j < relationsSize; j++) {
            if(relations[j][0] - 1 == course) {
                maxTime[relations[j][1] - 1] = max(maxTime[relations[j][1] - 1], maxTime[course] + time[course]);
            }
        }
    }
    for (int i = 0; i < n; i++) {
        result = max(result, maxTime[i] + time[i]);
    }
    return result;
}