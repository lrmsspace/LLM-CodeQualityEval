...
int* treeQueries(int n, int** edges, int edgesSize, int* edgesColSize, int** queries, int queriesSize, int* queriesColSize, int* returnSize) {
    ...
    *returnSize = queriesSize;
    for (int i = 0; i < queriesSize; i++) {
        ...
        int query_node = query[1];
        if (query_type == 1) {
            int query_weight = query[2];
            int query_node_parent = query[3];
            int query_node_depth = query[4];
            int query_node_max_depth = query[5];
            int query_node_max_depth_node = query[6];
            int query_node_max_depth_node_parent = query[7];
            int query_node_max_depth_node_depth = query[8];
        }
    }
    return 0;
}