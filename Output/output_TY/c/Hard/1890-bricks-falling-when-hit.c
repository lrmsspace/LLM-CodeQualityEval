...
int* hitBricks(int** grid, int gridSize, int* gridColSize, int** hits, int hitsSize, int* hitsColSize, int* returnSize){    
    ...
    *returnSize = hitsSize;
    ...
    int* dp2 = (int*)malloc(m * n * sizeof(int));
    memset(dp2, 0, m * n * sizeof(int));
    int* dp3 = (int*)malloc(m * n * sizeof(int));
    memset(dp3, 0, m * n * sizeof(int));
    for (int i = 0; i < m; i++) {
        for (int j = 0; j < n; j++) {
            if (grid[i][j] == 0) {
                continue;
    ...
    for (int i = 0; i < m; i++) {
        for (int j = 0; j < n; j++) {
            ...
            dp2[i * n + j] = min(dp2[i * n + j], dp[(i + 1) * n + j]);
        }
    }
    return dp2[m * n - 1];
}