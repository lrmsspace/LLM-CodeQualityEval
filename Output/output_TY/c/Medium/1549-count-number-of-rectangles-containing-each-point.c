...
int* countRectangles(int** rectangles, int rectanglesSize, int* rectanglesColSize, int** points, int pointsSize, int* pointsColSize, int* returnSize){
    ...
    a = (int *)malloc(sizeof(int) * pointsSize);
    for (i = 0; i < pointsSize; i++) {
        a[i] = 0;
    }
    b = (int *)malloc(sizeof(int) * pointsSize);
    for (i = 0; i < pointsSize; i++) {
        b[i] = 0;
    }
    for (i = 0; i < rectanglesSize; i++) {
        for (j = 0; j < pointsSize; j++) {
            if (rectangles[i][0] <= points[j][0] && rectangles[i][2] >= points[j][0] && rectangles[i][1] <= points[j][1] && rectangles[i][3] >= points[j][1]) {
                a[j]++;              
            }
        }
    }
