...
int* baseUnitConversions(int** conversions, int conversionsSize, int* conversionsColSize, int* returnSize) {
    ...
    int *ret = (int *)malloc(sizeof(int) * conversionsSize);
    memset(ret, 0, sizeof(int) * conversionsSize);
    
    int i, j, k;
    int *p;
    
    for (i = 0; i < conversionsSize; i++) {
        for (j = 0; j < conversionsSize; j++) {
            if (i == j) {
                continue;
            }
            
            p = conversions[j];
            for (k = 0; k < conversionsColSize[j]; k++) {
                if (p[k] == conversions[i][0]) {
                    ret[i] = conversions[i][1] * p[ k + 1] / conversions[j][1]; 
                }
            }
        }
    }
