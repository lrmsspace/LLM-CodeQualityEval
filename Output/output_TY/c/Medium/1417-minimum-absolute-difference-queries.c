...
int* minDifference(int* nums, int numsSize, int** queries, int queriesSize, int* queriesColSize, int* returnSize){
    ...
    *returnSize = queriesSize;
    ...
    memset(prefix, 0, sizeof(int) * numsSize);
    memset(suffix, 0, sizeof(int) * numsSize);
    memset(min, 0, sizeof(int) * numsSize);
    memset(max, 0, sizeof(int) * numsSize);
    for (int i = 0; i < numsSize; i++) {
        prefix[i] = (i == 0 ? 0 : prefix[i - 1]) + nums[i];
        suffix[numsSize - 1 - i] = (i == 0 ? 0 : suffix[numsSize - i]) + nums[numsSize - 1 - i];
    }
    for (int i = 0; i < numsSize; i++) {
        min[i] = (i == 0 ? 0 : min[i - 1]) + (nums[i] < min[i] ? nums[i] : min[i]);
        max[numsSize - 1 - i] = (i == 0 ? 0 : max[numsSize - i]) + (
            nums[numsSize - 1 - i] > max[numsSize - i] ? nums[numsSize - 1 - i] : max[numsSize - i]
        );
    }
    for (int i = 0; i < queriesSize; i++) {
        ...
        for (int j = li; j <= ri; j++) {
            int diff = 0;
            if (j == li) {
                diff = prefix[ri] - suffix[j + 1];
            } else if (j == ri) {
                diff = prefix[j - 1] - suffix[ri];
            } else {
                diff = prefix[j - 1] - suffix[j + 1];
            }
            }
        }
        ans[i] = min_val;
