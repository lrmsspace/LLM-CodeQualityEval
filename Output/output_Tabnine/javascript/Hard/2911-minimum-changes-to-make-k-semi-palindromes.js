...
var minimumChanges = function(s, k) {
    ...
    // Calculate the sum of all characters' counts
    let sum = 0;
    for (let count of Object.values(charCount)) {
        sum += count;
    }
    
    // Calculate the target number of characters to make semi-palindromes
    let target = Math.floor(sum / k);
    
    // Calculate the minimum number of changes required
    let changes = 0;
    for (let count of Object.values(charCount)) {
        changes += Math.abs(count - target);
    }
    
    return changes / 2; // Return the average number of changes per semi-palindrome
