...
var medianOfUniquenessArray = function(nums) {
    // Convert nums to a sorted array of uniqueness counts
    const uniquenessCounts = Array(nums.length + 1).fill(0);
    for (let i = 0; i < nums.length; i++) {
        uniquenessCounts[nums[i]]++;
    }
    
    let totalCount = 0;
    let medianIndex = Math.floor(nums.length / 2);
    for (let i = 1; i <= nums.length; i++) {
        totalCount += uniquenessCounts[i];
