...
var minimumDifference = function(nums, k) {
    const n = nums.length;
    const dp = Array(n).fill(0).map(() => Array(1024).fill(Infinity));
    dp[0][nums[0]] = 0;
    
    for (let i = 1; i < n; i++) {
        for (let j = 0; j < 1024; j++) {
            dp[i][j] = Math.min(dp[i][j], dp[i - 1][j] + Math.abs(k - j));
            dp[i][j | nums[i]] = Math.min(dp[i][j | nums[i]], dp[i - 1][j] + Math.abs(k - (j | nums[i])));
        }
    }
