...
var minDifference = function(nums, queries) {
    ...
    for (let i = 1; i <= n; i++) {
        prefixSum[i] = prefixSum[i - 1] + freq[nums[i - 1]];
    }
    
    const ans = new Array(queries.length);
    for (let i = 0; i < queries.length; i++) {
        const [left, right] = queries[i];
        const minDiff = Math.min(prefixSum[right + 1] - prefixSum[left], prefixSum[n] - prefixSum[right + 1]);
        ans[i] = minDiff === prefixSum[n]? -1 : minDiff;
    }
