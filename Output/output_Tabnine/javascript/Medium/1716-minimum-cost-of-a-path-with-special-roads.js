...
var minimumCost = function(start, target, specialRoads) {
    // Build the graph
    const graph = Array.from({ length: 106 }, () => Array.from({ length: 106 }, () => Infinity));
    for (const [x1, y1, x2, y2, cost] of specialRoads) {
        graph[x1][x2] = Math.min(graph[x1][x2], cost);
        graph[x2][x1] = Math.min(graph[x2][x1], cost);
    }
    
    // Dijkstra's algorithm
    const distances = Array.from({ length: 106 }, () => Infinity);
    distances[start[0]] = distances[start[1]] = 0;
    const visited = new Set([start[0], start[1]]);
    const queue = [{ x: start[0], y: start[1], cost: 0 }];
    
    while (queue.length) {
        const { x, y, cost } = queue.shift();
        
        for (const [nx, ny] of [[x - 1, y], [x + 1, y], [x, y - 1], [x, y + 1]]) {
            if (nx < 1 || nx > 105 || ny < 1 || ny > 105 || visited.has([nx, ny])) continue;
            
            const newCost = cost + graph[x][ny];
            if (newCost < distances[nx]) {
                distances[nx] = newCost;
