...
var minCost = function(startPos, homePos, rowCosts, colCosts) {
    ...
    // Initialize a 2D array dp to store the minimum cost to reach each cell from the start cell
    const dp = Array.from({length: m}, () => Array.from({length: n}, () => Infinity));
    dp[startPos[0]][startPos[1]] = 0;
    
    // Perform a breadth-first search (BFS) to find the minimum cost to reach each cell
    const queue = [[startPos[0], startPos[1]]];
    while (queue.length > 0) {
        const [row, col] = queue.shift();
        
        // Check the four possible directions: left, right, up, and down
        const directions = [[row - 1, col], [row + 1, col], [row, col - 1], [row, col + 1]];
        for (const [dr, dc] of directions) {
            if (0 <= dr && dr < m && 0 <= dc && dc < n) {
                const newCost = dp[row][col] + Math.min(rowCosts[dr], colCosts[dc]);
                if (newCost < dp[dr][dc]) {
                    dp[dr][dc] = newCost;
                    queue.push([dr, dc]);
                }
    ...
    // Return the minimum cost to reach the home cell
