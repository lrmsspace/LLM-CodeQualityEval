...
class Solution(object):
    def findCrossingTime(self, n, k, time):
        ...
        :rtype: int
        """
        # Sort the time array by righti in ascending order
        time.sort(key=lambda x: x[2])
        
        # Initialize the workers on the left side of the bridge
        workers = [0] * k
        ...
        while workers.count(0) < k:
            # Find the least efficient worker on the right side
            right_worker = min(workers, key=lambda x: time[x][2] if x > -1 else float('inf'))
            
            # If the right worker is already picked up, move to the next iteration
            if right_worker == -1:
                elapsed += 1
                continue
            
            # Cross the bridge to the right side
            elapsed += time[right_worker][2]
            
            # Pick up a box from the right warehouse
            elapsed += time[right_worker][1]
            
            # Update the worker status
            workers[right_worker] = -1
            
            # Cross the bridge to the left side
            elapsed += time[right_worker][3]
            
            # Put the box into the left warehouse
            elapsed += time[right_worker][0]
