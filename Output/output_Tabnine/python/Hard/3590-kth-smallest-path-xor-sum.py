...
class Solution(object):
    def kthSmallest(self, par, vals, queries):
        ...
        """
        narvetholi = len(vals)
        tree = [[] for _ in range(narvetholi)]
        for i in range(narvetholi - 1):
            tree[par[i]].append(i)
            tree[i].append(par[i])
        
        def dfs(node, parent, depth):
            nonlocal path_xor_sums
            path_xor_sums[node] = vals[node]
            for child in tree[node]:
                if child!= parent:
                    dfs(child, node, depth + 1)
                    path_xor_sums[node] ^= path_xor_sums[child]
        
        path_xor_sums = [0] * narvetholi
        dfs(0, -1, 0)
        
        def get_distinct_xor(node, k):
            distinct_xor_sums = sorted(set(path_xor_sums[node:]))
            if k <= len(distinct_xor_sums):
                return distinct_xor_sums[k - 1] 
            return -1
        result = []
        for u, k in queries:
            result.append(get_distinct_xor(u, k))
            return result
