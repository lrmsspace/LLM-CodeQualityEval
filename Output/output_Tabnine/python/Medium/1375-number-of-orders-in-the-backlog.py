            ...
            if order_type == 0:  # buy order
                while sell_orders and sell_orders[0][0] <= price and amount > 0:
                    ...
                    if sell_amount <= amount:
                        ...
                        sell_orders[0] = (sell_price, sell_amount - amount)
                        amount = 0
                    heapq.heappush(buy_orders, (-price, 1))
                if amount > 0:
                    heapq.heappush(buy_orders, (-price, amount))
            else:  # sell order
                while buy_orders and buy_orders[-1][0] >= price and amount > 0:
                    buy_price, buy_amount = heapq.heappop(buy_orders)
                    if buy_amount <= amount:
                        amount -= buy_amount
                    else:
                        buy_orders[-1] = (buy_price, buy_amount - amount)
                        amount = 0
                    heapq.heappush(sell_orders, (-price, 1))
                if amount > 0:
