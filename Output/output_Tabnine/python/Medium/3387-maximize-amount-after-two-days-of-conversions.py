...
class Solution(object):
    def maxAmount(self, initialCurrency, pairs1, rates1, pairs2, rates2):
        ...
        for i in range(len(pairs1)):
            ...
            graph2[pairs1[i][1]] = [(pairs1[i][0], 1 / rates1[i])]
        
        for i in range(len(pairs2)):
            graph1[pairs2[i][0]] = graph1.get(pairs2[i][0], []) + [(pairs2[i][1], rates2[i])]
            graph2[pairs2[i][1]] = graph2.get(pairs2[i][1], []) + [(pairs2[i][0], 1 / rates2[i])]
        ...
        def dfs(node, amount):
            ...
            for neighbor, rate in graph1.get(node, []):
                dfs(neighbor, amount * rate)
            
            for neighbor, rate in graph2.get(node, []):
                dfs(neighbor, amount * rate)
