...
public:
    int mostFrequentPrime(vector<vector<int>>& mat) {
        ...
        vector<vector<vector<int>>> dp(m, vector<vector<int>>(n, vector<int>(8, 0)));
        ...
        for (int i = 0; i < m; ++i) {
            for (int j = 0; j < n; ++j) {
                for (int k = 0; k < 8; ++k) {
                    int num = mat[i][j];
                    if (k < 4) {
                        dp[i][j][k] = num * 10 + primes[k];
                    } else {
                        dp[i][j][k] = num * 100 + primes[k - 4];
                    }
                }
            }
        }
        ...
        for (int i = 0; i < m; ++i) {
            for (int j = 0; j < n; ++j) {
                for (int k = 0; k < 8; ++k) {
                    int count = countPrimeDigits(dp[i][j][k]);
                    if (count > maxCount) {
                        maxCount = count;
                        maxPrime = dp[i][j][k];
                    } else if (count == maxCount && dp[i][j][k] > maxPrime) {
                        maxPrime = dp[i][j][k];
                    }
                }
            }
