...
public:
    vector<int> minReverseOperations(int n, int p, vector<int>& banned, int k) {
        ...
        pq.push({0, p});
        while (!pq.empty()) {
            int ops = pq.top().first;
            int pos = pq.top().second;
            pq.pop();
            if (ops >= res[pos]) continue;
            res[pos] = ops;
            if (pos + k <= n - 1 && res[pos + k] == -1 && find(banned.begin(), banned.end(), pos + k) == banned.end()) {
                pq.push({ops + 1, pos + k});
            }
            if (pos - k >= 0 && res[pos - k] == -1 && find(banned.begin(), banned.end(), pos - k) == banned.end()) {
                pq.push({ops + 1, pos - k});
            }
        }
        return res;
