...
public:
    vector<int> findMedian(int n, vector<vector<int>>& edges, vector<vector<int>>& queries) {
        ...
        for (int j = 0; j < queries.size(); j++) {
            ...
            // Perform BFS to find the weighted median
            ...
            while (!q.empty()) {
                ...
                for (const auto& neighbor : adjList[curr]) {
                    ...
                    if (dist[next] == -1 || dist[curr] + weight < dist[next]) {
            ...
            int totalWeight = dist[v];
            int median = u;
            for (int node = 0; node < n; node++) {
                if (node != u && node != v && dist[node] != -1 && dist[node] <= totalWeight / 2) {
                    median = node;
                }
            }
