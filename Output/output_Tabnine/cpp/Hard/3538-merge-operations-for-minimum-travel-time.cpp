...
public:
    int minTravelTime(int l, int n, int k, vector<int>& position, vector<int>& time) {
        ...
        // Sort the position and time arrays
        sort(position.begin(), position.end());
        sort(time.begin(), time.end());
        int min_time = 0;
        int i = 0, j = 0;
        while (i < n - 1 && j < k) {
            // Calculate the distance between the current sign and the next sign
            int distance = position[i + 1] - position[i];
            
            // Calculate the time required to travel the distance
            int travel_time = distance * time[i];
            
            // Update the minimum travel time
            min_time = max(min_time, travel_time);
            
            // Merge the current sign with the next sign
            time[i + 1] += time[i];
            i++;
            j++;
    

            // Remove the current sign from the position array
            position.erase(position.begin() + i);
            n--;
    
            // Update the position array to maintain the sorted order
            for (int m = i; m < n; m++) {
                position[m] -= distance;
            }
        }
