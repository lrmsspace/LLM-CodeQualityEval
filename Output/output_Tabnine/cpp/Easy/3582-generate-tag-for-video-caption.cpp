...
public:
    string generateTag(string caption) {
        ...
        string tag = "#";
        for (string word : split(caption)) {
            tag += toLowerCase(word[0]) + toLowerCase(word.substr(1));
        }
        
        // Remove all characters that are not an English letter, except the first '#'
        tag.erase(remove_if(tag.begin(), tag.end(), [](char c) {
            return!isalpha(c) && c!= '#';
        }), tag.end());
        
        // Truncate the result to a maximum of 100 characters
        if (tag.length() > 100) {
            tag.erase(tag.begin() + 100, tag.end());
        }
