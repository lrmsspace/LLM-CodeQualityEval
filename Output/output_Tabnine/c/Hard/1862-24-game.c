...
bool judgePoint24(int* cards, int cardsSize) {
    ...
    for (int i = 0; i < cardsSize; ++i) {
        numbers.insert(cards[i]);
    }
    
    // Generate all possible combinations of numbers
    std::vector<std::vector<double>> combinations = generateCombinations(numbers);
    
    // Check if any combination results in 24
    for (const auto& combination : combinations) {
        if (canReach24(combination)) {
            return true;
        }
    }
