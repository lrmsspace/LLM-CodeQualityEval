...
Node* createTree(int* par, int* vals, int n) {
    ...
    for (int i = 1; i < n; i++) {
        int parent = par[i];
        nodes[parent].numChildren++;
        nodes[parent].children = (Node*)realloc(nodes[parent].children, nodes[parent].numChildren * sizeof(Node));
        nodes[parent].children[nodes[parent].numChildren - 1] = nodes[i];
    }
...
int* kthSmallest(int* par, int parSize, int* vals, int valsSize, int** queries, int queriesSize, int* queriesColSize, int* returnSize) {
    *returnSize = queriesSize;
    ...
    Node* root = createTree(par, vals, valsSize);

    for (int i = 0; i < queriesSize; i++) {
        int uj = queries[i][0];
        int kj = queries[i][1];

        int* distinctXors = (int*)malloc(valsSize * sizeof(int));
        int distinctXorCount = 0;

        int xorSum = 0;
        Node* currentNode = &root[uj];
        while (currentNode != NULL) {
            xorSum ^= currentNode->val;
            distinctXors[distinctXorCount++] = xorSum;

            for (int j = 0; j < currentNode->numChildren; j++) {
                Node* child = &currentNode->children[j];
                int childXorSum = xorSum ^ child->val;
                int found = 0;
                for (int k = 0; k < distinctXorCount; k++) {
                    if (distinctXors[k] == childXorSum) {
                        found = 1;
                        break;
                    }
                }
                if (!found) {
                    distinctXors[distinctXorCount++] = childXorSum;
                }
            }

            currentNode = currentNode->numChildren > 0 ? &currentNode->children[0] : NULL;
        }

        if (distinctXorCount < kj) {
            result[i] = -1;
        } else {
            int kthSmallestXor = INT_MAX;
            for (int j = 0; j < kj; j++) {
                if (distinctXors[j] < kthSmallestXor) {
                    kthSmallestXor = distinctXors[j];
                }
            }
            result[i] = kthSmallestXor;
        }
