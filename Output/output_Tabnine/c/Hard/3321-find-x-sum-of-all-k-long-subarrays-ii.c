...
long long* findXSum(int* nums, int numsSize, int k, int x, int* returnSize) {
    ...
    int maxCount = 0;
    int maxCountIndex = 0;
    int* topXIndices = (int*)calloc(x, sizeof(int));
    int topXCount = 0;
    ...
    for (int i = 0; i < numsSize; i++) {
        ...
        if (count[nums[i]] > maxCount) {
            ...
            maxCountIndex = nums[i];
        }
    }
    // Keep only the occurrences of the top x most frequent elements
    for (int i = 0; i < numsSize; i++) {
        if (count[nums[i]] == maxCount && topXCount < x) {
            topXIndices[topXCount] = nums[i];
            topXCount++;
        }
    }
    
    // Calculate the x-sum of the subarray
