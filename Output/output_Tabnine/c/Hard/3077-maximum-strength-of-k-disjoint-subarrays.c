...
long long maximumStrength(int* nums, int numsSize, int k) {
    ...
    for (int i = 0; i < numsSize - k + 1; i++) {
        for (int j = i + k - 1; j < numsSize; j++) {
            // Calculate the strength of the current subarray
            long long strength = (long long)k * prefixSums[j] - (long long)(k - 1) * prefixSums[i - 1] + (long long)(k - 2) * prefixSums[i - 2];
            // Update the result if necessary
            result = fmax(result, strength);
        }
    }
