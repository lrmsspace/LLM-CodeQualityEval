...
int rectangleArea(int** rectangles, int rectanglesSize, int* rectanglesColSize) {
    ...
    // Sort the rectangles by their left-most x-coordinate
    qsort(rectangles, n, sizeof(int*) * 4, cmp);
    ...
    for (int i = 1; i < n; ++i) {
        if (rectangles[i][0] <= right) {
            right = fmax(right, rectangles[i][2]);
            overlap = fmax(overlap, right - left);
        } else {
            sum = (sum + (right - left) * overlap) % mod;
            left = rectangles[i][0];
