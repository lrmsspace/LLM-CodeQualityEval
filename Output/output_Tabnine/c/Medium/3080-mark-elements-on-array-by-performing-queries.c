...
long long* unmarkedSumArray(int* nums, int numsSize, int** queries, int queriesSize, int* queriesColSize, int* returnSize) {
    ...
    // Initialize an array to store the sums of unmarked elements
    long long* answer = malloc(queriesSize * sizeof(long long));
    *returnSize = queriesSize;
    for (int i = 0; i < queriesSize; i++) {
        ...
        if (nums[index]!= 0) {
            nums[index] = 0;
            // Find the smallest k unmarked elements with the smallest indices
            int count = 0;
