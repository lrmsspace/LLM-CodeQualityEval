...
char* findReplaceString(char* s, int* indices, int indicesSize, char** sources, int sourcesSize, char** targets, int targetsSize) {
    ...
    for (int i = 0; i < indicesSize; i++) {
        ...
        if (strstr(result + index, source) == result + index) {
            strncpy(result + index, target, strlen(target));
            memmove(result + index + strlen(target), result + index + strlen(source), strlen(result) - index - strlen(source) + 1);
        }
    }
