...
int maxFreeTime(int eventTime, int* startTime, int startTimeSize, int* endTime, int endTimeSize) {
    ...
    // Iterate through the sorted meetings and calculate the maximum free time
    for (int i = 0; i < startTimeSize; i++) {
        int startTime = startTime[i];
        int endTime = endTime[i];
        
        // If the meeting starts after the last end time, update the maximum free time
        if (startTime > lastEndTime) {
            maxFreeTime = max(maxFreeTime, startTime - lastEndTime);
        }
    ...
    // Calculate the remaining free time at the end of the event
