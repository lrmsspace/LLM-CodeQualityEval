...
void dfs(Node* node, int* maxScore, int currentScore, int* minIncrease) {
    currentScore += node->cost;
    if (node->numNeighbors == 0) {
        ...
        for (int i = 0; i < node->numNeighbors; i++) {
            ...
            if (neighbor->cost < diff) {
                neighbor->cost += diff;
                *minIncrease = (*minIncrease < diff) ? diff : *minIncrease;
            }
            dfs(neighbor, maxScore, currentScore, minIncrease);
...
int minIncrease(int n, int** edges, int edgesSize, int* edgesColSize, int* cost, int costSize) {
    ...
    int maxScore = 0;
    int minIncrease = 0;
    dfs(&nodes[0], &maxScore, 0, &minIncrease);
