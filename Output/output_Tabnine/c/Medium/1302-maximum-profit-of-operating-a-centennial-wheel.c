...
int minOperationsMaxProfit(int* customers, int customersSize, int boardingCost, int runningCost) {
    int maxProfit = 0, totalCustomers = 0, currGondola = 0, totalRotations = 0;
    for (int i = 0; i < customersSize; i++) {
        totalCustomers += customers[i];
        while (currGondola < 4 && totalCustomers > 0) {
            currGondola++;
            totalCustomers--;
            maxProfit += currGondola * boardingCost - runningCost;
        }
        if (currGondola == 4) {
            totalRotations++;
            currGondola = 0;
    ...
    return totalRotations == 0? -1 : totalRotations;
