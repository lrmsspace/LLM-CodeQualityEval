...
int* loudAndRich(int** richer, int richerSize, int* richerColSize, int* quiet, int quietSize, int* returnSize) {
    int* res = (int*)malloc(quietSize * sizeof(int));
    memset(res, -1, quietSize * sizeof(int));
    
    int** graph = (int**)malloc(quietSize * sizeof(int*));
    for (int i = 0; i < quietSize; i++) {
        graph[i] = (int*)malloc(quietSize * sizeof(int));
        memset(graph[i], 0, quietSize * sizeof(int));
    }
    ...
    for (int k = 0; k < quietSize; k++) {
        for (int i = 0; i < quietSize; i++) {
            for (int j = 0; j < quietSize; j++) {
                if (graph[i][k] == 1 && graph[k][j] == 1 && quiet[i] > quiet[j]) {
                    graph[i][j] = 0;
                }
            }
        }
    ...
    for (int i = 0; i < quietSize; i++) {
        int minQuiet = quietSize;
        for (int j = 0; j < quietSize; j++) {
            if (graph[i][j] == 1 && quiet[j] < minQuiet) {
                minQuiet = quiet[j];
            }
        }
        res[i] = minQuiet;
