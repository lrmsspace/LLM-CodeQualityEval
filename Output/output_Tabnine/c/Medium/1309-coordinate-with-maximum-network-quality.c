...
int* bestCoordinate(int** towers, int towersSize, int* towersColSize, int radius, int* returnSize) {
    int* maxQuality = (int*)malloc(sizeof(int) * 2);
    maxQuality[0] = 0;
    maxQuality[1] = 0;
    ...
    for (int i = 0; i < towersSize; i++) {
        ...
        for (int j = 0; j < towersSize; j++) {
            ...
            if (distance <= radius * radius) {
                networkQuality += quality / (1 + sqrt(distance));
            }
        }
        if (networkQuality > maxNetworkQuality) {
            ...
            maxQuality[0] = x;
            maxQuality[1] = y;
        } else if (networkQuality == maxNetworkQuality && (x < maxQuality[0] || (x == maxQuality[0] && y < maxQuality[1]))) {
            maxQuality[0] = x;
            maxQuality[1] = y;
        }
