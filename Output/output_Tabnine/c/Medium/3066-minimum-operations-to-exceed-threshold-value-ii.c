...
int minOperations(int* nums, int numsSize, int k) {
    ...
    // Iterate through the array while the smallest element is less than or equal to k
    for (int i = 0; i < numsSize && nums[i] <= k; i++) {
        // Calculate the minimum and maximum values to be added
        int minValue = nums[i];
        int maxValue = nums[i] * 2 + nums[numsSize - 1];
        
        // Update the array by adding the minimum and maximum values
        nums[i] = minValue;
        nums[numsSize - 1] = maxValue;
