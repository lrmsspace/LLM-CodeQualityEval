    ...
    *returnSize = numberOfUsers;
    ...
    for (int i = 0; i < eventsSize; i++) {
        ...
        if (strcmp(eventType, "MESSAGE") == 0) {
            ...
            // Tokenize the mentions string
            token = strtok(mentionsString, " ");
            while (token != NULL) {
                // Check if the token represents a user id
                if (token[0] == 'i' && token[1] == 'd') {
                    int userId = atoi(&token[2]);

                    // Increment the mention count for the user
                    mentions[userId]++;
                }
