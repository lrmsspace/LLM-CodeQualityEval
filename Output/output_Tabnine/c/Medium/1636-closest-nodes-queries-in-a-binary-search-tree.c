...
int** closestNodes(struct TreeNode* root, int* queries, int queriesSize, int* returnSize, int** returnColumnSizes) {
    ...
    *returnColumnSizes = malloc(sizeof(int) * queriesSize);
    ...
    findClosestNodesHelper(root, queries[0], &left, &right);
    
    for (int i = 0; i < queriesSize; i++) {
        int leftVal = findClosestVal(left, queries[i]);
        int rightVal = findClosestVal(right, queries[i]);
        
        if (leftVal!= -1 && rightVal!= -1) {
            if (abs(leftVal - queries[i]) <= abs(rightVal - queries[i])) {
                result[i][0] = leftVal;
                result[i][1] = rightVal;
            } else {
                result[i][0] = rightVal;
