...
int numberOfRounds(char* loginTime, char* logoutTime) {
    ...
    //Count full rounds from login time to midnight
    while (loginHour < 24) {
        rounds++;
        loginHour += 15;
    }
    
    //Count full rounds from midnight to logout time
    while (logoutHour > 0) {
        rounds++;
        logoutHour -= 15;
    }
    
    //Add partial round if logout time is after login time
    if (logoutHour == 0 && logoutMinute > loginMinute) {
        rounds++;
    }
