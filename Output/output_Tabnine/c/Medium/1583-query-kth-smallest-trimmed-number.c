    ...
    *returnSize = queriesSize;
    ...
    for (int i = 0; i < queriesSize; i++) {
        ...
        for (int j = 0; j < numsSize; j++) {
            int trimmedNum = 0;
            int numLength = strlen(nums[j]);
            for (int l = 0; l < trimi && l < numLength; l++) {
                trimmedNum = trimmedNum * 10 + nums[j][numLength - 1 - l] - '0';
            }
            sortedNums[j] = trimmedNum;
        }
        
        qsort(sortedNums, numsSize, sizeof(int), compare);
        
        result[i] = sortedNums[k - 1];
