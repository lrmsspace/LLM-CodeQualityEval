...
class Solution {
    public int findCrossingTime(int n, int k, int[][] time) {
        PriorityQueue<int[]> pq = new PriorityQueue<>((a, b) -> a[0] == b[0]? a[3] - b[3] : a[0] - b[0]);
        for (int[] t : time) pq.offer(t);
        
        int left = 0, right = 0, bridge = 0, ans = 0;
        while (right < k) {
            int[] cur = pq.poll();
            if (cur[2] <= left) {
                bridge += cur[1];
