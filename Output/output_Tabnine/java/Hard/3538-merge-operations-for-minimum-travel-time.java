...
class Solution {
    public int minTravelTime(int l, int n, int k, int[] position, int[] time) {
        ...
        int j = 0;
        while (k > 0 && i < n - 1) {
            // If the current position is within the distance of the next sign
            if (position[i + 1] - position[i] <= time[i]) {
                // Merge the signs
                totalTime += position[i + 1] - position[i];
                i++;
            } else {
                // Add the time required to travel between the current and next sign
                totalTime += time[i];
            }
            k--;
        ...
        // Add the remaining time to the total travel time
