...
class Solution {
    public int connectTwoGroups(List<List<Integer>> cost) {
        ...
        // Initialize the dp array with the maximum possible cost
        ...
        // Build the dp array using the given cost matrix
        for (int i = 2; i <= size1; i++) {
            for (int j = 2; j <= size2; j++) {
                for (int k = 1; k <= i && k <= j; k++) {
                    dp[i][j] = Math.min(dp[i][j], dp[i - k][j - k] + cost.get(i - 1).get(j - 1));
                }
            }
        }
