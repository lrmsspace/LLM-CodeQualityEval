...
class Solution {
    public List<List<Long>> splitPainting(int[][] segments) {
        ...
        long left = segments[0][0];
        long right = segments[0][1];
        ...
        for (int i = 1; i < segments.length; i++) {
            ...
            // If the current segment overlaps with the previous one, update the right endpoint
            if (start <= right) {
                right = Math.max(right, end);
                currentColorSum += color;
            } else {
                painting.add(Arrays.asList(left, right, currentColorSum));
                left = start;
                right = end;
                currentColorSum = color;
            }
