...
class Solution {
    public List<String> getWordsInLongestSubsequence(String[] words, int[] groups) {
        ...
        // Sort the words based on the groups
        Map<Integer, List<String>> groupMap = new HashMap<>();
        for (int i = 0; i < n; i++) {
            groupMap.computeIfAbsent(groups[i], k -> new ArrayList<>()).add(words[i]);
        }
        List<String> sortedWords = new ArrayList<>(groupMap.values());
        ...
        // Extract the words corresponding to the longest subsequence
