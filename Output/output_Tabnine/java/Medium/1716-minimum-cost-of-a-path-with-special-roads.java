...
class Solution {
    public int minimumCost(int[] start, int[] target, int[][] specialRoads) {
        ...
        for (int[] road : specialRoads) {
            int x1 = road[0] - 1, y1 = road[1] - 1, x2 = road[2] - 1, y2 = road[3] - 1, cost = road[4];
            dp[x1][y2] = Math.min(dp[x1][y2], cost);
            dp[x2][y1] = Math.min(dp[x2][y1], cost);
        }
        for (int k = 0; k < n; k++) {
            for (int i = 0; i < n; i++) {
                for (int j = 0; j < n; j++) {
        ...
        return dp[start[0] - 1][target[0] - 1];
