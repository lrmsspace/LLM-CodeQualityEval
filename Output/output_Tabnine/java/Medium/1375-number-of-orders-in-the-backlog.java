...
class Solution {
    public int getNumberOfBacklogOrders(int[][] orders) {
        ...
        for (int[] order : orders) {
            ...
            if (type == 0) {
                while (!sellHeap.isEmpty() && sellHeap.peek()[0] <= price && amount > 0) {
                    int[] sellOrder = sellHeap.poll();
                    amount -= sellOrder[1];
                    buyHeap.offer(sellOrder);
                }
                if (amount > 0) {
                    buyHeap.offer(new int[]{price, amount});
                }
            } else {
                while (!buyHeap.isEmpty() && buyHeap.peek()[0] >= price && amount > 0) {
                    int[] buyOrder = buyHeap.poll();
                    amount -= buyOrder[1];
                    sellHeap.offer(buyOrder);
                }
                if (amount > 0) {
        ...
        while (!buyHeap.isEmpty()) {
            totalOrders += buyHeap.poll()[1];
        }
        while (!sellHeap.isEmpty()) {
            totalOrders += sellHeap.poll()[1];
        }
