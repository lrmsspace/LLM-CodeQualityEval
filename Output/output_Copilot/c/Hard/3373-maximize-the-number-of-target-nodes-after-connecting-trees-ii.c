...
int* maxTargetNodes(int** edges1, int edges1Size, int* edges1ColSize, int** edges2, int edges2Size, int* edges2ColSize, int* returnSize) {
    ...
    *returnSize = n;
    ...
    int* degree1 = (int*)calloc(n, sizeof(int));
    int* degree2 = (int*)calloc(m, sizeof(int));

    // Build degree counts for both trees
    ...
    for (int i = 0; i < n; i++) {
        answer[i] = m - degree2[0]; // Connect to the root of the second tree
        if (degree1[i] % 2 == 0) {
            answer[i] += m; // All nodes are target if even degree
        } else {
            answer[i] += m - 1; // All but one node are target if odd degree
        }
    }
