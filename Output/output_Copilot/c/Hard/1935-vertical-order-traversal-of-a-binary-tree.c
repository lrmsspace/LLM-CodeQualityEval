...
int** verticalTraversal(struct TreeNode* root, int* returnSize, int** returnColumnSizes) {
        ...
        *returnSize = 0;
        ...
        int* row = (int*)malloc(1000 * sizeof(int));
        ...
        *min = 0;
        *max = 0;
        dfs(root, col, row, val, count, min, max, 0, 0);
        for (int i = *min; i <= *max; i++) {
            result[i] = (int*)malloc(count[i] * sizeof(int));
        ...
        for (int i = 0; i < 1000; i++) {
            ...
            qsort(val + col[i], count[i], sizeof(int), cmp);
            for (int j = 0; j < count[i]; j++) {
                result[i][j] = val[col[i] + j];
            }
            (*returnSize)++;
        }
        return result;     
