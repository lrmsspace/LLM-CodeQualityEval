...
int minTravelTime(int l, int n, int k, int* position, int positionSize, int* time, int timeSize) {
    ...
    // Calculate the initial travel time without any merges
    for (int i = 0; i < n - 1; i++) {
        int distance = position[i + 1] - position[i];
        totalTravelTime += (long long)distance * time[i];
    }
    
    // If no merges are allowed, return the total travel time
    if (k == 0) {
        return (int)totalTravelTime;
    }
    
    // Create an array to store the potential savings from merging
    long long* savings = (long long*)malloc((n - 1) * sizeof(long long));
    
    // Calculate the savings for each possible merge
    for (int i = 0; i < n - 2; i++) {
        savings[i] = (long long)(position[i + 2] - position[i]) * (time[i] + time[i + 1]) - 
                      (long long)(position[i + 1] - position[i]) * time[i] - 
                      (long long)(position[i + 2] - position[i + 1]) * time[i + 1];
    }
    
    // Sort the savings in descending order
    qsort(savings, n - 1, sizeof(long long), cmp);
    
    // Apply the k largest savings
    for (int i = 0; i < k && i < n - 1; i++) {
        totalTravelTime -= savings[i];
    }
