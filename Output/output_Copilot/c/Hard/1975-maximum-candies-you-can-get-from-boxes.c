...
int maxCandies(int* status, int statusSize, int* candies, int candiesSize, int** keys, int keysSize, int* keysColSize, int** containedBoxes, int containedBoxesSize, int* containedBoxesColSize, int* initialBoxes, int initialBoxesSize) {
    ...
    int* hasKey = (int*)malloc(n * sizeof(int));
    int* hasBox = (int*)malloc(n * sizeof(int));
    ...
    while (front < rear) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            for (int j = 0; j < keysColSize[box]; j++) {
                hasKey[keys[box][j]] = 1;
                if (hasBox[keys[box][j]] == 1) {
                    open[keys[box][j]] = 1;
                    queue[rear++] = keys[box][j];
                    visited[keys[box][j]] = 1;
                }
            }
            for (int j = 0; j < containedBoxesColSize[box]; j++) {
                hasBox[containedBoxes[box][j]] = 1;
                if (hasKey[containedBoxes[box][j]] == 1) {
                    open[containedBoxes[box][j]] = 1;
                    queue[rear++] = containedBoxes[box][j];
                    visited[containedBoxes[box][j]] = 1;
                }
            }
        }
    }
