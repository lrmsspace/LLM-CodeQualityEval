...
int minWastedSpace(int* packages, int packagesSize, int** boxes, int boxesSize, int* boxesColSize) {
    ...
    for (int i = 0; i < boxesSize; i++) {
        int* box = boxes[i];
        qsort(box, boxesColSize[i], sizeof(int), cmp);
        if (box[boxesColSize[i] - 1] < packages[packagesSize - 1]) {
            continue;
        }
        long long cur = 0;
        int j = 0;
        for (int k = 0; k < boxesColSize[i]; k++) {
            j = binarySearch(packages, packagesSize, box[k], j);
            cur += box[k] * (j - k);
            if (j == packagesSize) {
                break;
            }
