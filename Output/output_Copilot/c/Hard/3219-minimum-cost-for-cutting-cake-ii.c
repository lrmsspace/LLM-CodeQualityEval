...
long long minimumCost(int m, int n, int* horizontalCut, int horizontalCutSize, int* verticalCut, int verticalCutSize) {
    ...
    int horizontalCuts = horizontalCutSize + 1; // m - 1 cuts means m pieces
    int verticalCuts = verticalCutSize + 1; // n - 1 cuts means n pieces
    ...
    int hMultiplier = 1, vMultiplier = 1;
    ...
    while (hIndex < horizontalCutSize || vIndex < verticalCutSize) {
        if (vIndex >= verticalCutSize || (hIndex < horizontalCutSize && horizontalCut[hIndex] >= verticalCut[vIndex])) {
            totalCost += (long long)horizontalCut[hIndex] * vMultiplier;
            hMultiplier++;
            hIndex++;
        } else {
            totalCost += (long long)verticalCut[vIndex] * hMultiplier;
            vMultiplier++;
            vIndex++;
        }
    }
