...
int maxProfit(int n, int* present, int presentSize, int* future, int futureSize, int** hierarchy, int hierarchySize, int* hierarchyColSize, int budget) {
    ...
    // Initialize profit and cost arrays
    for (int i = 0; i < n; i++) {
        profit[i] = future[i] - present[i];
        cost[i] = present[i];
    }

    // Adjust costs based on hierarchy
    for (int i = 0; i < hierarchySize; i++) {
        int boss = hierarchy[i][0] - 1; // Convert to 0-based index
        int employee = hierarchy[i][1] - 1; // Convert to 0-based index
        cost[employee] = present[employee] / 2; // Discounted price for employee
    }
