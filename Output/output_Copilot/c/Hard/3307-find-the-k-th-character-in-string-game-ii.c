...
char kthCharacter(long long k, int* operations, int operationsSize) {
    ...
    for (int i = 0; i < operationsSize; i++) {
        if (operations[i] == 0) {
            ...
            length = length * 2 + (length / 2); // Doubling and adding the next character
        }
    }
    ...
    for (int i = operationsSize - 1; i >= 0; i--) {
        if (operations[i] == 0) {
            ...
            if (k > length / 2) {
                k -= length / 2; // Adjust k to the second half
                k--; // Move to the next character in the second half
            }
            length = (length + 1) / 2; // Halve and adjust for next character
        }
    }

    return 'a' + (k - 1) % 26; // Return the k-th character, wrapping around alphabet    
}