...
char** getWordsInLongestSubsequence(char** words, int wordsSize, int* groups, int groupsSize, int* returnSize) {
    ...
    for (int i = 0; i < wordsSize; i++) {
        for (int j = i + 1; j < wordsSize; j++) {
            if (groups[i] != groups[j] && hammingDistance(words[i], words[j]) == 1) {
                ...
                for (int k = j + 1; k < wordsSize; k++) {
                    if (groups[lastIndex] != groups[k] && hammingDistance(words[lastIndex], words[k]) == 1) {
                        length++;
                        lastIndex = k;
                    }
                }
    ...
    for (int i = startIndex; i < wordsSize; i++) {
        if (groups[i] != groups[startIndex] && hammingDistance(words[i], words[startIndex]) == 1) {
