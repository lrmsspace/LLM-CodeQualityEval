...
int* findOrder(int numCourses, int** prerequisites, int prerequisitesSize, int* prerequisitesColSize, int* returnSize) {
    ...
    int *graphSize = (int *)malloc(sizeof(int) * numCourses);
    int *queue = (int *)malloc(sizeof(int) * numCourses);
    int front = 0, rear = 0;
    int i, j, k, course, count = 0;
    ...
    for (i = 0; i < prerequisitesSize; i++) {
        ...
        graph[prerequisites[i][1]][graphSize[prerequisites[i][1]]++] = prerequisites[i][0];
    }
    for (i = 0; i < numCourses; i++) {
        if (inDegree[i] == 0) {
            queue[rear++] = i;
    ...
    while (front < rear) {
        course = queue[front++];
        result[count++] = course;
        for (i = 0; i < graphSize[course]; i++) {
            if (--inDegree[graph[course][i]] == 0) {
                queue[rear++] = graph[course][i];
            }
        }
    }
    if (count == numCourses) {
