...
int minimumCost(int* start, int startSize, int* target, int targetSize, int** specialRoads, int specialRoadsSize, int* specialRoadsColSize) {
    ...
    dp[0] = abs(start[0] - target[0]) + abs(start[1] - target[1]);
    for (int i = 0; i < m; i++) {
        for (int j = 0; j < n; j++) {
            if ((i & (1 << j)) == 0) {
                int x1 = specialRoads[j][0];
                int y1 = specialRoads[j][1];
                int x2 = specialRoads[j][2];
                int y2 = specialRoads[j][3];
                int cost = specialRoads[j][4];
                int next = i | (1 << j);
                dp[next] = fmin(dp[next], dp[i] - abs(start[0] - x1) - abs(start[1] - y1) + cost + abs(x2 - target[0]) + abs(y2 - target[1]);
            }
        }
    }
    return dp[m - 1];     
