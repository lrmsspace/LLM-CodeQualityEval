...
int* filterRestaurants(int** restaurants, int restaurantsSize, int* restaurantsColSize, int veganFriendly, int maxPrice, int maxDistance, int* returnSize) {
    ...
    int *tmp = (int*)malloc(sizeof(int)*restaurantsSize);
    ...
    for(i = 0; i < count; i++) {
        for(j = i+1; j < count; j++) {
            if(restaurants[tmp[i]][1] < restaurants[tmp[j]][1]) {
                k = tmp[i];
                tmp[i] = tmp[j];
                tmp[j] = k;
            } else if(restaurants[tmp[i]][1] == restaurants[tmp[j]][1] && restaurants[tmp[i]][0] < restaurants[tmp[j]][0]) {
                k = tmp[i];
                tmp[i] = tmp[j];
                tmp[j] = k;
            }
