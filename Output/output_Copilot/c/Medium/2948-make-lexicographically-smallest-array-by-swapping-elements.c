...
int* lexicographicallySmallestArray(int* nums, int numsSize, int limit, int* returnSize) {
    ...
    // Sort the index map based on the values in the sorted array
    qsort(indexMap, numsSize, sizeof(int), cmpIndex);
    
    // Fill the result array with the lexicographically smallest values
    for (int i = 0; i < numsSize; i++) {
        result[i] = sorted[indexMap[i]];
    }
