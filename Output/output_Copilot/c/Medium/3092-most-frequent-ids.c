...
long long* mostFrequentIDs(int* nums, int numsSize, int* freq, int freqSize, int* returnSize) {
    ...
    *returnSize = numsSize;
    ...
    // Map to store the frequency of each ID
    unordered_map<int, long long> idCount;
    // Variable to track the maximum frequency
    long long maxFreq = 0;

    for (int i = 0; i < numsSize; i++) {
        // Update the count for the current ID
        idCount[nums[i]] += freq[i];
        
        // If the count goes below zero, reset it to zero
        if (idCount[nums[i]] < 0) {
            idCount[nums[i]] = 0;
        }
        
        // Update the maximum frequency
        maxFreq = max(maxFreq, idCount[nums[i]]);
        
        // Store the current maximum frequency in ans
        ans[i] = maxFreq;
    }
