...
int* maximumBeauty(int** items, int itemsSize, int* itemsColSize, int* queries, int queriesSize, int* returnSize) {
       int* result = (int*)malloc(sizeof(int) * queriesSize);
        int* left = (int*)malloc(sizeof(int) * itemsSize);
        int* right = (int*)malloc(sizeof(int) * itemsSize);
        left[0] = 0;
        for (int i = 1; i < itemsSize; i++) {
            left[i] = left[i - 1] + items[i - 1][1];
        }
        right[itemsSize - 1] = 0;
        for (int i = itemsSize - 2; i >= 0; i--) {
            right[i] = right[i + 1] + items[i + 1][1];
        }
        for (int i = 0; i < queriesSize; i++) {
            int max = 0;
            for (int j = 0; j < itemsSize; j++) {
                if (items[j][0] <= queries[i]) {
                    max = fmax(max, items[j][1] + left[j] + right[j]);
                }
            }
            result[i] = max;
        }
