...
int minCost(int* startPos, int startPosSize, int* homePos, int homePosSize, int* rowCosts, int rowCostsSize, int* colCosts, int colCostsSize) {
    int m = rowCostsSize, n = colCostsSize;
    int dp[m][n];
    memset(dp, 0, sizeof(dp));
    for (int i = 0; i < m; i++) {
        for (int j = 0; j < n; j++) {
            dp[i][j] = rowCosts[i] + colCosts[j];
            if (i > 0) dp[i][j] = fmin(dp[i][j], dp[i - 1][j] + rowCosts[i]);
            if (j > 0) dp[i][j] = fmin(dp[i][j], dp[i][j - 1] + colCosts[j]);
        }
    }
