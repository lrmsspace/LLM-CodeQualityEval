...
int* baseUnitConversions(int** conversions, int conversionsSize, int* conversionsColSize, int* returnSize) {
    int* baseUnitConversion = (int*)malloc(conversionsColSize[0] * sizeof(int));
    long long mod = 1e9 + 7;

    // Initialize base unit conversion for unit 0
    ...
    for (int i = 0; i < conversionsSize; i++) {
        ...
        long long conversionFactor = conversions[i][2];

        // Update the target unit conversion based on the source unit
        baseUnitConversion[targetUnit] = (baseUnitConversion[targetUnit] + baseUnitConversion[sourceUnit] * conversionFactor) % mod;
    }

    *returnSize = conversionsColSize[0];
