...
int minimumTeachings(int n, int** languages, int languagesSize, int* languagesColSize, int** friendships, int friendshipsSize, int* friendshipsColSize) {
       int m = languagesSize;
        ...
        for (int i = 0; i < friendshipsSize; i++) {
            int root1 = find(parent, friendships[i][0] - 1);
            int root2 = find(parent, friendships[i][1] - 1);
            if (root1 != root2) {
                parent[root1] = root2;
            }
        }
        ...
        int* languageCount = (int*)malloc(sizeof(int) * (n + 1));
        memset(languageCount, 0, sizeof(int) * (n + 1));
        for (int i = 0; i < m; i++) {
        ...
        int maxLanguage = 0;
        for (int i = 1; i <= n; i++) {
            maxLanguage = fmax(maxLanguage, languageCount[i]);
        }
        return maxLanguage - count[find(parent, 0)];     
}