    ...
    for (int i = 0; i < coordsSize; i++) {
        for (int j = i + 1; j < coordsSize; j++) {
            for (int k = j + 1; k < coordsSize; k++) {
                ...
                if (coords[i][0] == coords[j][0] || coords[i][0] == coords[k][0] || coords[j][0] == coords[k][0] ||
                    coords[i][1] == coords[j][1] || coords[i][1] == coords[k][1] || coords[j][1] == coords[k][1]) {
                    // Calculate the area using the determinant method
                    long long area = std::abs((coords[i][0] * (coords[j][1] - coords[k][1]) +
                                                coords[j][0] * (coords[k][1] - coords[i][1]) +
                                                coords[k][0] * (coords[i][1] - coords[j][1])) / 2);
