...
int minIncrease(int n, int** edges, int edgesSize, int* edgesColSize, int* cost, int costSize) {
    ...
    std::function<void(int, int)> dfs = [&](int node, int parent) {
        ...
        if (isLeaf) {
            leafCosts.push_back(cost[node]);
        }
    };
    dfs(0, -1);

    if (leafCosts.empty()) return 0;

    int maxCost = *std::max_element(leafCosts.begin(), leafCosts.end());
    int minIncrements = 0;
    for (int leafCost : leafCosts) {
        minIncrements += maxCost - leafCost;
    }
