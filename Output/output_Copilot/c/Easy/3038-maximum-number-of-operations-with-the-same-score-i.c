...
int maxOperations(int* nums, int numsSize) {
    int score = nums[0] + nums[1]; // Initial score from the first two elements
    int operations = 0; // Count of operations performed

    for (int i = 2; i < numsSize; i += 2) {
        if (i + 1 < numsSize && nums[i] + nums[i + 1] == score) {
            operations++; // Increment operations if the score matches
        } else {
            break; // Stop if the score does not match
        }
    }

    return operations + 1; // Add one for the first operation
