...
int countTestedDevices(int* batteryPercentages, int batteryPercentagesSize) {
    int testedCount = 0; // Initialize the count of tested devices

    for (int i = 0; i < batteryPercentagesSize; i++) {
        if (batteryPercentages[i] > 0) { // If the current device has a positive battery percentage
            testedCount++; // Increment the count of tested devices
    ...
    return testedCount; // Return the total number of tested devices
