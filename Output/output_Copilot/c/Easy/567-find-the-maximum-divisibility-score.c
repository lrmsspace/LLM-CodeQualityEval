...
int maxDivScore(int* nums, int numsSize, int* divisors, int divisorsSize) {
    int hash[1000000001] = {0}; // To track divisibility score
    int maxDivisor = 0, maxScore = 0;
    for (int i = 0; i < numsSize; i++) {
        for (int j = 0; j < divisorsSize; j++) {
            if (nums[i] % divisors[j] == 0) {
                hash[divisors[j]]++;
                if (hash[divisors[j]] > maxScore || (hash[divisors[j]] == maxScore && divisors[j] < maxDivisor)) {
                    maxScore = hash[divisors[j]];
                    maxDivisor = divisors[j];
                }
            }
        }
    }
