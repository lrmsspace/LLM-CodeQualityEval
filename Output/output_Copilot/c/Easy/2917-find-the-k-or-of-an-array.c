...
int findKOr(int* nums, int numsSize, int k) {
    int result = 0;
    int bitCount[32] = {0}; // Array to count bits at each position

    // Count the number of 1s in each bit position across all numbers
    ...
    // Determine which bits should be set in the result
    for (int j = 0; j < 32; j++) {
        if (bitCount[j] >= k) {
            result |= (1 << j); // Set the bit in the result if it meets the k condition
        }
    }
