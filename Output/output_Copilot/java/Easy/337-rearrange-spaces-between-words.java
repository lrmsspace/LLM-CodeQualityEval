...
class Solution {
    public String reorderSpaces(String text) {
        // Split the text into words and count spaces
        ...
        // If there's only one word, return it with all spaces at the end
        if (words.length == 1) {
            return words[0] + " ".repeat(spaceCount);
        }

        // Calculate spaces between words and extra spaces
        int spacesBetweenWords = spaceCount / (words.length - 1);
