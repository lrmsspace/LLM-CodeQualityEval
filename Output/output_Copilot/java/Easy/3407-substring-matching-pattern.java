...
class Solution {
    public boolean hasMatch(String s, String p) {
        ...
        // If there is no '*', we can only check for exact match
        if (starIndex == -1) {
            return s.equals(p);
        }
        
        // Split the pattern into prefix and suffix around '*'
        ...
        if (s.length() < prefix.length() + suffix.length()) {
            return false; // Not enough characters to match both prefix and suffix
        }
