...
class Solution {
    public int maxOperations(int[] nums) {
        int n = nums.length;
        if (n < 2) return 0; // Not enough elements to perform any operation

        int maxScore = nums[0] + nums[1]; // Initial score from the first two elements
        int operations = 0;

        for (int i = 2; i < n; i += 2) {
            if (i + 1 < n && nums[i] + nums[i + 1] == maxScore) {
                operations++;
            } else {
                break; // If the score does not match, we stop
            }
        }
