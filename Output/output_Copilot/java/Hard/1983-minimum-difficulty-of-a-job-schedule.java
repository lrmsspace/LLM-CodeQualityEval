...
class Solution {
    public int minDifficulty(int[] jobDifficulty, int d) {
        ...
        // dp[i][j] = minimum difficulty of scheduling first i jobs in j days
        ...
        for (int j = 1; j <= d; j++) {
            for (int i = j; i <= n; i++) { // At least j jobs for j days
                int maxDifficulty = 0;
                for (int k = i; k >= j; k--) { // Try to schedule jobs from k to i on day j
