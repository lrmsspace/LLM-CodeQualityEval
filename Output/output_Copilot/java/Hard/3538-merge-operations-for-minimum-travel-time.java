...
class Solution {
    public int minTravelTime(int l, int n, int k, int[] position, int[] time) {
        ...
        for (int i = 0; i < n - 1; i++) {
            ...
            int travelTime = dist * time[i];
            pq.offer(new int[]{i, travelTime});
        }
        ...
        for (int i = 0; i < k; i++) {
            ...
            int[] segment = pq.poll();
            int idx = segment[0];
            int mergedTime = segment[1] + (position[idx + 1] - position[idx]) * time[idx + 1];
            pq.offer(new int[]{idx, mergedTime});
        }
