...
class Solution {
    public int findCheapestPrice(int n, int[][] flights, int src, int dst, int k) {
        // Create a graph from the flights array
        Map<Integer, List<int[]>> graph = new HashMap<>();
        for (int[] flight : flights) {
            graph.computeIfAbsent(flight[0], x -> new ArrayList<>()).add(new int[]{flight[1], flight[2]});
        }

        // Priority queue to store the current city, cost, and number of stops
        ...
        while (!pq.isEmpty()) {
            ...
            // If we reach the destination city within k stops
            if (city == dst) {
                return cost;
            }

            // If we have not exceeded k stops
            if (stops <= k) {
