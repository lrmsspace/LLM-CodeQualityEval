...
class Solution {
    public double maxAmount(String initialCurrency, List<List<String>> pairs1, double[] rates1, List<List<String>> pairs2, double[] rates2) {
        ...
        for (int i = 0; i < pairs1.size(); i++) {
            ...
            day1Rates.put(target, Math.max(day1Rates.getOrDefault(target, 0.0), rates1[i]));
        }
        ...
        for (int i = 0; i < pairs2.size(); i++) {
            ...
            day2Rates.put(target, Math.max(day2Rates.getOrDefault(target, 0.0), rates2[i]));
        }

        double maxAmount = 1.0;
        if (day1Rates.containsKey(initialCurrency)) {
            maxAmount *= day1Rates.get(initialCurrency);
        }
        if (day2Rates.containsKey(initialCurrency)) {
            maxAmount *= day2Rates.get(initialCurrency);
        }
