...
class Solution {
    public int minimumLevels(int[] possible) {
        ...
        int aliceLevels = 0, bobLevels = 0;

        // Calculate total points for Bob
        for (int i = 0; i < n; i++) {
            if (possible[i] == 1) {
                bobPoints++;
            }
        ...
        // Iterate through levels to find minimum levels Alice should play
        for (int i = 0; i < n; i++) {
            if (possible[i] == 1) {
                alicePoints++;
                aliceLevels++;
            ...
            bobLevels = n - aliceLevels;

            // Check if Alice has more points than Bob
            if (alicePoints > bobPoints - bobLevels) {
                return aliceLevels;
            }
        }
