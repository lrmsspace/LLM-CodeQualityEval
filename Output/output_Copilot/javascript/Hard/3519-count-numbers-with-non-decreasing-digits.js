...
var countNumbers = function(l, r, b) {
    ...
    // Helper function to check if a number has non-decreasing digits in base b
    const hasNonDecreasingDigits = (num, base) => {
        let prev = -1;
        while (num > 0) {
    ...
    let count = 0;
    for (let i = BigInt(l); i <= BigInt(r); i++) {
        if (hasNonDecreasingDigits(i, b)) {
            count = (count + 1) % mod;
        }
    }
    return count;
