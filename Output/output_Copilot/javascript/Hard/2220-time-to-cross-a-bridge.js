...
var findCrossingTime = function(n, k, time) {
    const pq = new MinPriorityQueue({ priority: (a, b) => a[0] - b[0] });
    const pq2 = new MinPriorityQueue({ priority: (a, b) => a[0] - b[0] });
    let ans = 0;
    let i = 0;
    for (let j = 0; j < k; j++) {
        pq.enqueue([time[j][1], time[j][2], time[j][3], j]);
    }
    while (i < n) {
        if (!pq.isEmpty()) {
            const [pick, left, put, idx] = pq.dequeue().element;
            ans += pick;
            pq2.enqueue([left + put, idx]);
        }
        if (!pq2.isEmpty()) {
            const [left, idx] = pq2.dequeue().element;
            ans += left;
            i++;
        }
    }
