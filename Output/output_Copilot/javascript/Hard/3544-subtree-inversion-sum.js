...
var subtreeInversionSum = function(edges, nums, k) {
    ...
    const dfs = (node, parent) => {
        ...
        for (let neighbor of graph[node]) {
            if (neighbor !== parent) {
                let [childSum, childInversions] = dfs(neighbor, node);
                sum += childSum;
                children.push([childSum, childInversions]);
            }
        }
        children.sort((a, b) => b[0] - a[0]);
        let maxInversions = 0;
        for (let i = 0; i < Math.min(k, children.length); i++) {
            maxInversions += children[i][1] + 1;
        }
        return [sum, maxInversions];
    };

    let [totalSum, maxInversions] = dfs(0, -1);
    return totalSum + maxInversions;
};