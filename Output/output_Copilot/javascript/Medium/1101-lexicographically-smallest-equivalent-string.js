...
var smallestEquivalentString = function(s1, s2, baseStr) {
    ...
    for (let i = 0; i < s1.length; i++) {
        union(s1.charCodeAt(i) - 'a'.charCodeAt(0), s2.charCodeAt(i) - 'a'.charCodeAt(0), parent);
    }

    let result = '';
    for (const char of baseStr) {
        result += String.fromCharCode(find(char.charCodeAt(0) - 'a'.charCodeAt(0), parent) + 'a'.charCodeAt(0));
    }
