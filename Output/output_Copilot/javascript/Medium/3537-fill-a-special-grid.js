...
var specialGrid = function(n) {
    ...
    const fillQuadrant = (x, y, len, start) => {
        if (len === 1) {
            grid[x][y] = start;
            return start + 1;
        }
        const half = len / 2;
        start = fillQuadrant(x, y + half, half, start);
        start = fillQuadrant(x + half, y + half, half, start);
        start = fillQuadrant(x + half, y, half, start);
        start = fillQuadrant(x, y, half, start);
        return start;
    };

    fillQuadrant(0, 0, size, 0);
    return grid;
};