...
var assignTasks = function(servers, tasks) {
    ...
    for (let j = 0; j < m; j++) {
        ...
        while (busyServers.length && busyServers[0][0] <= time) {
            freeServers.push(busyServers.shift());
        }
        if (freeServers.length) {
            let server = freeServers.shift();
            ans[j] = server[1];
            busyServers.push([time + tasks[j], server[1]]);
            busyServers.sort((a, b) => a[0] - b[0]);
        } else {
            let server = busyServers.shift();
            ans[j] = server[1];
            busyServers.push([server[0] + tasks[j], server[1]]);
            busyServers.sort((a, b) => a[0] - b[0]);
        }
    }
    return ans;    
