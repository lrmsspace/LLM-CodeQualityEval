...
var minimumLevels = function(possible) {
    ...
    // Count the total number of levels that can be cleared
    const totalCleared = possible.reduce((sum, level) => sum + level, 0);

    // If there are no levels that can be cleared, return -1
    if (totalCleared === 0) return -1;
    ...
    // If we finish the loop and Alice never had more points than Bob
    return -1;
