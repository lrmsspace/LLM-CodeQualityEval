...
var minAbsDiff = function(grid, k) {
    ...
    for (let i = 0; i <= m - k; i++) {
        for (let j = 0; j <= n - k; j++) {
            ...
            const distinctValues = Array.from(submatrix).sort((a, b) => a - b);
            let minDiff = Infinity;

            for (let l = 1; l < distinctValues.length; l++) {
                minDiff = Math.min(minDiff, Math.abs(distinctValues[l] - distinctValues[l - 1]));
            }
