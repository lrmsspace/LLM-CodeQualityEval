...
var generateTag = function(caption) {
    ...
    // Convert to camelCase and prefix with '#'
    let tag = '#';
    for (let i = 0; i < words.length; i++) {
        if (words[i].length > 0) {
            if (i === 0) {
                tag += words[i].toLowerCase(); // First word in lowercase
            } else {
                tag += words[i][0].toUpperCase() + words[i].slice(1).toLowerCase(); // Subsequent words in camelCase
            }
        }
    }
