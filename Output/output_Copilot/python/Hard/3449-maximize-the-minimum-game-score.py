...
class Solution(object):
    def maxScore(self, points, m):
        ...
        # Initialize the gameScore array
        gameScore = [0] * n
        ...
        for i in range(n):
            ...
            for j in range(max(0, i - m), min(n, i + m + 1)):
                gameScore[j] += points[i]
            
            # Calculate the minimum value in gameScore
            current_min = min(gameScore)
            max_min_value = max(max_min_value, current_min)

            # Reset gameScore for the next iteration
            gameScore = [0] * n
