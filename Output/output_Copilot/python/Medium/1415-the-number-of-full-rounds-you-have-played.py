...
class Solution(object):
    def numberOfRounds(self, loginTime, logoutTime):
        ...
        # Convert login and logout times to minutes since midnight
        login_minutes = login_hour * 60 + login_minute
        logout_minutes = logout_hour * 60 + logout_minute

        # If logout time is earlier than login time, add 24 hours (1440 minutes)
        if logout_minutes < login_minutes:
            logout_minutes += 1440

        # Calculate the start and end of the first full round
        start_round = (login_minutes + 14) // 15 * 15
        end_round = (logout_minutes // 15) * 15
