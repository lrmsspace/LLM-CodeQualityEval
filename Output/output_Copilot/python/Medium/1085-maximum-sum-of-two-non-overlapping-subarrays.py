...
class Solution(object):
    def maxSumTwoNoOverlap(self, nums, firstLen, secondLen):
        ...
        for i in range(n - firstLen - secondLen + 1):
            first_sum = prefix_sums[i + firstLen] - prefix_sums[i]
            # Find the maximum sum of the second subarray that does not overlap with the first
            for j in range(i + firstLen, n - secondLen + 1):
                second_sum = prefix_sums[j + secondLen] - prefix_sums[j]
