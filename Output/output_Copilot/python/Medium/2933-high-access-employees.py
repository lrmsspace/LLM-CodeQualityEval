...
class Solution(object):
    def findHighAccessEmployees(self, access_times):
        ...
        # Step 1: Parse the access times and store them in a dictionary
        access_dict = defaultdict(list)
        for name, time in access_times:
            hour = int(time[:2])
            minute = int(time[2:])
            total_minutes = hour * 60 + minute
            access_dict[name].append(total_minutes)
        
        # Step 2: Check each employee's access times for high-access condition
