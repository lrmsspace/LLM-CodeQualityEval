...
class Solution(object):
    def maxOperations(self, nums):
        ...
        while i < j:
            if sorted_nums[i] + sorted_nums[j] == sorted_nums[i] + sorted_nums[j]:
                operations += min(freq[sorted_nums[i]], freq[sorted_nums[j]])
                i += 1
                j -= 1
            elif sorted_nums[i] + sorted_nums[j] < sorted_nums[i] + sorted_nums[j]:
                i += 1
            else:
                j -= 1
