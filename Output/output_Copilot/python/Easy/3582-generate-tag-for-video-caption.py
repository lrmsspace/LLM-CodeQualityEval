...
class Solution(object):
    def generateTag(self, caption):
        ...
        # Create the tag with the first word in lowercase and subsequent words in camelCase
        tag = '#' + words[0].lower() + ''.join(word.capitalize() for word in words[1:])
        
        # Remove all characters that are not English letters, except the first '#'
        tag = ''.join(c for c in tag if c.isalpha() or c == '#')
        
        # Truncate the tag to a maximum of 100 characters
        return tag[:100]
