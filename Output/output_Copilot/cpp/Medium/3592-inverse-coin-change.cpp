...
public:
    vector<int> findCoins(vector<int>& numWays) {
        ...
        // Check for each amount if it can be formed
        for (int i = 1; i < n; ++i) {
            if (numWays[i] > 0) {
                coins.push_back(i);
            }
        ...
        // Verify if the found coins can generate the numWays array
        vector<int> generatedWays(n, 0);
        generatedWays[0] = 1;  // Base case: 1 way to make 0

        for (int coin : coins) {
            for (int j = coin; j < n; ++j) {
                generatedWays[j] += generatedWays[j - coin];
            }
        }

        // Check if the generatedWays matches the numWays
        if (generatedWays == numWays) {
            return coins;
        }
