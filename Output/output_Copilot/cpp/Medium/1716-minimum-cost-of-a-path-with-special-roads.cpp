...
public:
    int minimumCost(vector<int>& start, vector<int>& target, vector<vector<int>>& specialRoads) {
        ...
        dp[n][0] = abs(target[0] - start[0]) + abs(target[1] - start[1]);
        dp[n][1] = dp[n][0];
        for (int i = n-1; i >= 0; --i) {
            dp[i][0] = min(dp[i][0], dp[i+1][0] + abs(specialRoads[i][0] - specialRoads[i][2]) + abs(specialRoads[i][1] - specialRoads[i][3]));
            dp[i][0] = min(dp[i][0], dp[i+1][1] + abs(specialRoads[i][0] - specialRoads[i][2]) + abs(specialRoads[i][1] - specialRoads[i][3]));
            dp[i][1] = min(dp[i][1], dp[i+1][0] + specialRoads[i][4]);
            dp[i][1] = min(dp[i][1], dp[i+1][1] + specialRoads[i][4]);
        }
