...
public:
    bool checkValidCuts(int n, vector<vector<int>>& rectangles) {
        unordered_set<int> horizontalCuts, verticalCuts;

        // Collect all unique horizontal and vertical cut positions
        for (const auto& rect : rectangles) {
            int startX = rect[0], startY = rect[1], endX = rect[2], endY = rect[3];
            horizontalCuts.insert(endY);
            verticalCuts.insert(endX);
        }
