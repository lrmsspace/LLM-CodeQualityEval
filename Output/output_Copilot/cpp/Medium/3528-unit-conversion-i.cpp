...
public:
    vector<int> baseUnitConversions(vector<vector<int>>& conversions) {
        ...
        vector<long long> baseUnitConversion(n, 0);
        baseUnitConversion[0] = 1;

        for (const auto& conv : conversions) {
            int sourceUnit = conv[0];
            int targetUnit = conv[1];
            long long conversionFactor = conv[2];

            baseUnitConversion[targetUnit] += baseUnitConversion[sourceUnit] * conversionFactor;
            baseUnitConversion[targetUnit] %= 1000000007;
        }

        return vector<int>(baseUnitConversion.begin(), baseUnitConversion.end());
    }
};