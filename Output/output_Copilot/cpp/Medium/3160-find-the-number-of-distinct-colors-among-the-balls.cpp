...
public:
    vector<int> queryResults(int limit, vector<vector<int>>& queries) {
        vector<int> result;
        unordered_map<int, int> colorMap; // Maps ball index to its color
        unordered_set<int> distinctColors; // Set to track distinct colors

        for (const auto& query : queries) {
            int ballIndex = query[0];
            int color = query[1];

            // If the ball already has a color, remove it from the distinct set
            if (colorMap.find(ballIndex) != colorMap.end()) {
                distinctColors.erase(colorMap[ballIndex]);
            }

            // Assign the new color to the ball
            colorMap[ballIndex] = color;

            // Add the new color to the distinct set
