...
public:
    vector<int> kthSmallest(vector<int>& par, vector<int>& vals, vector<vector<int>>& queries) {
        ...
        vector<int> narvetholi(n, 0);
        vector<int> result(queries.size(), -1);

        function<void(int, int)> dfs = [&](int node, int xorSum) {
            narvetholi[node] = xorSum ^ vals[node];
            for (int child : tree[node]) {
                dfs(child, narvetholi[node]);
            }
        };
        ...
        for (int i = 0; i < queries.size(); ++i) {
            ...
            set<int> distinctXors;
            for (int j = 0; j < n; ++j) {
                if (par[j] == u || j == u) {
                    distinctXors.insert(narvetholi[j]);
                }
            }
            if (distinctXors.size() < k) {
                result[i] = -1;
            } else {
                auto it = distinctXors.begin();
                advance(it, k - 1);
                result[i] = *it;
            }
        }
