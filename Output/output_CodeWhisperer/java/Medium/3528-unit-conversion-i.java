...
class Solution {
    public int[] baseUnitConversions(int[][] conversions) {
        ...
        for (int[] conversion : conversions) {
            graph.get(conversion[0]).add(new int[]{conversion[1], conversion[2]});
            graph.get(conversion[1]).add(new int[]{conversion[0], 1.0 / conversion[2]});
        }
        int[] ans = new int[n];
        Arrays.fill(ans, -1);
        ans[0] = 1;
        ...
        while (!queue.isEmpty()) {
            ...
            for (int[] next : graph.get(cur)) {
                if (ans[next[0]] == -1) {
                    ans[next[0]] = (int) (ans[cur] * next[1] % 1000000007);
                    queue.offer(next[0]);
                }
            }
        }
