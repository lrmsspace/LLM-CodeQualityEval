...
class Solution {
    public int[] minimumWeight(int[][] edges, int[][] queries) {
        ...
        for (int[] edge : edges) {
            ...
            map.computeIfAbsent(u, x -> new HashMap<>()).put(v, Math.min(w, map.getOrDefault(u, new HashMap<>()).getOrDefault(v, Integer.MAX_VALUE)));
            map.computeIfAbsent(v, x -> new HashMap<>()).put(u, Math.min(w, map.getOrDefault(v, new HashMap<>()).getOrDefault(u, Integer.MAX_VALUE)));
        }
        for (int i = 0; i < queries.length; i++) {
            int[] query = queries[i];
            int s = query[0], t = query[1], d = query[2];
            if (s == t) {
                ans[i] = 0;
                continue;
            }
                ...
                if (map.containsKey(node)) {
                    for (Map.Entry<Integer, Integer> entry : map.get(node).entrySet()) {
                        int next = entry.getKey(), weight = entry.getValue();
                            pq.offer(new int[]{cost + weight, next});
                    }
                }
            }
            if (ans[i] == query[2]) {
