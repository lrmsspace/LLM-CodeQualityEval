...
class Solution {
    public String abbreviateProduct(int left, int right) {
        ...
        for (int i = left; i <= right; i++) {
            ...
            while (suffix > 0 && suffix % 10 == 0) {
                suffix /= 10;
                zeros++;
            }
            if (prefix > 9999999999L) {
                return String.format("%de%d", (int)(prefix / 100000), zeros);
            }
        }
        long prefixInt = (long)(prefix);
        return String.format("%de%d", prefixInt, zeros);      
    }
}