...
public:
    int minimumCost(vector<int>& start, vector<int>& target, vector<vector<int>>& specialRoads) {
        ...
        dp[0][0] = abs(start[0]-target[0])+abs(start[1]-target[1]);
        for(int i=0; i<n; i++){
            for(int j=0; j<n; j++){
                dp[i+1][j] = min(dp[i+1][j], dp[i][j]+abs(specialRoads[i][2]-target[0])+abs(specialRoads[i][3]-target[1]));
                dp[i][j+1] = min(dp[i][j+1], dp[i][j]+abs(specialRoads[j][0]-start[0])+abs(specialRoads[j][1]-start[1]));
                dp[i+1][j+1] = min(dp[i+1][j+1], dp[i][j]+abs(specialRoads[i][2]-specialRoads[j][0])+abs(specialRoads[i][3]-specialRoads[j][1])+specialRoads[j][4]);
            }
        }
        int res = INT_MAX;
        for(int i=0; i<=n; i++){
