...
public:
    vector<int> baseUnitConversions(vector<vector<int>>& conversions) {
        vector<int> baseUnitConversion;
        int n = conversions.size() + 1;
        vector<vector<pair<int, double>>> adj(n);
        for (auto& c : conversions) {
            adj[c[0]].push_back({c[1], log(c[2])});
            adj[c[1]].push_back({c[0], -log(c[2])});
        }
        for (int i = 0; i < n; ++i) {
            ...
            int ans = 1;
            for (int j = 0; j < n; ++j) {
                ans = (int)((ans * exp(dist[j])) % 1000000007);
            }
            baseUnitConversion.push_back(ans);
        }
