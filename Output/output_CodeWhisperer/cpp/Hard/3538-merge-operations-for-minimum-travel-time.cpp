...
public:
    int minTravelTime(int l, int n, int k, vector<int>& position, vector<int>& time) {
        vector<int> dp(n, 0);
        for (int i = 1; i < n; i++) {
            dp[i] = dp[i - 1] + (position[i] - position[i - 1]) * time[i - 1];
        }
        
        for (int m = 1; m <= k; m++) {
            for (int i = n - 1; i > 0; i--) {
                int min_time = dp[i - 1] + (position[i] - position[i - 1]) * time[i - 1];
                for (int j = i - 1; j >= 0; j--) {
                    min_time = min(min_time, dp[j] + (position[i] - position[j]) * time[j]);
                    dp[i] = min(dp[i], min_time);
                }
            }
        }
        
        return dp[n - 1] + (l - position[n - 1]) * time[n - 1];
