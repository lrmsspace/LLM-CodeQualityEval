...
public:
    int minDifference(vector<int>& nums) {
        ...
        for (int i = 0; i < n; i++) {
            if (nums[i] == -1) {
                nums[i] = maxv;
                int maxv1 = INT_MIN, minv1 = INT_MAX;
                for (int j = 0; j < n; j++) {
                    if (nums[j] != -1) {
                        maxv1 = max(maxv1, nums[j]);
                        minv1 = min(minv1, nums[j]);
                    }
                }
                diff = min(diff, maxv1 - minv1);
                nums[i] = -1;
                nums[i] = minv;
                int maxv2 = INT_MIN, minv2 = INT_MAX;
                for (int j = 0; j < n; j++) {
                    if (nums[j] != -1) {
                        maxv2 = max(maxv2, nums[j]);
                        minv2 = min(minv2, nums[j]);
                    }
                }
                diff = min(diff, maxv2 - minv2);
                nums[i] = -1;
            }
        }
        return diff;
