...
class Solution {
public:
    // Find parent node in disjoint set
    ...
    vector<int> minimumWeight(vector<vector<int>>& edges, vector<vector<int>>& queries) {
        ...
        for (const auto& query : queries) {
            ...
            for (const auto& edge : edges) {
                ...
                // Add edge to current subtree
                unionSet(parent, rank, u, v);
                totalWeight += w;
                
                // Check if both sources can reach destination
                if (find(parent, src1) == find(parent, dest) && 
                    find(parent, src2) == find(parent, dest)) {
                    found = true;
                    break;
                }
            }
