...
int* baseUnitConversions(int** conversions, int conversionsSize, int* conversionsColSize, int* returnSize) {
    int* baseUnitConversion = (int*)malloc(sizeof(int) * conversionsSize + 1);
    baseUnitConversion[0] = 1;
    for (int i = 1; i < conversionsSize + 1; i++)
        baseUnitConversion[i] = -1;
    for (int i = 0; i < conversionsSize; i++) {
        if (baseUnitConversion[conversions[i][1]] == -1)
            baseUnitConversion[conversions[i][1]] = baseUnitConversion[conversions[i][0]] * conversions[i][2];
        else
            baseUnitConversion[conversions[i][1]] = fmin(baseUnitConversion[conversions[i][1]], baseUnitConversion[conversions[i][0]] * conversions[i][2]);
        for (int j = 0; j < conversionsSize + 1; j++)
            if (baseUnitConversion[j] != -1)
                baseUnitConversion[j] %= 1000000007;
    }
    *returnSize = conversionsSize + 1;
