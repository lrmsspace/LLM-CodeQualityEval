    ...
    *returnSize = m;
    ...
    for (int i = 0; i < m; i++)
    {
        for (int j = 0; j < n; j++)
        {
            int leftAbove = 0;
            int rightBelow = 0;
            int leftAboveValues[50] = { 0 };
            int rightBelowValues[50] = { 0 };
            for (int k = 1; k <= 50; k++)
            {
                if (i - k >= 0 && j - k >= 0)
                {
                    if (leftAboveValues[grid[i - k][j - k]] == 0)
                    {
                        leftAbove++;
                    }
                    leftAboveValues[grid[i - k][j - k]]++;
                }
                if (i + k < m && j + k < n)
                {
                    if (rightBelowValues[grid[i + k][j + k]] == 0)
                    {
                        rightBelow++;
                    }
                    rightBelowValues[grid[i + k][j + k]]++;
                }
            }
            answer[i][j] = abs(leftAbove - rightBelow);
        }
    }return answer;
