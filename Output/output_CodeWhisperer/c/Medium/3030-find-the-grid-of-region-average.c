...
int** resultGrid(int** image, int imageSize, int* imageColSize, int threshold, int* returnSize, int** returnColumnSizes) {
    ...
    *returnColumnSizes = (int*)malloc(m * sizeof(int));
    ...
    // For each pixel, store sum and count of regions it belongs to
    int** sums = (int**)malloc(m * sizeof(int*));
    int** counts = (int**)malloc(m * sizeof(int*));
    for(int i = 0; i < m; i++) {
        sums[i] = (int*)calloc(n, sizeof(int));
        counts[i] = (int*)calloc(n, sizeof(int));
    }
    ...
    for(int i = 0; i <= m-3; i++) {
        for(int j = 0; j <= n-3; j++) {
            ...
            if(isValid) {
                ...
                // Add to sums and counts for each pixel in region
                for(int x = i; x < i+3; x++) {
                    for(int y = j; y < j+3; y++) {
                        sums[x][y] += avg;
                        counts[x][y]++;
    ...
    for(int i = 0; i < m; i++) {
        for(int j = 0; j < n; j++) {
            if(counts[i][j] == 0) {
                ...
                result[i][j] = sums[i][j] / counts[i][j];
            }
        }
    }
