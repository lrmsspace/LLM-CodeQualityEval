...
int* countMentions(int numberOfUsers, char*** events, int eventsSize, int* eventsColSize, int* returnSize) {
    ...
    for (int i = 0; i < eventsSize; i++) {
        ...
        if (strcmp(events[i][0], "OFFLINE") == 0) {
            ...
            if (strcmp(events[i][2], "ALL") == 0) {
                continue;
            }
            char* mentions_string = events[i][2];
            char* token = strtok(mentions_string, " ");
            while (token != NULL) {
                if (strcmp(token, "HERE") == 0) {
                    for (int j = 0; j < numberOfUsers; j++) {
                        if (online[j]) {
                            mentions[j]++;
                        }
                    ...
                    mentions[atoi(token + 3)]++;
                }
                token = strtok(NULL, " ");
            }
        }
        for (int j = 0; j < numberOfUsers; j++) {
            if (!online[j] && timestamp >= atoi(events[i - 1][1]) + 60) {
                online[j] = true;
            }
        }
    }
