...
int minimumCost(int* start, int startSize, int* target, int targetSize, int** specialRoads, int specialRoadsSize, int* specialRoadsColSize) {
    int i, j, k, min, tmp, cost = 0, n = specialRoadsSize;
    ...
    for (i = 0; i < n; i++) {
        for (j = i; j >= 0; j--) {
            dp[j + 1] = min(dp[j + 1], dp[j] + abs(specialRoads[i][0] - specialRoads[j][0]) + abs(specialRoads[i][1] - specialRoads[j][1]));
            tmp = dp[j] + abs(specialRoads[i][2] - specialRoads[j][0]) + abs(specialRoads[i][3] - specialRoads[j][1]) + specialRoads[i][4];
            dp[j + 1] = min(dp[j + 1], tmp);
        }
        cost = min(cost + abs(specialRoads[i][0] - target[0]) + abs(specialRoads[i][1] - target[1]), dp[i + 1] + abs(target[0] - specialRoads[i][2]) + abs(target[1] - specialRoads[i][3]));
    }
