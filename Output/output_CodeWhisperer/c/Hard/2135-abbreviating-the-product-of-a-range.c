...
char* abbreviateProduct(int left, int right) {
    ...
    for (int i = left; i <= right; i++) {
        ...
        suffix = (suffix * j) % 10000000000;
        prefix = (prefix * i) % 10000000000;
    }
    if (prefix < 10000000000) {
        char* result = (char*)malloc(20 * sizeof(char));
        sprintf(result, "%llde%d", (long long)prefix, zeros);
        return result;
    }
    char* result = (char*)malloc(30 * sizeof(char));
    sprintf(result, "%lld...%05llde%d", (long long)(prefix / suffix / 100000), (long long)(suffix % 100000), zeros);
    return result;
}