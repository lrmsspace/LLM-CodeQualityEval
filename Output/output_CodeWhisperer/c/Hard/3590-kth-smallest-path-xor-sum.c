...
int* kthSmallest(int* par, int parSize, int* vals, int valsSize, int** queries, int queriesSize, int* queriesColSize, int* returnSize) {
    int* narvetholi = (int*)malloc(sizeof(int) * parSize);
    int* narvetholi2 = (int*)malloc(sizeof(int) * parSize);
    ...
    for (int i = 0; i < queriesSize; i++) {
        int u = queries[i][0];
        int k = queries[i][1];
        int* distinctXORs = (int*)malloc(sizeof(int) * valsSize);
        int distinctXORsSize = 0;

        for (int j = 0; j < valsSize; j++) {
            int node = j;
            int pathXOR = vals[node];
            ...
            if (node == u) {
                int found = 0;
                for (int l = 0; l < distinctXORsSize; l++) {
                    if (distinctXORs[l] == pathXOR) {
                        found = 1;
                        break;
                    }
                }
                if (!found) {
                    distinctXORs[distinctXORsSize++] = pathXOR;
                }
            }
        }

        if (distinctXORsSize < k) {
            result[i] = -1;
        } else {
            for (int j = 0; j < distinctXORsSize - 1; j++) {
                for (int l = 0; l < distinctXORsSize - j - 1; l++) {
                    if (distinctXORs[l] > distinctXORs[l + 1]) {
                        int temp = distinctXORs[l];
                        distinctXORs[l] = distinctXORs[l + 1];
                        distinctXORs[l + 1] = temp;
                    }
                }
            }
