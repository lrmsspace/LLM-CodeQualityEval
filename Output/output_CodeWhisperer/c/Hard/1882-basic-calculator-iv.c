...
char** basicCalculatorIV(char* expression, char** evalvars, int evalvarsSize, int* evalints, int evalintsSize, int* returnSize) {
    ...
    int* result = (int*)malloc(sizeof(int) * (top + 1));
    ...
    for (int i = 0; i <= top; i++) {
        ...
        if (stack[i] == '+' || stack[i] == '-' || stack[i] == '*' || stack[i] == '/') {
            ...
            int a = result[resultTop--];

            int b = result[resultTop--];

            if (stack[i] == '+') {

                result[++resultTop] = b + a;
            ...
            result[++resultTop] = stack[i];

        }

    }
    *returnSize = resultTop + 1;
