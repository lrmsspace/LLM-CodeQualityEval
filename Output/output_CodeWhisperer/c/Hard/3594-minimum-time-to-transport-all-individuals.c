...
double minTime(int n, int k, int m, int* time, int timeSize, double* mul, int mulSize) {
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < k; j++) {
            int max_time = 0;
            for (int l = i; l <= j; l++) {
                max_time = fmax(max_time, time[l]);
            }
            int stage = (i + j + 1) % m;
            int return_time = (int)(max_time * mul[stage]);
            int next_stage = (stage + floor(return_time)) % m;
            int next_time = (int)(max_time * mul[next_stage]);
            dp[j + 1] = fmin(dp[j + 1], dp[j] + fmax(max_time * mul[stage], next_time));
        }
    }

    double result = (double)dp[k] / 1.00;
