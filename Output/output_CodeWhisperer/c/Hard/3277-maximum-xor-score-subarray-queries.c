...
int* maximumSubarrayXor(int* nums, int numsSize, int** queries, int queriesSize, int* queriesColSize, int* returnSize) {
    ...
    *returnSize = queriesSize;
    ...
    for(int q = 0; q < queriesSize; q++) {
        ...
        // Try all possible subarrays in range [left, right]
        for(int i = left; i <= right; i++) {
            for(int j = i; j <= right; j++) {
                // Calculate XOR score for subarray [i,j]
                int len = j - i + 1;
                ...
                // Calculate XOR score by repeatedly applying operations
                while(len > 1) {
                ...
                // Update max score if current score is larger
                if(temp[0] > maxScore) {
                    maxScore = temp[0];
                }
