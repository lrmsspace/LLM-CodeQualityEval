...
char* countOfAtoms(char* formula) {
    ...
    while (i < len) {
        if (formula[i] == '(') {
            ...
            int num = 0;
            if (j + 1 < len && isdigit(formula[j + 1])) {
                num = atoi(formula + j + 1);
            }
            int* temp = (int*)calloc(26, sizeof(int));
            countOfAtoms(formula + i + 1, temp);
            for (int k = 0; k < 26; k++) {
                count[k] += temp[k] * (num == 0 ? 1 : num);
            }
            free(temp);
            i = j + 1 + (num == 0 ? 0 : (int)log10(num) + 1);
        }
        else {
            int j = i + 1;
            ...
            count[formula[i] - 'A'] += (num == 0 ? 1 : num);
            i = j + (num == 0 ? 0 : (int)log10(num) + 1);
        }
    }
