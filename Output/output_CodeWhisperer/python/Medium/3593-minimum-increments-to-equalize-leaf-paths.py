...
class Solution(object):
    def minIncrease(self, n, edges, cost):
        ...
        # Perform DFS to find the minimum increase
        def dfs(node, parent):
            nonlocal min_increase
            increase = 0
            for child in tree[node]:
                if child != parent:
                    child_increase = dfs(child, node)
                    if child_increase > 0:
                        increase = max(increase, child_increase)
            increase = max(increase, cost[node])
            min_increase += increase - cost[node]
            return increase
