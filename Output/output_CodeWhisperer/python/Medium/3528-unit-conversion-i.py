        ...
        def dfs(node, visited):
            ...
            for neighbor, weight in graph[node]:
                if neighbor not in visited:
                    baseUnitConversion[neighbor] = baseUnitConversion[node] * weight
                    dfs(neighbor, visited)

        baseUnitConversion = [1.0] * n
        dfs(0, set())
        return [int(x / baseUnitConversion[0] % (10**9 + 7)) for x in baseUnitConversion]        