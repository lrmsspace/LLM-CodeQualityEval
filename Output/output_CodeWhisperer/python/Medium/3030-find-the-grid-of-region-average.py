...
class Solution(object):
    def resultGrid(self, image, threshold):
        ...
        def is_region(i, j, x, y):
            return all(abs(image[i][j] - image[x][y]) <= threshold for x in range(i, i + 3) for y in range(j, j + 3) if is_adjacent(i, j, x, y))

        for i in range(m - 2):
            for j in range(n - 2):
                if is_region(i, j, i + 2, j + 2):
                    for x in range(i, i + 3):
                        for y in range(j, j + 3):
                            region_sum[x][y] += image[i][j]
                            region_count[x][y] += 1
