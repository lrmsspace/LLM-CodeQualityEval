    ...
    def treeQueries(self, n, edges, queries):
        ...
        def dfs(node, parent, depth):
            ...
            for child, weight in graph[node]:
                if child != parent:
                    child_depth = dfs(child, node, depth + 1)
                    if child_depth > max_depth:
                        max_depth_second = max_depth
                        max_depth = child_depth
                    elif child_depth > max_depth_second:
                        max_depth_second = child_depth
            result[node] = max_depth_second
        ...
        for query in queries:
            if len(query) == 2:
                answer.append(result[query[1]])
            else:
                u, v, w, _ = query
                result[u] = max(result[u], result[v] + w)
                result[v] = max(result[v], result[u] + w)
