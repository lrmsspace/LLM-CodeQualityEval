...
class Solution(object):
    def countKSubsequencesWithMaxBeauty(self, s, k):
        ...
        :rtype: int
        """
        # Step 1: Calculate the frequency of each character in the string s
        freq = {}
        for c in s:
            if c not in freq:
                freq[c] = 1
        ...
        # Step 3: Count the number of k-subsequences with maximum beauty
        count = 0
        for i in range(len(sorted_chars)):
            if i < k:
                count += freq[sorted_chars[i]]
            else:
                count += min(freq[sorted_chars[i]], k)
