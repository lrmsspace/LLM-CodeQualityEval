...
class Solution(object):
    def concatenatedDivisibility(self, nums, k):
        ...
        :rtype: List[int]
        """
        def is_divisible_concatenation(perm):
            concatenated_value = int(''.join(map(str, perm)))
            return concatenated_value % k == 0

        def lexicographically_smallest_divisible_concatenation(nums):
            n = len(nums)
            for perm in itertools.permutations(nums):
                if is_divisible_concatenation(perm):
                    return list(perm)
            return []

        return lexicographically_smallest_divisible_concatenation(nums)
