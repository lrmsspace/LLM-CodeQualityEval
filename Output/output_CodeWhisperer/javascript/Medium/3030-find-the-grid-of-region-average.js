...
var resultGrid = function(image, threshold) {
    ...
    const regionSums = Array.from({ length: m }, () => Array(n).fill(0));
    const regionCounts = Array.from({ length: m }, () => Array(n).fill(0));
    ...
    for (let i = 0; i < m; i++) {
        for (let j = 0; j < n; j++) {
            result[i][j] = regionCounts[i][j] > 0 ? Math.floor(regionSums[i][j] / regionCounts[i][j]) : image[i][j];
        }
    }
