...
var baseUnitConversions = function(conversions) {
    ...
    for(const [source, target, factor] of conversions) graph.get(source).push([target, factor])
    const visited = new Array(n).fill(false)
    const result = new Array(n).fill(0)
    const dfs = (node, factor) => {
        visited[node] = true
        result[node] = factor
        for(const [next, nextFactor] of graph.get(node)) {
            if(!visited[next]) dfs(next, factor * nextFactor)
        }
    }
    dfs(0, 1)
    return result.map(x => x % (1e9 + 7))    
};