...
var findCrossingTime = function(n, k, time) {
    let left = new PriorityQueue((a, b) => a[0] > b[0] || (a[0] == b[0] && a[1] > b[1]));
    let right = new PriorityQueue((a, b) => a[0] > b[0] || (a[0] == b[0] && a[1] > b[1]));
    let work = new PriorityQueue((a, b) => a[0] > b[0] || (a[0] == b[0] && a[1] > b[1]));
    for (let i = 0; i < k; i++) {
        left.push([0, i]);
    }
    let res = 0;
    while (n > 0) {
        while (work.size() > 0 && work.top()[0] <= res) {
            let [_, i] = work.pop();
            if (time[i][2] > time[i][3]) {
                left.push([res + time[i][3], i]);
            } else {
                right.push([res + time[i][2], i]);
                n--;
            }
        }
        if (right.size() > 0) {
            let [_, i] = right.pop();
            work.push([res + time[i][0] + time[i][1], i]);
        } else {
            let [_, i] = left.pop();
            res = left.top()[0];
            work.push([res + time[i][0], i]);
        }
    }
    while (work.size() > 0) {
        res = work.top()[0];
