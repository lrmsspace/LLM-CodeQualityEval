...
var maximumCount = function(nums, queries) {
    ...
    for (let i = 0; i < m; i++) {
        const [idx, val] = queries[i];
        nums[idx] = val;
        const prefix = nums.slice(0, idx);
        const suffix = nums.slice(idx);
        answer[i] = Math.max(countDistinctPrimes(prefix), countDistinctPrimes(suffix));
    }
