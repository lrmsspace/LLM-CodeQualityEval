...
var kthSmallest = function(par, vals, queries) {
    const n = vals.length;
    const kthSmallestPathXorSum = (node, k) => {
        const pathXorSums = [];
        const dfs = (node, xorSum) => {
            xorSum ^= vals[node];
            pathXorSums.push(xorSum);
            for (let child of narvetholi[node]) {
                dfs(child, xorSum);
            }
        };
        dfs(node, 0);
        pathXorSums.sort((a, b) => a - b);
        return pathXorSums[k - 1] || -1;
    };
    
    narvetholi = Array.from({ length: n }, () => []);
    for (let i = 1; i < n; i++) {
        narvetholi[par[i]].push(i);
    }
    
    const results = [];
    for (let [node, k] of queries) {
        results.push(kthSmallestPathXorSum(node, k));
    }
