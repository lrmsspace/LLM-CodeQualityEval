...
var minInterval = function(intervals, queries) {
    ...
    const pq = new MinPriorityQueue({ priority: (x) => x[1] - x[0] + 1 });
    let i = 0;
    for (const q of sortedQueries) {
        while (i < sortedIntervals.length && sortedIntervals[i][0] <= q) {
            pq.enqueue(sortedIntervals[i]);
            i++;
        }
        while (pq.size() && pq.front().element[1] < q) {
            pq.dequeue();
        }
        if (pq.size()) {
            res[queries.indexOf(q)] = pq.front().element[1] - pq.front().element[0] + 1;
        }
    }
    return res;
