...
var maxBuilding = function(n, restrictions) {
    ...
    let max = 0;
    for (let i = 1; i < restrictions.length; i++) {
        let mid = (restrictions[i][0] + restrictions[i - 1][0]) / 2;
        let diff = Math.abs(restrictions[i][0] - restrictions[i - 1][0]);
        let height = Math.min(restrictions[i][1], restrictions[i - 1][1]) + diff / 2;
        max = Math.max(max, Math.floor(height));
    }
    max = Math.max(max, restrictions[restrictions.length - 1][1] + n - restrictions[restrictions.length - 1][0]);
    return max;
};