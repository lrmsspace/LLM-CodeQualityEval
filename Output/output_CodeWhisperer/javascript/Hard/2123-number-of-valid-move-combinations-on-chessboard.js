...
var countCombinations = function(pieces, positions) {
    ...
    const limit = (piece, x, y) => {
        if (piece === 'rook') return x === 0 || y === 0;
        if (piece === 'bishop') return x === y || x === 0 || y === 0;
        return true;
    };
    const dfs = (i, state) => {
        if (i === n) return 1;
        ...
        for (let [dx, dy] of dirs) {
            for (let j = 1; j <= 8; j++) {
                ...
                if (!limit(pieces[i], Math.abs(dx * j), Math.abs(dy * j))) break;
                if (state & (1 << (x * 8 + y - 1))) continue;
                res += dfs(i + 1, state | (1 << (x * 8 + y - 1)));
            }
        }
        return res;
    };
