...
var mostPopularCreator = function(creators, ids, views) {
    ...
    for(let i=0;i<creators.length;i++){
        let curr = map.get(creators[i]);
        if(curr){
            curr[0]+=views[i];
            curr[1]=Math.max(curr[1],views[i]);
            curr[2].push([ids[i],views[i]]);
        }else{
            map.set(creators[i],[views[i],views[i],[[ids[i],views[i]]]]);
        }
    }
    let ans=[];
    let max=0;
