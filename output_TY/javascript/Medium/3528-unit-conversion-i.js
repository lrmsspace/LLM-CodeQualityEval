...
var baseUnitConversions = function(conversions) {
    ...
    for (let [source, target, factor] of conversions) {
    ...
    while (q.length) {
        let node = q.shift();
        for (let [nei, factor] of graph[node]) {
            if (dist[nei] > dist[node] * factor) {
                dist[nei] = dist[node] * factor;
                q.push(nei);
            }
        }
    }
    return dist.slice(1).map(x => x <= k ? 1 : 0);
