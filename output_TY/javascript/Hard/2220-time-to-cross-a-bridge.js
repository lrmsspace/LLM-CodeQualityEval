...
var findCrossingTime = function(n, k, time) {
    ...
    var right_time = [];
    var left_time_sum = 0;
    var right_time_sum = 0;
    var left_time_sum_max = 0;
    var right_time_sum_max = 0;
    var left_time_sum_min = 0;
    var right_time_sum_min = 0;
    var left_time_sum_min_index = 0;
    var right_time_sum_min_index = 0;
    var left_time_sum_max_index = 0;
   var right_time_sum_max_index = 0;
    for (var i = 0; i < batteries.length; i++) {
        if (i % 2 == 0) {
            left.push(batteries[i]);
            left_time.push(batteries[i]);
            left_time_sum += batteries[i];
        } else {
            right.push(batteries[i]);
            right_time.push(batteries[i]);
            right_time_sum += batteries[i];
        }
    }
    left_time.sort(function (a, b) {
        return b - a;
    });
    right_time.sort(function (a, b) {
        return b - a;
    });
    while (left_time_sum > target) {
        if (left_time.length == 0) {
            return -1;
        }
        let temp = left_time.pop();
        left_time_sum -= temp;
        right.push(temp);
        right_time.push(temp);
        right_time_sum += temp;
        right_time.sort(function (a, b) {
            return b - a;
        });
    }
    while (right_time_sum > target) {
        if (right.length == 0) {
            return -1;
        }
        let temp = right.pop();
    ...
    return left_time_sum / left_time.length;
};