...
var findMedian = function(n, edges, queries) {
	...
	for (let i = 1; i < n; i++) {
		count[i] = count[i - 1] + tree[i].length;
	}
	let sorted = [...weights].sort((a, b) => a - b);
	let ans = new Array(queries.length).fill(0);
	for (let i = 0; i < queries.length; i++) {
		let [u, v] = queries[i];
		let left = count[u - 1] + count[v - 1];
		let right = count[n - 1] - left;
		let leftOdd = Math.floor(left / 2);
		let rightOdd = Math.floor(right / 2);
		let leftEven = left - leftOdd;
		let rightEven = right - rightOdd;
		let leftSum = sorted[leftOdd] * 2 + sorted[leftEven];
		let rightSum = sorted[rightOdd] * 2 + sorted[rightEven];
