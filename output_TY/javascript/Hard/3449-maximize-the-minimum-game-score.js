...
var maxScore = function(points, m) {
    let n = points.length;
    let gameScore = new Array(n).fill(0).map(() => new Array(m).fill(0));
    for (let i = 0; i < n; i++) {
        gameScore[i][1] = points[i];
    }
    for (let i = 1; i < n; i++) {
        for (let j = 2; j <= m; j++) {
            let maxScore = 0;
            for (let k = 0; k < i; k++) {
                maxScore = Math.max(maxScore, gameScore[k][j - 1] + points[i] - points[k]);
            }
            gameScore[i][j] = maxScore;
        }
    }
