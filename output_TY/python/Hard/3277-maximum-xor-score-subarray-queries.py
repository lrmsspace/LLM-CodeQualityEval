...
class Solution(object):
    def maximumSubarrayXor(self, nums, queries):
        ...
        # 预处理 dp[i][j] 表示从 i 到 j 的 XOR Score
        ...
        # 构建每个 [l, r] 区间内的最大 XOR Score
        max_xor_scores = [0] * n
        from collections import defaultdict
        interval_map = defaultdict(dict)  # interval_map[l][r] = max_xor
        
        for l in range(n):
            max_xor = float('-inf')
            for r in range(l, n):
                max_xor = max(max_xor, dp[l][r])
                interval_map[l][r] = max_xor
        
        # 回答查询
        answer = []
        for l, r in queries:
            answer.append(interval_map[l][r])
