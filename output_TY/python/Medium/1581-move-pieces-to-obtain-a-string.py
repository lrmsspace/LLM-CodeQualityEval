...
class Solution(object):
    def canChange(self, start, target):
        ...
        if len(start_L)!=len(target_L) or len(start_R)!=len(target_R): return False
        for i in range(len(start_L)):
            if start_L[i]>target_L[i] or start_L[i]<target_L[i]-1: return False
        for i in range(len(start_R)):
            if start_R[i]<target_R[i] or start_R[i]>target_R[i]+1: return False
        return True
