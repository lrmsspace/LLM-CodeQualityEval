...
bool* distanceLimitedPathsExist(int n, int** edgeList, int edgeListSize, int* edgeListColSize, int** queries, int queriesSize, int* queriesColSize, int* returnSize){
    ...
    int *dis = malloc(edgeListSize * sizeof(int));
    for (int i = 0; i < edgeListSize; i++) dis[i] = edgeList[i][2];
    qsort(dis, edgeListSize, sizeof(int), cmp);
    int *index = malloc(edgeListSize * sizeof(int));
    for (int i = 0; i < edgeListSize; i++) index[i] = i;
    for (int i = 0; i < queriesSize; i++) {
        int p = queries[i][0], q = queries[i][1], limit = queries[i][2];
        while (index[i] < edgeListSize && dis[index[i]] < limit) {
            int u = edgeList[index[i]][0], v = edgeList[index[i]][1];
