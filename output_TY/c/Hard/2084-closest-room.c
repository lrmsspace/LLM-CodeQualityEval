...
int* closestRoom(int** rooms, int roomsSize, int* roomsColSize, int** queries, int queriesSize, int* queriesColSize, int* returnSize){
    ...
    *returnSize = queriesSize;
    ...
    int *roomId = (int*)malloc(sizeof(int) * roomsSize);
    int *roomSize = (int*)malloc(sizeof(int) * roomsSize);
    ...
    for(i = 0; i < queriesSize; i++){
        ...
        int minDiff = INT_MAX;
        int minRoomId = -1;
        for(j = 0; j < roomsSize; j++){
            int room = roomId[j];
            int size = roomSize[j];
            if(size >= minSize){
                int diff = abs(room - preferred);
                if(diff < minDiff || (diff == minDiff && room < minRoomId)){
                    minDiff = diff;
                    minRoomId = room;
                }
            }
