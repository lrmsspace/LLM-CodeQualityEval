...
int* minInterval(int** intervals, int intervalsSize, int* intervalsColSize, int* queries, int queriesSize, int* returnSize){
    ...
    int *s = (int *)malloc(sizeof(int) * (intervalsSize + queriesSize));
    for (int i = 0; i < intervalsSize; i++) {
        s[i] = intervals[i][0];
        s[i + queriesSize] = intervals[i][1];
    }
    for (int i = 0; i < queriesSize; i++) {
        s[i + intervalsSize] = q[i];
    }
    qsort(s, intervalsSize + queriesSize, sizeof(int), cmp);
    for (int i = 0; i < queriesSize; i++) {
        res[i] = -1;
        for (int j = 0; j < intervalsSize; j++) {
            if (s[j] <= q[i] && q[i] <= s[j + intervalsSize]) {
                res[i] = s[j + intervalsSize] - s[j] + 1;
                break;
            }
        }
    }
