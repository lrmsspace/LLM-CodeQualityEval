...
int* maximumCount(int* nums, int numsSize, int** queries, int queriesSize, int* queriesColSize, int* returnSize) {
    int i, j, k, n, m, *a, *b, *c, *d, *e, *f, *g, *h, *r, *s, *t, *u, *v, *w, *x, *y, *z;
    a = (int*)malloc(sizeof(int) * numsSize);
    for (i = 0; i < numsSize; i++) {
        a[i] = nums[i] < 0 ? -1 : nums[i] > 0 ? 1 : 0;
    }
    b = (int*)malloc(sizeof(int) * queriesSize);
    for (i = 0; i < queriesSize; i++) {
        b[i] = queries[i][0] < 0 ? -1 : queries[i][0] > 0 ? 1 : 0;
    }
    c = (int*)malloc(sizeof(int) * queriesSize);
    for (i = 0; i < queriesSize; i++) {
        c[i] = queries[i][1] < 0 ? -1 : queries[i][1] > 0 ? 1 : 0;
    }
    for (i = 0; i < queriesSize; i++) {
