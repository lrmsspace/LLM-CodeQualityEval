...
public:
    int minimumCost(vector<int>& start, vector<int>& target, vector<vector<int>>& specialRoads) {
        ...
        while (!pq.empty()) {
            ...
            if (pos == (1 << n) - 1) return cost;
            for (int i = 0; i < n; i++) {
                if (pos & (1 << i)) continue;
                int x1 = specialRoads[i][0], y1 = specialRoads[i][1], x2 = specialRoads[i][2], y2 = specialRoads[i][3], costi = specialRoads[i][4];
                int newCost = cost + costi + abs(x1 - x2) + abs(y1 - y2);
                pq.push({newCost, pos | (1 << i)});
            }
            int newCost = cost + abs(start[0] - target[0]) + abs(start[1] - target[1]);
            pq.push({newCost, pos});
        }
        return -1;
    }
