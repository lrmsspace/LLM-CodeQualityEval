...
class Solution(object):
    def minimumCost(self, start, target, specialRoads):
        ...
        # 初始化最小成本为起点到终点的曼哈顿距离
        min_cost = manhattan_distance(start[0], start[1], target[0], target[1])

        # 遍历所有特殊道路
        for road in specialRoads:
            x1, y1, x2, y2, cost = road
            # 计算起点到特殊道路起点的曼哈顿距离
            cost_to_road_start = manhattan_distance(start[0], start[1], x1, y1)
            # 计算特殊道路终点到终点的曼哈顿距离
            cost_to_target = manhattan_distance(x2, y2, target[0], target[1])
            # 更新最小成本
            min_cost = min(min_cost, cost_to_road_start + cost + cost_to_target)
