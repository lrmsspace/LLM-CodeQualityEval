...
class Solution(object):
    def countKSubsequencesWithMaxBeauty(self, s, k):
        ...
        # Count how many times the k-th most frequent character appears
        kth_freq = sorted_freq[k - 1]
        
        # Count how many characters have frequency equal to kth_freq
        count_kth_freq = sum(1 for f in sorted_freq if f == kth_freq)
        
        # Calculate the number of ways to choose characters with frequency >= kth_freq
        ways = comb(count_kth_freq, sorted_freq[:k].count(kth_freq))
