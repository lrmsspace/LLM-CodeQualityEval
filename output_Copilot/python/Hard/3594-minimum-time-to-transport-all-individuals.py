...
class Solution(object):
    def minTime(self, n, k, m, time, mul):
        ...
        # Function to calculate the total time for a given group and stage
        def calculate_time(group, stage):
            max_time = max(time[i] for i in group)
            return max_time * mul[stage]

        # Function to transport individuals
        def transport(individuals, current_stage):
            if not individuals:
                return 0.0
            ...
            while len(individuals) > 0:
                ...
                # Calculate crossing time for the group
                crossing_time = calculate_time(group, current_stage)
                total_time += crossing_time
                
                # Update the current stage
                current_stage = (current_stage + floor(crossing_time)) % m
                
                # If there are still individuals left, one must return
                if individuals:
                    returning_person = group[0]
                    returning_time = time[returning_person] * mul[current_stage]
                    total_time += returning_time
                    current_stage = (current_stage + floor(returning_time)) % m

            return total_time

        # Try all combinations of individuals
        min_time = float('inf')
        for combo in combinations(range(n), k):
            individuals = list(combo)
            current_stage = 0
            total_time = transport(individuals, current_stage)
