...
class Solution(object):
    def kthSmallest(self, par, vals, queries):
        ...
        # To store the path XOR sums for each node
        path_xor = [0] * n
        ...
        for u, k in queries:
            ...
            while queue:
                node = queue.popleft()
                subtree_xors.append(path_xor[node])
                for neighbor in graph[node]:
                    queue.append(neighbor)

            # Get distinct path XORs and sort them
            distinct_xors = sorted(set(subtree_xors))
