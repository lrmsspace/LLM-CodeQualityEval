...
int* loudAndRich(int** richer, int richerSize, int* richerColSize, int* quiet, int quietSize, int* returnSize) {
    int* res = (int*)malloc(sizeof(int) * quietSize);
    *returnSize = quietSize;
    int* graph[quietSize];
    int* outdegree = (int*)malloc(sizeof(int) * quietSize);
    int* indegree = (int*)malloc(sizeof(int) * quietSize);
    ...
    for (int i = 0; i < richerSize; i++) {
        graph[richer[i][1]][outdegree[richer[i][1]]++] = richer[i][0];
        indegree[richer[i][0]]++;
    }
    for (int i = 0; i < quietSize; i++) {
        res[i] = -1;
