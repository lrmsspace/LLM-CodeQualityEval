 ...
 */
        ...
        char** watchedVideosByFriends(char*** watchedVideos, int watchedVideosSize, int* watchedVideosColSize, int** friends, int friendsSize, int* friendsColSize, int id, int level, int* returnSize) {
            bool visited[watchedVideosSize];
            ...
            int videoCount[watchedVideosSize * 100] = {0};
            char* videoNames[watchedVideosSize * 100];
            int videoIndex = 0;
            ...
            for (int i = front; i < rear; i++) {
                ...
                for (int j = 0; j < watchedVideosColSize[friendId]; j++) {
                    ...
                    bool found = false;
                    for (int k = 0; k < videoIndex; k++) {
                        if (strcmp(videoNames[k], video) == 0) {
                            videoCount[k]++;
                            found = true;
                            break;
                        }
                    ...
                    if (!found) {
                        videoNames[videoIndex] = video;
                        videoCount[videoIndex++] = 1;
                    }
                }
            }

            VideoCount videoCounts[videoIndex];
            for (int i = 0; i < videoIndex; i++) {
                videoCounts[i].video = videoNames[i];
                videoCounts[i].count = videoCount[i];
            }

            qsort(videoCounts, videoIndex, sizeof(VideoCount), compare);

            char** result = (char**)malloc(videoIndex * sizeof(char*));
            *returnSize = videoIndex;
            for (int i = 0; i < videoIndex; i++) {
