...
char** findHighAccessEmployees(char*** access_times, int access_timesSize, int* access_timesColSize, int* returnSize) {
    ...
    // Key: employee name, Value: list of access times
    std::unordered_map<std::string, std::vector<int>> accessMap;

    for (int i = 0; i < access_timesSize; i++) {
        std::string employee = access_times[i][0];
        int accessTime = std::stoi(access_times[i][1]);
        accessMap[employee].push_back(accessTime);
    }

    // Find high-access employees
    std::vector<std::string> highAccessEmployees;
    for (const auto& entry : accessMap) {
        const std::string& employee = entry.first;
        const std::vector<int>& times = entry.second;

        // Check for high access within one-hour periods
    ...
    *returnSize = highAccessEmployees.size();
    char** result = (char**)malloc(*returnSize * sizeof(char*));
    for (int i = 0; i < *returnSize; i++) {
        result[i] = strdup(highAccessEmployees[i].c_str());
    }
    return result;
}