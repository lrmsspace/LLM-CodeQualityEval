...
int maximizeWin(int* prizePositions, int prizePositionsSize, int k) {
      int n = prizePositionsSize;
        ...
        prefixSum[0] = 0;
        for (int i = 0; i < n; i++) {
            prefixSum[i + 1] = prefixSum[i] + prizePositions[i];
        }
        int result = 0;
        for (int i = 0; i <= k; i++) {
            int left = i, right = n - k + i;
            int sum = prefixSum[right] - prefixSum[left];
            result = fmax(result, sum);
            for (int j = 0; j <= i; j++) {
                int leftSum = prefixSum[j + 1] - prefixSum[0];
                int rightSum = prefixSum[n] - prefixSum[n - (k - j)];
                result = fmax(result, leftSum + rightSum);
            }
        }
        free(prefixSum);
        return result;      
