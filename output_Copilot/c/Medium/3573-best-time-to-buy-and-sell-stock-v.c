...
long long maximumProfit(int* prices, int pricesSize, int k) {
    ...
    // If k is larger than half the days, we can make unlimited transactions
    if (k > pricesSize / 2) {
    ...
    // DP array to store the maximum profit at each day with at most k transactions
    std::vector<std::vector<long long>> dp(k + 1, std::vector<long long>(pricesSize, 0));
