...
int minZeroArray(int* nums, int numsSize, int** queries, int queriesSize, int* queriesColSize) {
    ...
    // Process each query and check if we can make nums a Zero Array
    for (int k = 0; k < queriesSize; k++) {
        int l = queries[k][0];
        int r = queries[k][1];
        int val = queries[k][2];

        // Apply the query to the prefix sum
        for (int i = l; i <= r; i++) {
            prefixSum[i + 1] -= val;
        }

        // Check if all elements in nums are zero after this query
        if (prefixSum[numsSize] == 0) {
            free(prefixSum);
