...
int* concatenatedDivisibility(int* nums, int numsSize, int k, int* returnSize) {
    ...
    *returnSize = 0;
    ...
    // Sort the array to find the lexicographically smallest permutation
    qsort(nums, numsSize, sizeof(int), cmpfunc);

    // Check if the concatenated number is divisible by k
    long long concatenatedValue = 0;
    for (int i = 0; i < numsSize; i++) {
        concatenatedValue = concatenatedValue * pow(10, (int)log10(nums[i]) + 1) + nums[i];
    }

    if (concatenatedValue % k == 0) {
