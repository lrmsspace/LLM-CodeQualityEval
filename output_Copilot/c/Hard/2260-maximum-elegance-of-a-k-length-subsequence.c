        ...
        for (int mask = 0; mask < (1 << n); mask++) {
            ...
            for (int i = 0; i < n; i++) {
                if (mask & (1 << i)) {
                    ...
                    categories |= 1 << items[i][1];
                }
            }
            dp[mask] = totalProfit + __builtin_popcount(categories) * __builtin_popcount(categories);
        }
        for (int i = 0; i < n; i++) {
            for (int mask = 0; mask < (1 << n); mask++) {
                if (mask & (1 << i)) {
                    dp[mask] = fmax(dp[mask], dp[mask ^ (1 << i)]);
                }
            }
        }
        for (int mask = 0; mask < (1 << n); mask++) {
            if (__builtin_popcount(mask) == k) {
                answer = fmax(answer, dp[mask]);
            }
        }
        return answer;     
}