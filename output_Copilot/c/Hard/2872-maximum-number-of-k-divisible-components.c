...
int maxKDivisibleComponents(int n, int** edges, int edgesSize, int* edgesColSize, int* values, int valuesSize, int k) {
    ...
    int* subtreeSum = (int*)calloc(n, sizeof(int));
    int maxComponents = 0;
    ...
    void dfs(int node) {
        ...
        for (int i = 0; i < edgesSize; i++) {
            if ((edges[i][0] == node || edges[i][1] == node) && !visited[edges[i][0] == node ? edges[i][1] : edges[i][0]]) {
                int child = edges[i][0] == node ? edges[i][1] : edges[i][0];
                dfs(child);
    ...
    return maxComponents - 1; // Subtract one for the root component    
}