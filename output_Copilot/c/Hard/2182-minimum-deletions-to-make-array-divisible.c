...
int minOperations(int* nums, int numsSize, int* numsDivide, int numsDivideSize) {
    ...
    memset(count, 0, sizeof(int) * 100001);
    ...
    int *dp2 = (int *)malloc(sizeof(int) * 100001);
    memset(dp2, 0, sizeof(int) * 100001);
    for (int i = 1; i <= 100000; i++) {
        for (int j = i; j <= 100000; j += i) {
            dp2[j] = dp[i];
        }
    ...
    for (int i = 0; i < numsDivideSize; i++) {
        if (dp2[numsDivide[i]] == 0) {
            continue;
        }
        ans = fmin(ans, i - dp2[numsDivide[i]] + 1);
    }
