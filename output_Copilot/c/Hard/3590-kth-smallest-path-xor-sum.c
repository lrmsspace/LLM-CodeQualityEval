...
int* kthSmallest(int* par, int parSize, int* vals, int valsSize, int** queries, int queriesSize, int* queriesColSize, int* returnSize) {
    ...
    *returnSize = queriesSize;
    ...
    // Create a tree structure to hold the nodes and their values
    int n = parSize;
    int** tree = (int**)malloc(n * sizeof(int*));
    for (int i = 0; i < n; i++) {
        tree[i] = (int*)malloc(2 * sizeof(int)); // [parent, value]
        tree[i][0] = par[i];
        tree[i][1] = vals[i];
    }
    ...
    for (int q = 0; q < queriesSize; q++) {
        ...
        // Collect path XOR sums in the subtree rooted at u
        int* pathXorSums = (int*)malloc(n * sizeof(int));
        int count = 0;
        
        // DFS to find all nodes in the subtree of u
        void dfs(int node, int currentXor) {
            currentXor ^= tree[node][1]; // Update current XOR with node value
            pathXorSums[count++] = currentXor; // Store the current path XOR sum
            
            for (int i = 0; i < n; i++) {
                if (tree[i][0] == node) { // Check if this node is a child of the current node
                    dfs(i, currentXor);
                }
            }
        }
        
        dfs(u, 0); // Start DFS from node u
        
        // Find distinct path XOR sums and sort them
        qsort(pathXorSums, count, sizeof(int), cmpfunc);
        
        // Remove duplicates
        int distinctCount = 0;
        for (int i = 0; i < count; i++) {
            if (i == 0 || pathXorSums[i] != pathXorSums[i - 1]) {
                pathXorSums[distinctCount++] = pathXorSums[i];
            }
        }
        
        // Check if k is within bounds
        if (k > distinctCount) {
            result[q] = -1; // Not enough distinct sums
        } else {
            result[q] = pathXorSums[k - 1]; // k-th smallest distinct path XOR sum
            free(pathXorSums);
        }
    }
