...
int** buildMatrix(int k, int** rowConditions, int rowConditionsSize, int* rowConditionsColSize, int** colConditions, int colConditionsSize, int* colConditionsColSize, int* returnSize, int** returnColumnSizes) {
    ...
    int* col = (int*)malloc(sizeof(int) * k);
    memset(row, 0, sizeof(int) * k);
    memset(col, 0, sizeof(int) * k);
    int** matrix = (int**)malloc(sizeof(int*) * k);
    for (int i = 0; i < k; i++) {
        matrix[i] = (int*)malloc(sizeof(int) * k);
        memset(matrix[i], 0, sizeof(int) * k);
    }
    for (int i = 0; i < rowConditionsSize; i++) {
        int above = rowConditions[i][0] - 1;
        int below = rowConditions[i][1] - 1;
        if (row[above] >= k || row[below] >= k || row[above] >= row[below]) {
            *returnSize = 0;
            return matrix;
        }
        matrix[row[above]][above] = below + 1;
        row[above]++;
        row[below]++;
    }
    for (int i = 0; i < colConditionsSize; i++) {
        ...
        if (col[left] >= k || col[right] >= k || col[left] >= col[right]) {
            *returnSize = 0;
            return matrix;
        }
        matrix[left][col[left]] = right + 1;
        col[left]++;
        col[right]++;
    }
    *returnSize = k;
