...
int earliestSecondToMarkIndices(int* nums, int numsSize, int* changeIndices, int changeIndicesSize) {
    ...
    int marked[n + 1]; // 1-indexed
    for (int i = 1; i <= n; i++) {
        marked[i] = 0; // initially unmarked
    }
    
    for (int second = 1; second <= m; second++) {
        int idx = changeIndices[second - 1];
        if (marked[idx] == 0) {
            nums[idx - 1] = 0; // set to zero
        } else {
            nums[idx - 1] = nums[idx - 1] > 0 ? nums[idx - 1] - 1 : 0; // decrement if already marked
        }
        
        // Check if we can mark any index
        for (int i = 1; i <= n; i++) {
            if (nums[i - 1] == 0 && marked[i] == 0) {
                marked[i] = 1; // mark index
            }
        }
    ...
    return -1; // not possible to mark all indices    
}