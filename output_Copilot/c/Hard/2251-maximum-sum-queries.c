...
int* maximumSumQueries(int* nums1, int nums1Size, int* nums2, int nums2Size, int** queries, int queriesSize, int* queriesColSize, int* returnSize) {
        ...
        int* maximumSumQueries(int* nums1, int nums1Size, int* nums2, int nums2Size, int** queries, int queriesSize, int* queriesColSize, int* returnSize) {
            *returnSize = queriesSize;
            ...
            int* maxNums2 = (int*)malloc(nums1Size * sizeof(int));
            maxNums2[0] = nums2[pairs[0].index];
            for (int i = 1; i < nums1Size; i++) {
                maxNums2[i] = (nums2[pairs[i].index] > maxNums2[i - 1]) ? nums2[pairs[i].index] : maxNums2[i - 1];
            }
            ...
            for (int i = 0; i < queriesSize; i++) {
                int xi = queries[i][0];
                int yi = queries[i][1];
                while (j < nums1Size && nums1[pairs[j].index] >= xi) {
                    j++;
                }
                if (j == 0 || maxNums2[j - 1] < yi) {
                    result[i] = -1;
                } else {
                    result[i] = pairs[j - 1].sum;
                }
            }
