...
var lastVisitedIntegers = function(nums) {
    ...
    for (const num of nums) {
        if (num !== -1) {
            seen.unshift(num); // Prepend to the front of seen
            k = 0; // Reset k since we encountered a positive integer
        } else {
            k++; // Increment k for each -1 encountered
            if (k <= seen.length) {
                ans.push(seen[k - 1]); // Append the k-th element of seen to ans
            } else {
                ans.push(-1); // Append -1 if k is greater than the length of seen
            }
        }
    }
