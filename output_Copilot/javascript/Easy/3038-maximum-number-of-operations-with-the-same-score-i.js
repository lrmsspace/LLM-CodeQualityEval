...
var maxOperations = function(nums) {
    ...
    // Sort the array to easily find pairs with the same score
    nums.sort((a, b) => a - b);
    ...
    while (left < right) {
        const score = nums[left] + nums[left + 1];

        // Check if we can form a pair with the same score
        if (nums[right] + nums[right - 1] === score) {
            operations++;
            left += 2; // Move past the used elements
            right -= 2; // Move past the used elements
        } else {
            right--; // Try with a smaller right element
        }
    }
