...
var maxCompatibilitySum = function(students, mentors) {
    ...
    for (let i = 0; i < m; i++) {
        for (let j = 0; j < m; j++) {
            score[i][j] = countScore(students[i], mentors[j]);
        }
    }

    for (let mask = 0; mask < (1 << m); mask++) {
        const studentIndex = countBits(mask);
        if (studentIndex === m) continue;

        for (let mentorIndex = 0; mentorIndex < m; mentorIndex++) {
            if ((mask & (1 << mentorIndex)) === 0) {
                dp[mask | (1 << mentorIndex)] = Math.max(dp[mask | (1 << mentorIndex)], dp[mask] + score[studentIndex][mentorIndex]);
            }
        }
    }
