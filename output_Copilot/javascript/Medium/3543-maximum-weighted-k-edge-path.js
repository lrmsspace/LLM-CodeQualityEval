...
var maxWeight = function(n, edges, k, t) {
    ...
    for (let edgesUsed = 0; edgesUsed < k; edgesUsed++) {
        for (let u = 0; u < n; u++) {
            if (dp[u][edgesUsed] >= 0) {
                for (const [v, w] of graph[u]) {
                    const newWeight = dp[u][edgesUsed] + w;
                    if (newWeight < t) {
                        dp[v][edgesUsed + 1] = Math.max(dp[v][edgesUsed + 1], newWeight);
                    }
                }
            }
        }
