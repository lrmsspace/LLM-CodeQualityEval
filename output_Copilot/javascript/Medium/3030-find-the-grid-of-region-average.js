...
var resultGrid = function(image, threshold) {
    ...
    const regionCount = Array.from({ length: m }, () => Array(n).fill(0));
    ...
    function isValidRegion(i, j) {
        for (const [di, dj] of directions) {
            const ni = i + di;
            const nj = j + dj;
            if (ni >= 0 && ni < m && nj >= 0 && nj < n) {
                if (Math.abs(image[i][j] - image[ni][nj]) > threshold) {
                    return false;
                }
            }
        }
        ...
        for (let j = 0; j < n; j++) {
            if (isValidRegion(i, j)) {
                ...
                for (const [di, dj] of directions) {
                    const ni = i + di;
                    const nj = j + dj;
                    if (ni >= 0 && ni < m && nj >= 0 && nj < n && isValidRegion(ni, nj)) {
                        sum += image[ni][nj];
                        count++;
                        regionCount[ni][nj]++;
                    }
