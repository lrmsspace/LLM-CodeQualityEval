...
var getNumberOfBacklogOrders = function(orders) {
    ...
    for (const [price, amount, type] of orders) {
        if (type === 0) {
            ...
            while (amount > 0 && buy.length > 0 && buy[buy.length - 1][0] >= price) {
                const [buyPrice, buyAmount] = buy[buy.length - 1];
                if (amount >= buyAmount) {
                    amount -= buyAmount;
                    buy.pop();
                } else {
                    buy[buy.length - 1][1] -= amount;
                    amount = 0;
                }
            }
            if (amount > 0) {
    ...
    let total = 0;
    for (const [_, amount] of buy) total = (total + amount) % mod;
    for (const [_, amount] of sell) total = (total + amount) % mod;
    return total;    
};