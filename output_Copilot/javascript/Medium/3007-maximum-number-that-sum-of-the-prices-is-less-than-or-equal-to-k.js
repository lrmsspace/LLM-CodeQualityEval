...
var findMaximumNumber = function(k, x) {
    let left = 1, right = 10 ** 15, result = -1;

    while (left <= right) {
        const mid = Math.floor((left + right) / 2);
        const price = calculatePrice(mid, x);
        const accumulatedPrice = calculateAccumulatedPrice(mid, x);

        if (accumulatedPrice <= k) {
            result = mid; // Update result if the accumulated price is within the limit
            left = mid + 1; // Try to find a larger cheap number
        } else {
