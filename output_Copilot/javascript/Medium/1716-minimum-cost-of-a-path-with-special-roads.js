...
var minimumCost = function(start, target, specialRoads) {
    ...
    dp[0][0] = Math.abs(startX - targetX) + Math.abs(startY - targetY);
    for (let i = 0; i < n; i++) {
        const [x1, y1, x2, y2, cost] = specialRoads[i];
        for (let j = 0; j <= i; j++) {
            dp[i + 1][j] = Math.min(dp[i + 1][j], dp[i][j] + Math.abs(x1 - startX) + Math.abs(y1 - startY) + cost);
            dp[i][i + 1] = Math.min(dp[i][i + 1], dp[i][j] + Math.abs(x2 - x1) + Math.abs(y2 - y1) + cost);
        }
    }
    return dp[n][n];    
};