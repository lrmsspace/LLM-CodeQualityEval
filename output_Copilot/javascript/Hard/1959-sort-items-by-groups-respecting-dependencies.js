...
var sortItems = function(n, m, group, beforeItems) {
    ...
    for (const g of sortedGroups) {
        const itemsInGroup = groupItems[g];
        const sortedItems = topologicalSort(itemGraph, itemIndegree.slice());
        if (!sortedItems.length) return [];
        
        result.push(...sortedItems.filter(item => itemsInGroup.includes(item)));
    }
