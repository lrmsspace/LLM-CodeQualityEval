...
var kthSmallest = function(par, vals, queries) {
    ...
    const getXorSums = (node, currXor) => {
        currXor ^= vals[node];
        const xorSums = [currXor];
        for (const child of tree[node]) {
            xorSums.push(...getXorSums(child, currXor));
        }
        return xorSums;
    };
    ...
    for (const [u, k] of queries) {
        const xorSums = getXorSums(u, 0);
        const distinctXors = [...new Set(xorSums)].sort((a, b) => a - b);
        results.push(distinctXors[k - 1] || -1);
    }
    return results;
};