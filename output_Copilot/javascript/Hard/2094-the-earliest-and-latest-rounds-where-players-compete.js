...
var earliestAndLatest = function(n, firstPlayer, secondPlayer) {
    const earliest = (n, firstPlayer, secondPlayer) => {
        let round = 0;
        while (firstPlayer <= n && secondPlayer > 0) {
            round++;
            if (firstPlayer === secondPlayer) return round;
            if (firstPlayer > secondPlayer) [firstPlayer, secondPlayer] = [secondPlayer, firstPlayer];
            firstPlayer = Math.ceil(firstPlayer / 2);
            secondPlayer = Math.ceil((n - secondPlayer + 1) / 2);
        }
        return round;
    };

    const latest = (n, firstPlayer, secondPlayer) => {
        let round = 0;
        while (firstPlayer <= n && secondPlayer > 0) {
            round++;
            if (firstPlayer === secondPlayer) return round;
