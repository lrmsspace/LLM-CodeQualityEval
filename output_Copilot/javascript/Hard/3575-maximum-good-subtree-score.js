...
var goodSubtreeSum = function(vals, par) {
    ...
    const dfs = (node, usedDigits) => {
        let maxSum = 0;
        const currDigits = new Set();
        const currVal = vals[node];
        for (let digit of String(currVal)) {
            if (usedDigits.has(digit)) return 0; // Not a good subset
            currDigits.add(digit);
        }
        // Include current node
        maxSum += currVal;
        // Explore children
        for (let child of children[node]) {
            maxSum += dfs(child, new Set([...usedDigits, ...currDigits]));
        }
        return maxSum;
    };
