...
public:
    string generateTag(string caption) {
        ...
        caption.erase(0, caption.find_first_not_of(' '));
        caption.erase(caption.find_last_not_of(' ') + 1);

        // Step 2: Convert to camelCase and prefix with '#'
        string tag = "#";
        ...
        for (char c : caption) {
            if (c == ' ') {
                ...
                if (capitalizeNext) {
                    tag += toupper(c); // Capitalize the character
                    capitalizeNext = false;
                } else {
                    tag += tolower(c); // Keep the character lowercase
                }
            }
        }

        // Step 3: Truncate to a maximum of 100 characters
        if (tag.length() > 100) {
