...
public:
    vector<vector<int>> resultGrid(vector<vector<int>>& image, int threshold) {
        ...
        vector<vector<int>> regionSum(m, vector<int>(n, 0));
        vector<vector<int>> regionCount(m, vector<int>(n, 0));

        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                if (i > 0 && abs(image[i][j] - image[i - 1][j]) <= threshold) {
                    regionSum[i][j] += regionSum[i - 1][j] + image[i][j];
                    regionCount[i][j] += regionCount[i - 1][j] + 1;
                } else {
                    regionSum[i][j] += image[i][j];
                    regionCount[i][j] += 1;
                }
                if (j > 0 && abs(image[i][j] - image[i][j - 1]) <= threshold) {
                    regionSum[i][j] += regionSum[i][j - 1] + image[i][j];
                    regionCount[i][j] += regionCount[i][j - 1] + 1;
                } else {
                    regionSum[i][j] += image[i][j];
                    regionCount[i][j] += 1;
                }
