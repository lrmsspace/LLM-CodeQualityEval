...
public:
    vector<int> colorTheArray(int n, vector<vector<int>>& queries) {
        ...
        unordered_map<int, vector<int>> colorIndexMap;
        for (int i = 0; i < n; i++) {
            colorIndexMap[colors[i]].push_back(i);
        }
        for (auto& query : queries) {
            ...
            for (int i = -1; i <= 1; i++) {
                if (index + i >= 0 && index + i < n) {
                    colorIndexMap[oldColor].erase(find(colorIndexMap[oldColor].begin(), colorIndexMap[oldColor].end(), index + i));
                    colorIndexMap[color].push_back(index + i);
                }
            }
            int count = 0;
            for (int i = -1; i <= 1; i++) {
                if (index + i >= 0 && index + i < n) {
                    if (colors[index + i] == color) {
                        count++;
                    }
                }
            }
