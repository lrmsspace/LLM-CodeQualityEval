...
public:
    double maxAmount(string initialCurrency, vector<vector<string>>& pairs1, vector<double>& rates1, vector<vector<string>>& pairs2, vector<double>& rates2) {
        ...
        for (int i = 0; i < pairs1.size(); i++) {
            ...
            if (day1.count(from)) {
                day1[to] = max(day1[to], day1[from] * rate);
            }
        }

        // Day 2 conversions
        day2[initialCurrency] = 1.0;
        for (int i = 0; i < pairs2.size(); i++) {
            ...
            if (day2.count(from)) {
                day2[to] = max(day2[to], day2[from] * rate);
            }
        }

        // Calculate maximum amount after both days
        double maxAmount = 0.0;
        for (const auto& [currency, amount] : day1) {
            if (day2.count(currency)) {
                maxAmount = max(maxAmount, amount * day2[currency]);
            }
        }
