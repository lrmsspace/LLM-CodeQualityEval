...
public:
    int minimumChanges(string s, int k) {
        int n = s.length();
        if (k > n / 2) return -1; // Not enough characters to form k substrings
        ...
        for (int i = 1; i <= k; ++i) {
            for (int j = i; j <= n; ++j) {
                for (int d = 1; d < j; ++d) { // Check all divisors
                    int changes = 0;
                    for (int m = 0; m < d; ++m) {
                        int count[26] = {0};
                        for (int l = m; l < j; l += d) {
                        ...
                        changes += (j / d) - maxCount; // Changes needed to make this group a palindrome
                    }
                    dp[i][j] = min(dp[i][j], dp[i - 1][j - d] + changes);
                }
            }
        }
