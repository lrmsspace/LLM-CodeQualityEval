...
public:
    double minTime(int n, int k, int m, vector<int>& time, vector<double>& mul) {
        if (k == 1 && n > 1) return -1.0; // If k is 1 and n > 1, it's impossible to transport all individuals

        double totalTime = 0.0;
        int currentStage = 0;

        while (n > 0) {
            int groupSize = min(k, n);
            double maxTime = 0.0;

            // Calculate the maximum time for the current group
            for (int i = 0; i < groupSize; ++i) {
                maxTime = max(maxTime, time[n - 1 - i]);
            }
            ...
            // Update the stage
            currentStage = (currentStage + static_cast<int>(floor(crossingTime))) % m;

            // Reduce the number of individuals left
            n -= groupSize;

            // If there are still individuals left, one must return
            if (n > 0) {
                double returnTime = time[n] * mul[currentStage];
                totalTime += returnTime;
                currentStage = (currentStage + static_cast<int>(floor(returnTime))) % m;
                n++; // The returning individual is counted again
            }
        }
