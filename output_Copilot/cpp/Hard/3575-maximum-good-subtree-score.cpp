...
public:
    int goodSubtreeSum(vector<int>& vals, vector<int>& par) {
        ...
        vector<long long> maxScore(n, 0);
        const int MOD = 1e9 + 7;

        function<void(int, int)> dfs = [&](int node, int mask) {
            long long currentSum = vals[node];
            int currentMask = mask | getDigitMask(vals[node]);
            for (int child : tree[node]) {
                dfs(child, currentMask);
                currentSum += maxScore[child];
            }
            maxScore[node] = currentSum % MOD;
        };
