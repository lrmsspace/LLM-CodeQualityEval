...
public:
    int minTravelTime(int l, int n, int k, vector<int>& position, vector<int>& time) {
        ...
        dp[0] = 0; // Starting point, no time needed to reach the first sign

        for (int i = 1; i < n; ++i) {
            for (int j = 0; j < i; ++j) {
                int distance = position[i] - position[j];
                int travelTime = distance * time[j];
                dp[i] = min(dp[i], dp[j] + travelTime);
            }
        }

        // Now we need to perform exactly k merges
        for (int m = 0; m < k; ++m) {
            vector<int> newDp(n, INT_MAX);
            for (int i = 0; i < n - 1; ++i) {
                if (i + 1 < n) {
                    newDp[i + 1] = min(newDp[i + 1], dp[i] + time[i]);
                }
                if (i + 2 < n) {
                    newDp[i + 2] = min(newDp[i + 2], dp[i] + time[i] + time[i + 1]);
                }
            }
            dp.swap(newDp);
        }

        return dp[n - 1]; // Minimum travel time to reach the last sign
