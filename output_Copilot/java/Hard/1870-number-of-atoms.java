    ...
    public String countOfAtoms(String formula) {
        ...
        while (i < formula.length()) {
            ...
            if (Character.isUpperCase(c)) {
                ...
                if (count == 0) count = 1;
                atomCount.put(atom.toString(), atomCount.getOrDefault(atom.toString(), 0) + count);
            } else if (c == '(') {
                stack.push(new HashMap<>(atomCount));
                i++;
            } else if (c == ')') {
                i++;
                int count = 0;
                ...
                for (String atom : top.keySet()) {
                    atomCount.put(atom, atomCount.getOrDefault(atom, 0) + top.get(atom) * count);
                }
            }
        }
