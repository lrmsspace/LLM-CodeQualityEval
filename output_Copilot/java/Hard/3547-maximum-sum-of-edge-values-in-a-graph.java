...
class Solution {
    public long maxScore(int n, int[][] edges) {
        ...
        Arrays.sort(nodes, (a, b) -> Integer.compare(degree[b], degree[a]));
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int[] edge : edges) {
                if (edge[0] == node || edge[1] == node) {
                    int otherNode = edge[0] == node ? edge[1] : edge[0];
                    score += (long) value * (n - Arrays.asList(nodes).indexOf(otherNode));
                }
            }
        }
