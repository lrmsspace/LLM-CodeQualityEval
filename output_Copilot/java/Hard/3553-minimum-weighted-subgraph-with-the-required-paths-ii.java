...
class Solution {
    public int[] minimumWeight(int[][] edges, int[][] queries) {
        ...
        int[][] graph = new int[n][];
        for (int[] edge : edges) {
            graph[edge[0]] = addEdge(graph[edge[0]], edge[1], edge[2]);
            graph[edge[1]] = addEdge(graph[edge[1]], edge[0], edge[2]);
        }

        int[] result = new int[queries.length];
        for (int i = 0; i < queries.length; i++) {
            result[i] = findMinimumWeight(graph, queries[i][0], queries[i][1], queries[i][2]);
        }
        return result;
