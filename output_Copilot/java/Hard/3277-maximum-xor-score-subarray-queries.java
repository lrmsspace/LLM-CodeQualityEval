...
class Solution {
    public int[] maximumSubarrayXor(int[] nums, int[][] queries) {
        ...
        // Precompute the maximum XOR score for all subarrays
        int[][] maxXor = new int[n][n];
        
        for (int i = 0; i < n; i++) {
            maxXor[i][i] = nums[i];
            for (int j = i + 1; j < n; j++) {
                maxXor[i][j] = maxXor[i][j - 1] ^ nums[j];
            }
        }
        ...
        for (int i = 0; i < q; i++) {
            ...
            // Find the maximum XOR score in the subarray nums[li..ri]
            for (int start = li; start <= ri; start++) {
                for (int end = start; end <= ri; end++) {
                    maxScore = Math.max(maxScore, maxXor[start][end]);
                }
            }
