...
class Solution {
    public int findCrossingTime(int n, int k, int[][] time) {
        int[][] workers = new int[k][4];
        for (int i = 0; i < k; i++) {
            workers[i][0] = time[i][0] + time[i][1]; // right + pick
            workers[i][1] = time[i][2] + time[i][3]; // left + put
            workers[i][2] = i; // index
            workers[i][3] = time[i][0] + time[i][2]; // right + left
        }
        Arrays.sort(workers, (a, b) -> a[3] - b[3]);
        PriorityQueue<int[]> pq = new PriorityQueue<>((a, b) -> a[1] - b[1]);
        int ans = 0;
        for (int i = 0; i < n; i++) {
            while (!pq.isEmpty() && pq.peek()[1] <= ans) {
                pq.poll();
            }
            if (i < k) {
                pq.offer(new int[]{i, ans + workers[i][0]});
                ans += workers[i][0];
            } else if (!pq.isEmpty()) {
                int[] worker = pq.poll();
                ans += worker[1];
                pq.offer(new int[]{worker[0], ans + workers[worker[0]][1]});
            }
        }
        while (!pq.isEmpty()) {
            ans += pq.poll()[1];
