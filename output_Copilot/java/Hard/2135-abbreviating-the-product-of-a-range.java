...
class Solution {
    public String abbreviateProduct(int left, int right) {
        ...
        for (int i = left; i <= right; i++) {
            ...
            // Count digits
            digitCount += String.valueOf(product).length();
        }
        
        // Remove trailing zeros from product
        while (product % 10 == 0) {
            product /= 10;
        }
        ...
        if (digitCount > 10) {
            String pre = productStr.substring(0, Math.min(5, productStr.length()));
            String suf = productStr.substring(Math.max(0, productStr.length() - 5), productStr.length());
            return pre + "..." + suf + "e" + trailingZeros;
        } else {
