...
class Solution {
    public double minTime(int n, int k, int m, int[] time, double[] mul) {
        if (k == 1 && n > 1) {
            return -1; // If k is 1 and n > 1, it's impossible to transport all individuals.
        }
        ...
        dp[0][0] = 0; // Starting point, no individuals transported.

        for (int mask = 0; mask < (1 << n); mask++) {
            for (int stage = 0; stage < m; stage++) {
                if (dp[mask][stage] == Double.MAX_VALUE) continue;

                // Try all combinations of individuals to send
                for (int submask = 1; submask < (1 << n); submask++) {
                    if ((submask & mask) != 0 || Integer.bitCount(submask) > k) continue;
                    ...
                    int nextStage = (stage + (int) Math.floor(crossingTime)) % m;

                    dp[nextMask][nextStage] = Math.min(dp[nextMask][nextStage], dp[mask][stage] + crossingTime);

                    // Handle return trip
                    for (int r : submaskToReturn(submask)) {
                        double returnTime = time[r] * mul[nextStage];
                        int returnStage = (nextStage + (int) Math.floor(returnTime)) % m;
                        dp[mask][returnStage] = Math.min(dp[mask][returnStage], dp[nextMask][nextStage] + returnTime);
                    }
                }
            }
        }
        ...
        for (int stage = 0; stage < m; stage++) {
            result = Math.min(result, dp[(1 << n) - 1][stage]);
        }
