...
class Solution {
    public int mostFrequentPrime(int[][] mat) {
        ...
        // Traverse the matrix and collect numbers
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                collectNumbers(mat, i, j, primeCount, primes);
            }
        }
        ...
        for (Map.Entry<Integer, Integer> entry : primeCount.entrySet()) {
            if (entry.getKey() > 10 && entry.getValue() > maxCount) {
                maxCount = entry.getValue();
                result = entry.getKey();
            } else if (entry.getKey() > 10 && entry.getValue() == maxCount) {
