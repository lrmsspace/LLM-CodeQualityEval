...
class Solution {
    public int maxPointsInsideSquare(int[][] points, String s) {
        ...
        // Iterate through all possible side lengths of the square
        for (int sideLength = 0; sideLength <= 2000000000; sideLength++) {
            int count = 0;
            Set<Character> tags = new HashSet<>();

            // Check each point to see if it lies within the square
            for (int i = 0; i < n; i++) {
                int x = points[i][0];
                int y = points[i][1];
                char tag = s.charAt(i);
