...
class Solution {
    public int removeDuplicates(int[] nums) {
        int n = nums.length;
        if (n <= 2) return n; // If the array has 2 or fewer elements, return its length
        ...
        for (int i = 2; i < n; i++) {
            if (nums[i] == nums[index]) {
                count++;
                if (count <= 2) {
                    nums[++index] = nums[i]; // Place the element at the next index
                }
            } else {
                count = 1; // Reset count for a new unique element
                nums[++index] = nums[i]; // Place the new unique element
            }
        }
