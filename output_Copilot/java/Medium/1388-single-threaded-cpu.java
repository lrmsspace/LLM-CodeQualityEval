...
class Solution {
    public int[] getOrder(int[][] tasks) {
           int n = tasks.length;
            ...
            Arrays.sort(indexedTasks, (a, b) -> a[0] - b[0]); // Sort by enqueueTime
            ...
            while (index < n) {
                ...
                if (!minHeap.isEmpty()) {
                    ...
                    result[index++] = task[2]; // Store the original index
                    time += task[1];           // Update time with processingTime
                } else {
                    time = indexedTasks[i][0];  // Jump to the next available task's enqueueTime
                }
            }
            return result; // Return the order of processed tasks     
    }
}