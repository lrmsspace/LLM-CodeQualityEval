...
class Solution {
    public List<Integer> lastVisitedIntegers(int[] nums) {
        ...
        for (int num : nums) {
            if (num != -1) {
                seen.add(0, num); // Prepend to the front of seen
                count = 0; // Reset count when a positive integer is encountered
            } else {
                count++;
                if (count <= seen.size()) {
                    ans.add(seen.get(count - 1)); // Append the k-th element of seen
                } else {
                    ans.add(-1); // Append -1 if k is greater than the size of seen
                }
            }
        }
